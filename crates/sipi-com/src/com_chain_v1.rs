...
use crate::build_noise_pdf_v1::{build_r480_noise_pdf_v1, R480NoisePdfV1};
use crate::combined_noise_pdf_v1::{combine_r480_noise_pdf_v1, CombinedNoisePdfV1};
use crate::com_metrics_v1::{calculate_com_metrics_v1, ComMetricsV1};
use crate::discrete_pdf_v1::PdfErrorV1;
use crate::equalizer_frontend_v1::{cursor_sample_index_v1, CursorSampleV1, EqualizerErrorV1};
use crate::residual_channel_pdf_v1::{residual_channel_pdf_v1, ResidualPdfResultV1};
...
pub fn run_com_chain_v1(
    ...
    let cursor_index = cursor
        .cursor_index()
        .ok_or(ComChainErrorV1::Equalizer)? as usize;
...
mod tests {
    ...
    fn controls() -> ComChainControlsV1 {
        ComChainControlsV1::try_new(
            8, 4, 0.01, 0.0, "MM", 16, Some(44), 0, Vec::new(), Vec::new(), 0.0, false,
            None, 0.5, 50.0, 30.0, 0.03, 1e-4, vec![0.3, 0.5, 0.2], 0.01, 0.4, 1e-4, 0.0,
            0.0, None, None, None, 3.0, 0.0, None,
        )
        .expect("controls")
    }
    ...
    fn invalid_controls_rejected() {
        let cases: Vec<ComChainErrorV1> = vec![
            ComChainControlsV1::try_new(
                0, 4, 0.01, 0.0, "peak", 20, Some(40), 0, Vec::new(), Vec::new(), 0.0, false,
                None, 0.5, 50.0, 30.0, 0.03, 1e-4, vec![0.3], 0.01, 0.4, 1e-4, 0.0, 0.0,
                None, None, None, 3.0, 0.0, None,
            )
            .expect_err("spu"),
            ComChainControlsV1::try_new(
                8, 1, 0.01, 0.0, "peak", 20, Some(40), 0, Vec::new(), Vec::new(), 0.0, false,
                None, 0.5, 50.0, 30.0, 0.03, 1e-4, vec![0.3], 0.01, 0.4, 1e-4, 0.0, 0.0,
                None, None, None, 3.0, 0.0, None,
            )
            .expect_err("levels"),
            ComChainControlsV1::try_new(
                8, 4, 0.0, 0.0, "peak", 20, Some(40), 0, Vec::new(), Vec::new(), 0.0, false,
                None, 0.5, 50.0, 30.0, 0.03, 1e-4, vec![0.3], 0.01, 0.4, 1e-4, 0.0, 0.0,
                None, None, None, 3.0, 0.0, None,
            )
            .expect_err("bin"),
            ComChainControlsV1::try_new(
                8, 4, 0.01, 0.0, "peak", 20, Some(40), 0, Vec::new(), Vec::new(), 0.0, false,
                None, 0.5, 50.0, 30.0, 0.03, 1e-4, vec![], 0.01, 0.4, 1e-4, 0.0, 0.0,
                None, None, None, 3.0, 0.0, None,
            )
            .expect_err("jitter"),
            ComChainControlsV1::try_new(
                8, 4, 0.01, 0.0, "peak", 20, Some(40), 0, Vec::new(), Vec::new(), 0.0, false,
                None, 0.5, 50.0, 30.0, 0.03, 1e-4, vec![0.3], 0.01, 0.4, 1.5, 0.0, 0.0,
                None, None, None, 3.0, 0.0, None,
            )
            .expect_err("spec_ber"),
        ];
