...
use crate::receiver_noise_v1::{rx_ffe_frequency_response_v1, NoiseErrorV1};

/// Explicit scope policy of the crosstalk noise stage.
pub const CROSSTALK_NOISE_POLICY_V1: &str =
        ...
        match role.as_str() {
            "FEXT" => {
                ...
                for index in 0..index_f2 {
                    let weight = sinc[index] * sinc[index] * magnitude(tx_filter[index]) * magnitude(h_rx_ffe[index]);
...
pub fn td_source_crosstalk_noise_v1(
    ...
    let weight_fext: Vec<f64> = (0..count)
        .map(|index| {
            let value = sinc[index] * sinc[index] * magnitude(tx_filter[index]);
            value
        })
        .collect();
    ...
    let response_row_count = channels.first().map(|(_, response, _)| response.len()).unwrap_or(0);
    for (role, response, amplitude) in channels {
...
mod tests {
    ...
    fn empty_channels_zero() {
        ...
        let h_ctf: Vec<Complex64> = (0..32).map(|index| complex(1.0, index as f64 * 1e-3)).collect();
        let parameters = XtalkParamsV1 { fb: 26.5625e9, f2: 26.5625e9, sigma_x: 0.03 };
        let noise = crosstalk_noise_v1(&frequency, &h_ctf, &[1.0], &[], &parameters, false, None, None)
            .expect("noise");
        assert_eq!(noise, 0.0);
    }
    ...
    fn fext_next_integration() {
        let frequency: Vec<f64> = (0..32).map(|index| index as f64 * 1e9).collect();
        let h_ctf: Vec<Complex64> = frequency.iter().map(|value| complex(1.0, value * 1e-11)).collect();
        let fext: Vec<Complex64> = frequency.iter().map(|value| complex(0.5 * value.cos(), 0.0)).collect();
        let next: Vec<Complex64> = frequency.iter().map(|value| complex(0.3 * value.sin(), 0.0)).collect();
        let parameters = XtalkParamsV1 { fb: 26.5625e9, f2: 26.5625e9, sigma_x: 0.03 };
        let channels = vec![
            ("FEXT".to_string(), fext, 0.5),
            ("NEXT".to_string(), next, 0.4),
        ];
        let noise = crosstalk_noise_v1(&frequency, &h_ctf, &[0.5, 1.0, -0.25], &channels, &parameters, false, None, None)
            .expect("noise");
    ...
    fn td_outer_product_path() {
        let frequency: Vec<f64> = (0..32).map(|index| index as f64 * 1e9).collect();
        let h_ctf: Vec<Complex64> = frequency.iter().map(|value| complex(1.0, value * 1e-11)).collect();
        let tx_filter: Vec<Complex64> = frequency.iter().map(|value| complex(value.cos(), value.sin())).collect();
        let sinc: Vec<f64> = frequency.iter().map(|value| {
            let angle = std::f64::consts::PI * value / 26.5625e9;
            if angle == 0.0 { 1.0 } else { angle.sin() / angle }
        }).collect();
        let fext: Vec<Complex64> = frequency.iter().map(|value| complex(0.4 * value.cos(), 0.1)).collect();
        let parameters = XtalkParamsV1 { fb: 26.5625e9, f2: 26.5625e9, sigma_x: 0.03 };
    ...
    fn role_and_frequency_errors() {
        let frequency: Vec<f64> = (0..32).map(|index| index as f64 * 1e9).collect();
        let h_ctf: Vec<Complex64> = (0..32).map(|index| complex(1.0, 0.0)).collect();
        let parameters = XtalkParamsV1 { fb: 26.5625e9, f2: 26.5625e9, sigma_x: 0.03 };
        let channels = vec![("XTLK".to_string(), h_ctf.clone(), 0.5)];
        assert!(crosstalk_noise_v1(&frequency, &h_ctf, &[1.0], &channels, &parameters, false, None, None).is_err());
        let short: Vec<f64> = (0..5).map(|index| index as f64 * 1e9).collect();
        assert!(crosstalk_noise_v1(&short, &h_ctf, &[1.0], &channels, &parameters, false, None, None).is_err());
        assert!(crosstalk_noise_v1(&frequency, &h_ctf, &[1.0], &channels, &parameters, false, Some(&[1.0]), None).is_err());
    }
