...
pub use prbs9_impulse_candidate_v1::{
    ...
    SelectedP3cPrbs9ImpulseCandidateV1, generate_selected_p3c_prbs9_impulse_candidate_v1,
};
...
pub const SELECTED_P3C_S4P_SHA256_V1: &str =
...
impl SelectedP3cSealedS4pIdentityV1 {
    ...
    ) -> Result<Self, SelectedP3cSealedS4pAdmissionErrorV1> {
...
impl SelectedP3cSealedS4pIdentityV2 {
    pub fn try_new(
        artifact_id: impl Into<String>,
        manifest_sha256: impl Into<String>,
    ) -> Result<Self, SelectedP3cSealedS4pAdmissionErrorV2> {
        let artifact_id = artifact_id.into();
        let manifest_sha256 = manifest_sha256.into();
        if artifact_id.is_empty()
            || artifact_id.len() > 128
...
impl AdmittedSelectedP3cStaticTransferV1 {
    ...
    pub fn transfer(&self) -> &SelectedP3cStaticDifferentialTransferV1 {
        &self.transfer
    }
}
...
impl fmt::Display for SelectedP3cSealedS4pAdmissionErrorV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "selected P3C sealed S4P admission failed: {self:?}"
        )
    }
}
...
impl fmt::Display for SelectedP3cSealedS4pAdmissionErrorV2 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "selected P3C sealed S4P v2 admission failed: {self:?}"
        )
    }
}
...
fn parser_limits() -> TouchstoneParseLimitsV1 {
    TouchstoneParseLimitsV1::new(
        NonZeroUsize::new(SELECTED_P3C_S4P_BYTE_LENGTH_V1 as usize)
            .expect("selected length is nonzero"),
...
pub fn admit_selected_p3c_sealed_s4p_v1(
    ...
    let bytes = files
        .file(SELECTED_P3C_S4P_FILE_NAME_V1)
        .ok_or(SelectedP3cSealedS4pAdmissionErrorV1::SourceLengthMismatch)?;
    let (transfer, record_count) = admit_bytes(
        bytes,
        SELECTED_P3C_S4P_BYTE_LENGTH_V1,
...
pub fn admit_selected_p3c_sealed_s4p_v2(
    ...
    let bytes = files
        .file(SELECTED_P3C_S4P_FILE_NAME_V1)
        .ok_or(SelectedP3cSealedS4pAdmissionErrorV2::SourceLengthMismatch)?;
...
mod tests {
    ...
    fn miniature_profile_exercises_exact_seal_parse_and_reduce_chain() {
        ...
        let files = reader
            .consume_exact_verified_v1(
                "network-1",
                &sha256(&manifest),
                &[(SELECTED_P3C_S4P_FILE_NAME_V1, payload.len() as u64)],
                VerifiedConsumptionPolicyV1::try_new(65_536, payload.len() as u64).unwrap(),
            )
            .unwrap();
        let (transfer, records) = admit_bytes(
            files.file(SELECTED_P3C_S4P_FILE_NAME_V1).unwrap(),
            payload.len() as u64,
            &sha256(&payload),
        )
        .unwrap();
    ...
    fn production_profile_rejects_synthetic_or_mutated_sealed_content() {
        ...
        assert!(matches!(
            admit_selected_p3c_sealed_s4p_v1(&reader, &identity),
            Err(SelectedP3cSealedS4pAdmissionErrorV1::Artifact(_))
                | Err(SelectedP3cSealedS4pAdmissionErrorV1::SourceLengthMismatch)
        ));
        fs::remove_dir_all(root).unwrap();
    }
