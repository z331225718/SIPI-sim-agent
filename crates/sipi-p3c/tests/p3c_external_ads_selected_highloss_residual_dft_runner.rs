...
fn factorized_dft(values: &[Complex], factors: &[usize]) -> Result<Vec<Complex>, String> {
    ...
    if radix < 2 || !length.is_multiple_of(radix) {
    ...
    for frequency_inner in 0..inner {
        for frequency_outer in 0..radix {
            ...
            for branch in 0..radix {
                let angle =
                    -2.0 * std::f64::consts::PI * (frequency * branch) as f64 / length as f64;
                let (sin, cos) = angle.sin_cos();
                sum = sum
                    .add(subtransforms[branch][frequency_inner].mul(Complex { re: cos, im: sin }));
            }
...
fn weighted_energy(spectrum: &[Complex], start: usize, end: usize) -> Result<f64, String> {
    if spectrum.len() != THIRD_SAMPLES || start >= end || end > ONE_SIDED_BINS {
        return Err("dft_band".to_owned());
    }
    let mut sum = 0.0;
    for index in start..end {
        let weight = if index == 0 || index == THIRD_SAMPLES / 2 {
            1.0
        } else {
            2.0
        };
        let energy = spectrum[index]
            .norm_sqr()
            .ok_or_else(|| "dft_numeric".to_owned())?;
        sum += weight * energy;
        if !sum.is_finite() {
            return Err("dft_numeric".to_owned());
        }
...
fn band_fact(
    ...
    if reference_energy == 0.0 {
        return Err("zero_band_reference".to_owned());
    }
...
fn run_once(source: &Path, reference_path: &Path, index: usize) -> Result<RunFact, String> {
    ...
    let result = (|| {
        ...
        let time = compare_selected_highloss_prbs9_waveform_only_v3(&pair)
        ...
        let frequency_nrmse = f64::from_bits(
            u64::from_str_radix(&total.residual_to_reference_sqrt_bits, 16)
                .map_err(|_| "dft_bits".to_owned())?,
        );
        let absolute = (frequency_nrmse - time).abs();
        let relative = absolute / time.abs();
        if !(absolute <= ABS_INTEGRITY_LIMIT || relative <= REL_INTEGRITY_LIMIT) {
            return Err("parseval_integrity".to_owned());
        }
        let mut maximum_energy = f64::NEG_INFINITY;
        let mut maximum_residual_energy_bin = 0;
        for (index, value) in residual_spectrum[..ONE_SIDED_BINS]
            .iter()
            .copied()
            .enumerate()
        {
            ...
            if energy > maximum_energy {
                ...
                maximum_residual_energy_bin = index;
            }
        }
...
fn band_json(band: &BandFact) -> String {
    format!(
        "{{\"reference_energy_bits\":\"{}\",\"candidate_energy_bits\":\"{}\",\"residual_energy_bits\":\"{}\",\"residual_to_reference_sqrt_bits\":\"{}\"}}",
        band.reference_energy_bits,
        band.candidate_energy_bits,
        band.residual_energy_bits,
        band.residual_to_reference_sqrt_bits
    )
}

fn run_json(fact: &RunFact) -> String {
    format!(
        "{{\"source_manifest_sha256\":\"{}\",\"record_count\":{},\"reference_rx_payload_sha256\":\"{}\",\"candidate_prefix_sha256\":\"{}\",\"time_nrmse_bits\":\"{}\",\"frequency_nrmse_bits\":\"{}\",\"reference_spectrum_sha256\":\"{}\",\"candidate_spectrum_sha256\":\"{}\",\"residual_spectrum_sha256\":\"{}\",\"maximum_residual_energy_bin\":{},\"bands\":[{},{},{},{}]}}",
        fact.source_manifest_sha256,
        fact.record_count,
        fact.reference_rx_payload_sha256,
        fact.candidate_prefix_sha256,
        fact.time_nrmse_bits,
        fact.frequency_nrmse_bits,
        fact.reference_spectrum_sha256,
        fact.candidate_spectrum_sha256,
        fact.residual_spectrum_sha256,
        fact.maximum_residual_energy_bin,
    ...
    for (frequency, value) in actual.iter().enumerate() {
        let expected = input
            ...
            .fold(Complex::ZERO, |sum, (index, sample)| {
                let angle =
                    -2.0 * std::f64::consts::PI * (frequency * index) as f64 / input.len() as f64;
                let (sin, cos) = angle.sin_cos();
                sum.add(Complex {
                    re: *sample * cos,
                    im: *sample * sin,
...
fn one_sided_band_edges_and_parseval_weights_are_fixed() {
    ...
    assert_eq!(
        weighted_energy(&spectrum, 0, 1).unwrap().to_bits(),
        (9.0 / (THIRD_SAMPLES as f64).powi(2)).to_bits()
    );
...
fn p3c_external_ads_selected_highloss_residual_dft_runner_v2() {
    ...
    assert_eq!(
        first.reference_spectrum_sha256,
        second.reference_spectrum_sha256
    );
    assert_eq!(
        first.candidate_spectrum_sha256,
        second.candidate_spectrum_sha256
    );
    assert_eq!(
        first.residual_spectrum_sha256,
        second.residual_spectrum_sha256
    );
    assert_eq!(
        first.maximum_residual_energy_bin,
        second.maximum_residual_energy_bin
    );
    assert_eq!(first.bands, second.bands);
    let payload = format!(
        "{{\"schema\":\"{REPORT_SCHEMA}\",\"status\":\"observed\",\"source_byte_length\":{},\"source_sha256\":\"{}\",\"ads_canonical_triple_payload_sha256\":\"{ADS_CANONICAL_SHA256}\",\"contract_sha256\":\"{SELECTED_HIGHLOSS_PRBS9_WAVEFORM_ONLY_CONTRACT_SHA256_V3}\",\"fixed_dft\":{{\"samples\":{THIRD_SAMPLES},\"sample_interval_bits\":\"{DT_BITS:016x}\",\"window\":\"rectangular\",\"forward_sign\":\"negative\",\"normalization\":\"none\",\"factorization\":[32,7,73],\"bands\":[[0,1],[1,256],[256,639],[639,{ONE_SIDED_BINS}]]}},\"source_reference_identity_checks\":\"before_stage_after_equal\",\"fresh_runs\":[{},{}],\"cleanup_status\":\"complete\"}}\n",
        SELECTED_P3C_S4P_BYTE_LENGTH_V1,
