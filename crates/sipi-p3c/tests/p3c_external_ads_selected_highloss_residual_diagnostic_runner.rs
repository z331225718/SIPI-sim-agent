...
use sipi_compare::selected_highloss_prbs9_waveform_only_v3::{
    ...
    diagnose_selected_highloss_prbs9_residual_v1, SelectedHighlossPrbs9WaveformPairV3,
};
use sipi_contracts::{
...
fn candidate_values(source: &Path, root: &Path, index: usize) -> Result<(usize, String, Vec<f64>), String> {
    ...
    let candidate = generate_selected_p3c_prbs9_impulse_candidate_v1(&truncated).map_err(|_| "candidate_convolution".to_owned())?;
...
fn run_once(source: &Path, reference_path: &Path, index: usize) -> Result<RunFact, String> {
    ...
    let result = (|| {
        let (record_count, source_manifest_sha256, candidate) = candidate_values(source, &root, index)?;
        ...
        let pair = SelectedHighlossPrbs9WaveformPairV3::try_new(reference, candidate).map_err(|_| "diagnostic_pair".to_owned())?;
        let report = compare_selected_highloss_prbs9_waveform_only_v3(&pair).map_err(|_| "diagnostic_v3".to_owned())?;
        let diagnostic = diagnose_selected_highloss_prbs9_residual_v1(&pair).map_err(|_| "diagnostic_residual".to_owned())?;
        if diagnostic.periods()[2].residual_nrmse().to_bits() != report.waveform_nrmse().to_bits() { return Err("third_period_nrmse_mismatch".to_owned()); }
        let periods = diagnostic.periods().iter().map(|period| PeriodFact {
            reference_rms_bits: format!("{:016x}", period.reference_rms().to_bits()), candidate_rms_bits: format!("{:016x}", period.candidate_rms().to_bits()), residual_rms_bits: format!("{:016x}", period.residual_rms().to_bits()), residual_mean_bits: format!("{:016x}", period.residual_mean().to_bits()), residual_nrmse_bits: format!("{:016x}", period.residual_nrmse().to_bits()), residual_digest: period.residual_digest().to_owned(), maximum_absolute_residual_bits: format!("{:016x}", period.maximum_absolute_residual().to_bits()), maximum_absolute_residual_offset: period.maximum_absolute_residual_offset(),
        }).collect();
        Ok(RunFact { source_manifest_sha256, record_count, reference_rx_payload_sha256, candidate_prefix_sha256, periods, third_period_ui_energy_digest: diagnostic.third_period_ui_energy_digest().to_owned(), third_period_maximum_energy_ui_offset: diagnostic.third_period_maximum_energy_ui_offset() })
    })();
...
fn run_json(fact: &RunFact) -> String {
    ...
    format!("{{\"source_manifest_sha256\":\"{}\",\"record_count\":{},\"reference_rx_payload_sha256\":\"{}\",\"candidate_prefix_sha256\":\"{}\",\"periods\":[{}],\"third_period_ui_energy_digest\":\"{}\",\"third_period_maximum_energy_ui_offset\":{}}}", fact.source_manifest_sha256, fact.record_count, fact.reference_rx_payload_sha256, fact.candidate_prefix_sha256, periods, fact.third_period_ui_energy_digest, fact.third_period_maximum_energy_ui_offset)
}
...
fn p3c_external_ads_selected_highloss_residual_diagnostic_runner_v1() {
    ...
    assert_eq!(first.third_period_ui_energy_digest, second.third_period_ui_energy_digest);
    assert_eq!(first.third_period_maximum_energy_ui_offset, second.third_period_maximum_energy_ui_offset);
    let payload = format!("{{\"schema\":\"{REPORT_SCHEMA}\",\"status\":\"observed\",\"source_byte_length\":{},\"source_sha256\":\"{}\",\"ads_canonical_triple_payload_sha256\":\"{ADS_CANONICAL_SHA256}\",\"contract_sha256\":\"{SELECTED_HIGHLOSS_PRBS9_WAVEFORM_ONLY_CONTRACT_SHA256_V3}\",\"source_reference_identity_checks\":\"before_stage_after_equal\",\"fresh_runs\":[{},{}],\"cleanup_status\":\"complete\"}}\n", SELECTED_P3C_S4P_BYTE_LENGTH_V1, SELECTED_P3C_S4P_SHA256_V1, run_json(&first), run_json(&second));
...
fn p3c_external_ads_selected_highloss_residual_diagnostic_runner_v2() {
    ...
    assert_eq!(first.third_period_ui_energy_digest, second.third_period_ui_energy_digest);
    assert_eq!(first.third_period_maximum_energy_ui_offset, second.third_period_maximum_energy_ui_offset);
    let payload = format!("{{\"schema\":\"{REPORT_SCHEMA_V2}\",\"status\":\"observed\",\"source_byte_length\":{},\"source_sha256\":\"{}\",\"ads_canonical_triple_payload_sha256\":\"{ADS_CANONICAL_SHA256}\",\"contract_sha256\":\"{SELECTED_HIGHLOSS_PRBS9_WAVEFORM_ONLY_CONTRACT_SHA256_V3}\",\"source_reference_identity_checks\":\"before_stage_after_equal\",\"fresh_runs\":[{},{}],\"cleanup_status\":\"complete\"}}\n", SELECTED_P3C_S4P_BYTE_LENGTH_V1, SELECTED_P3C_S4P_SHA256_V1, run_json(&first), run_json(&second));
    serde_json::from_str::<Value>(&payload).expect("report_json");
    fs::create_dir_all(report.parent().unwrap()).unwrap();
    fs::write(report, payload).unwrap();
}
