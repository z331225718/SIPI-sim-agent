...
//! External-only observation of the selected sealed S4P through fixed IEEE BSD
//! interpolation and raw-periodic inverse transform. It creates no waveform.
...
const REPORT_ENV: &str = "SIPI_P3C_SEALED_S4P_RAW_PERIODIC_RUNNER_REPORT";
const SCHEMA: &str = "sipi.p3c.sealed-selected-s4p-raw-periodic-runner.v1";
...
enum Outcome {
    Admitted {
        ...
        endpoint_imaginary_residue_bits: String,
        inverse_imaginary_residue_bits: String,
        endpoint_imaginary_residue_bound_bits: String,
        inverse_imaginary_residue_bound_bits: String,
...
fn fresh_root(index: usize) -> Result<PathBuf, String> {
    let nonce = SystemTime::now()
        ...
        .as_nanos();
    let root = env::temp_dir().join(format!("sipi-p3c-raw-periodic-{index}-{nonce}"));
...
fn transform_error_name(error: RawPeriodicTransformErrorV1) -> &'static str {
    match error {
        RawPeriodicTransformErrorV1::TooFewOneSidedBins => "too_few_one_sided_bins",
        RawPeriodicTransformErrorV1::OutputLengthOverflow => "output_length_overflow",
        RawPeriodicTransformErrorV1::OutputSampleLimitExceeded => "output_sample_limit_exceeded",
        RawPeriodicTransformErrorV1::EndpointImaginaryResidue => "endpoint_imaginary_residue",
        RawPeriodicTransformErrorV1::InverseImaginaryResidue => "inverse_imaginary_residue",
        RawPeriodicTransformErrorV1::NonFiniteCalculation => "nonfinite_calculation",
    }
}
...
fn outcome(admitted: &sipi_p3c::AdmittedSelectedP3cStaticTransferV2) -> Outcome {
    ...
    let endpoint_bound = 1.0e-12 + 1.0e-10 * uniform_scale;
    let raw_scale = raw
        .samples()
        .iter()
        .map(|value| value.get().abs())
        .fold(0.0_f64, f64::max);
    let inverse_bound = 1.0e-12 + 1.0e-10 * raw_scale;
    let mut uniform_digest = Sha256::new();
    uniform_digest.update(b"sipi.p3c.selected-uniform-spectrum.v1\0");
    uniform_digest.update((uniform.sample_count() as u64).to_be_bytes());
    uniform_digest.update(uniform.frequency_step().get().to_bits().to_be_bytes());
    for value in uniform.values() {
        uniform_digest.update(value.real().to_bits().to_be_bytes());
        uniform_digest.update(value.imaginary().to_bits().to_be_bytes());
    }
    let mut raw_digest = Sha256::new();
    raw_digest.update(b"sipi.p3c.selected-raw-periodic-response.v1\0");
    raw_digest.update((raw.sample_count() as u64).to_be_bytes());
    raw_digest.update(raw.sample_interval().get().to_bits().to_be_bytes());
    for value in raw.samples() {
        raw_digest.update(value.get().to_bits().to_be_bytes());
    }
    Outcome::Admitted {
        uniform_bin_count: uniform.sample_count(),
        raw_sample_count: raw.sample_count(),
        frequency_step_bits: format!("{:016x}", uniform.frequency_step().get().to_bits()),
        sample_interval_bits: format!("{:016x}", raw.sample_interval().get().to_bits()),
        endpoint_imaginary_residue_bits: format!(
            "{:016x}",
            raw.endpoint_imaginary_residue().get().to_bits()
        ),
        inverse_imaginary_residue_bits: format!(
            "{:016x}",
            raw.inverse_imaginary_residue().get().to_bits()
        ),
        endpoint_imaginary_residue_bound_bits: format!("{:016x}", endpoint_bound.to_bits()),
        inverse_imaginary_residue_bound_bits: format!("{:016x}", inverse_bound.to_bits()),
        uniform_spectrum_sha256: format!("{:x}", uniform_digest.finalize()),
        raw_response_sha256: format!("{:x}", raw_digest.finalize()),
    }
}
...
fn observe_once(source: &Path, index: usize) -> Result<RunFact, String> {
    ...
    let result = (|| {
        ...
        let artifact_id = format!("selected-s4p-raw-periodic-{index}");
        let mut stage = store
            .begin(&artifact_id)
...
#[test]
#[ignore = "external-only selected S4P raw-periodic observation; requires explicit source and report paths"]
fn p3c_sealed_s4p_external_raw_periodic_runner_v1() {
    run().unwrap_or_else(|error| {
        panic!("p3c_sealed_s4p_external_raw_periodic_runner_v1_failed:{error}")
    });
}
