...
use sipi_compare::prbs9_waveform_v2::{
...
const REFERENCE_CANONICAL_SHA: &str =
...
fn identity(path: &Path) -> Result<(u64, String), String> {
    ...
    loop {
        let count = file.read(&mut buf).map_err(|_| "source_read".to_owned())?;
        if count == 0 {
            break;
        }
        length += count as u64;
...
fn reference_values(path: &Path) -> Result<(Vec<f64>, String), String> {
    ...
    if bytes.len() != COUNT * 24 || digest(&bytes) != REFERENCE_CANONICAL_SHA {
    ...
    for (index, row) in rows.by_ref().enumerate() {
        ...
        if !time.is_finite()
            || !tx.is_finite()
            || !rx.is_finite()
            || (time - expected_time).abs() > grid_tolerance
        {
    ...
    if !rows.remainder().is_empty() {
        return Err("reference_layout".to_owned());
    }
    let payload = values
        .iter()
...
fn publish(root: &Path, id: &str, values: &[f64]) -> Result<(String, String), String> {
    if values.len() != COUNT || values.iter().any(|value| !value.is_finite()) {
        return Err("artifact_values".to_owned());
    }
    let payload = values
        .iter()
        .flat_map(|value| value.to_le_bytes())
    ...
    let metadata = format!(
        "{{\"schema\":\"sipi.compare.prbs9-waveform-artifact.v1\",\"contract_sha256\":\"{CONTRACT}\",\"quantity\":\"differential_voltage\",\"unit\":\"volts_differential\",\"timebase_profile\":\"prbs9-v2-32gtps-osr32-three-period-half-open\",\"sample_count\":49056,\"encoding\":\"ieee754-binary64-little-endian\",\"payload\":{{\"byte_length\":392448,\"sha256\":\"{payload_sha}\"}}}}"
    );
    ...
    stage
        ...
        .map_err(|_| "artifact_metadata".to_owned())?;
    stage
        .stage_reader("waveform.f64le", payload.as_slice(), 392_448)
        .map_err(|_| "artifact_payload".to_owned())?;
    stage
        .seal()
    ...
    Ok((
        digest(
            &fs::read(root.join(id).join("success.json"))
...
fn candidate(source: &Path, root: &Path, index: usize) -> Result<Vec<f64>, String> {
    ...
    let id = format!("s4p-{index}");
    let mut stage = store.begin(&id).map_err(|_| "source_begin".to_owned())?;
    stage
        .stage_reader(
    ...
    let manifest = digest(
        &fs::read(root.join(&id).join("success.json")).map_err(|_| "source_manifest".to_owned())?,
    );
    let reader = ArtifactRoot::open_existing(root).map_err(|_| "source_reopen_root".to_owned())?;
    let admitted = admit_selected_p3c_sealed_s4p_v2(
        &reader,
        &SelectedP3cSealedS4pIdentityV2::try_new(&id, &manifest)
            .map_err(|_| "source_identity_request".to_owned())?,
    )
    ...
    Ok(generate_selected_p3c_prbs9_impulse_candidate_v1(&truncated)
...
fn metric_reason(reference: Vec<f64>, candidate: Vec<f64>) -> &'static str {
    ...
    match compare_prbs9_metrics_v2(&pair) {
        ...
        Err(Prbs9WaveformMetricErrorV2::ZeroReferenceEyeMetric { metric: "height" }) => {
            "zero_reference_eye_height"
        }
        Err(Prbs9WaveformMetricErrorV2::ZeroReferenceEyeMetric { metric: "width" }) => {
            "zero_reference_eye_width"
        }
        Err(Prbs9WaveformMetricErrorV2::ZeroReferenceEyeMetric { .. }) => "numeric_rejection",
        Err(Prbs9WaveformMetricErrorV2::ZeroPlateau {
            waveform: "reference",
            ..
        }) => "zero_plateau_reference",
        Err(Prbs9WaveformMetricErrorV2::ZeroPlateau {
            waveform: "candidate",
            ..
        }) => "zero_plateau_candidate",
        Err(Prbs9WaveformMetricErrorV2::ZeroPlateau { .. }) => "numeric_rejection",
        Err(Prbs9WaveformMetricErrorV2::CrossingCount {
            waveform: "reference",
            actual: 0,
            ..
        }) => "crossing_missing_reference",
        Err(Prbs9WaveformMetricErrorV2::CrossingCount {
            waveform: "candidate",
            actual: 0,
            ..
        }) => "crossing_missing_candidate",
        Err(Prbs9WaveformMetricErrorV2::CrossingCount {
            waveform: "reference",
            ..
        }) => "crossing_multiple_reference",
        Err(Prbs9WaveformMetricErrorV2::CrossingCount {
            waveform: "candidate",
            ..
        }) => "crossing_multiple_candidate",
        Err(
            Prbs9WaveformMetricErrorV2::CrossingCount { .. }
            | Prbs9WaveformMetricErrorV2::NumericOverflow { .. }
            | Prbs9WaveformMetricErrorV2::LengthMismatch { .. }
            | Prbs9WaveformMetricErrorV2::NonFiniteValue { .. },
        ) => "numeric_rejection",
    }
}
...
fn run_once(
    ...
    let s4p_root = fresh("sipi-p3c-metric-s4p", index)?;
    let metric_root = fresh("sipi-p3c-metric-artifacts", index)?;
    let result = (|| {
        ...
        let (reference_manifest, _) = publish(
            &metric_root,
            &format!("reference-{index}"),
            &reference_values,
        )?;
        let (candidate_manifest, candidate_payload_sha) = publish(
            &metric_root,
            &format!("candidate-{index}"),
            &candidate_values,
        )?;
        let request = format!(
            "{{\"schema\":\"sipi.compare.prbs9-metric-artifacts-request.v1\",\"contract_sha256\":\"{CONTRACT}\",\"reference\":{{\"artifact_id\":\"reference-{index}\",\"manifest_sha256\":\"{reference_manifest}\"}},\"candidate\":{{\"artifact_id\":\"candidate-{index}\",\"manifest_sha256\":\"{candidate_manifest}\"}}}}"
        );
        let mut child = Command::new(cli)
            .args([
                ...
                "--stdin",
                "--artifact-root",
                metric_root.to_string_lossy().as_ref(),
            ])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
        ...
        if exit_code == 3 {
            ...
            if !stdout.contains("\"command\":\"compare\"")
                || !stdout.contains("\"status\":\"invalid\"")
                || !stdout.contains("\"result\":null")
                || !stderr.contains("\"code\":\"contract_rejected\"")
                || !stderr.contains("\"stage\":\"schema\"")
                || !stderr.contains("\"rule_id\":\"contract.cross-field.v1\"")
            {
                return Err("cli_rejection_envelope".to_owned());
            }
            let token = if direct_core_reason == "core_ok" {
                "core_ok_cli_rejected"
            } else {
                direct_core_reason
            };
            return Ok((
                false,
                format!(
                    "{{\"status\":\"rejected\",\"stage\":\"cli\",\"exit_code\":{exit_code},\"direct_core_reason\":\"{token}\",\"cli_stdout_byte_length\":{},\"cli_stdout_sha256\":\"{}\",\"cli_stderr_byte_length\":{},\"cli_stderr_sha256\":\"{}\"}}",
                    stdout.len(),
                    digest(stdout.as_bytes()),
                    stderr.len(),
                    digest(stderr.as_bytes())
                ),
            ));
        }
        if !output.status.success() || !output.stderr.is_empty() {
            return Err("cli_failed".to_owned());
        }
        let response = String::from_utf8(output.stdout).map_err(|_| "cli_utf8".to_owned())?;
        Ok((
            true,
            format!(
                "{{\"status\":\"admitted\",\"reference_manifest_sha256\":\"{reference_manifest}\",\"candidate_manifest_sha256\":\"{candidate_manifest}\",\"reference_rx_payload_sha256\":\"{reference_payload_sha}\",\"candidate_payload_sha256\":\"{candidate_payload_sha}\",\"cli_result\":{}}}",
                response.trim()
            ),
        ))
    })();
    let left = fs::remove_dir_all(&s4p_root);
    let right = fs::remove_dir_all(&metric_root);
...
fn p3c_external_ads_reference_metric_runner_v1() {
    let source = env_path(SOURCE).unwrap();
    let reference = env_path(REFERENCE).unwrap();
    let cli = env_path(CLI).unwrap();
    let report = env_path(REPORT).unwrap();
    ...
    if !first_admitted {
        assert_eq!(first, second, "rejected_run_mismatch");
    }
    let outcome = if first_admitted {
        "admitted"
    } else {
        "rejected"
    };
