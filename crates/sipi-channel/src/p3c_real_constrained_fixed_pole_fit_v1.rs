...
impl fmt::Display for RealConstrainedFixedPoleFitErrorV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "selected P3C real-constrained fixed-pole fit is invalid: {self:?}"
        )
    }
}
...
impl SelectedP3cRealConstrainedFixedPoleFitV1 {
    ...
    pub fn positive_imaginary_poles_per_second(&self) -> &[Complex64] {
        &self.positive_imaginary_poles_per_second
    }
    pub fn positive_imaginary_residues_per_second(&self) -> &[Complex64] {
        &self.positive_imaginary_residues_per_second
    }
    pub fn metrics(&self) -> RealConstrainedFixedPoleFitMetricsV1 {
        self.metrics
    }
}
...
fn pair_basis(axis: c64, pole: c64) -> Result<(c64, c64), RealConstrainedFixedPoleFitErrorV1> {
    ...
    let imaginary_residue =
        c64::new(0.0, 1.0) * (c64::new(1.0, 0.0) / positive - c64::new(1.0, 0.0) / negative);
...
fn model_value(
    ...
    poles: &[c64],
    residues: &[c64],
) -> Result<c64, RealConstrainedFixedPoleFitErrorV1> {
    let value =
        poles
            .iter()
            .zip(residues)
            .try_fold(c64::new(0.0, 0.0), |sum, (pole, residue)| {
                let positive = axis - *pole;
                let negative = axis - pole.conj();
                if (positive.re == 0.0 && positive.im == 0.0)
                    || (negative.re == 0.0 && negative.im == 0.0)
                {
                    return Err(RealConstrainedFixedPoleFitErrorV1::NonFiniteModel);
                }
...
fn metrics(
    reference: &[c64],
    candidate: &[c64],
) -> Result<RealConstrainedFixedPoleFitMetricsV1, RealConstrainedFixedPoleFitErrorV1> {
    let peak = reference
        .iter()
        .copied()
        .map(magnitude)
    ...
    if !peak.is_finite() || peak == 0.0 {
        return Err(RealConstrainedFixedPoleFitErrorV1::ZeroSignalNorm);
    }
    let dc = magnitude(reference[0]);
    if dc <= ZERO_MAGNITUDE_FLOOR {
        return Err(RealConstrainedFixedPoleFitErrorV1::ZeroDcReference);
    }
    let (signal, error, maximum) = reference.iter().zip(candidate).try_fold(
...
fn fit_order(
    ...
    order: usize,
) -> Result<SelectedP3cRealConstrainedFixedPoleFitV1, RealConstrainedFixedPoleFitErrorV1> {
    let axis = input
        .frequencies()
        .iter()
    ...
    let mut system = Mat::<f64>::from_fn(rows, columns, |row, column| {
        ...
        let (for_real_residue, for_imaginary_residue) =
            pair_basis(axis[sample], poles[column / 2]).expect("finite admitted basis");
        let basis = if column % 2 == 0 {
            for_real_residue
        } else {
            for_imaginary_residue
        };
        if row < axis.len() { basis.re } else { basis.im }
    });
    let mut response = Mat::<f64>::from_fn(rows, 1, |row, _| {
        if row < axis.len() {
            reference[row].re
        } else {
            reference[row - axis.len()].im
        }
    });
    ...
    let singular_values = system
        .as_ref()
        .thin_svd()
        .map_err(|_| RealConstrainedFixedPoleFitErrorV1::SolverFailure)?;
    ...
    let residues = (0..poles.len())
        .map(|pair| c64::new(coefficients[(2 * pair, 0)], coefficients[(2 * pair + 1, 0)]))
        .collect::<Vec<_>>();
    ...
    let convert = |value: c64| {
        Complex64::try_new(value.re * OMEGA_SCALE, value.im * OMEGA_SCALE)
            .map_err(|_| RealConstrainedFixedPoleFitErrorV1::NonFiniteModel)
    };
    Ok(SelectedP3cRealConstrainedFixedPoleFitV1 {
        order,
        positive_imaginary_poles_per_second: poles
            .into_iter()
            .map(convert)
            .collect::<Result<_, _>>()?,
        positive_imaginary_residues_per_second: residues
            .into_iter()
            .map(convert)
            .collect::<Result<_, _>>()?,
        metrics,
...
pub fn fit_selected_p3c_real_constrained_fixed_pole_v1(
    input: &SelectedP3cStaticDifferentialTransferV1,
) -> Result<SelectedP3cRealConstrainedFixedPoleFitV1, RealConstrainedFixedPoleFitErrorV1> {
...
mod tests {
    ...
    fn transfer_from_real_pairs(
        ...
        let residues = poles
            .iter()
            .enumerate()
            .map(|(index, _)| c64::new(0.05 + index as f64 * 0.01, 0.02 + index as f64 * 0.005))
            .collect::<Vec<_>>();
        let frequencies = (0..=96)
            .map(|index| {
                Hertz::try_new(start_hz + (end_hz - start_hz) * index as f64 / 96.0).unwrap()
            })
            .collect::<Vec<_>>();
    ...
    fn real_constrained_basis_recovers_a_canonical_real_model() {
        let result = fit_selected_p3c_real_constrained_fixed_pole_v1(&transfer_from_real_pairs(
            8,
            0.0,
            MAX_FREQUENCY_HZ,
        ))
        ...
        assert!(
            result
                .positive_imaginary_poles_per_second()
                .iter()
                .all(|pole| pole.real() < 0.0 && pole.imaginary() > 0.0)
        );
    ...
    fn rejects_a_missing_dc_before_constructing_pair_coefficients() {
        ...
        assert_eq!(
            fit_selected_p3c_real_constrained_fixed_pole_v1(&transfer).unwrap_err(),
            RealConstrainedFixedPoleFitErrorV1::InvalidFrequencyRange
        );
    }
}
