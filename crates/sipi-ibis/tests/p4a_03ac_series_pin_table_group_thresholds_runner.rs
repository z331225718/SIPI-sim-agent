//! External-only typed series pin group threshold table cross-check runner (P4A-03ac).
//!
//! Lifts one typed [Series Pin Mapping] group threshold table record from input parameters
...
use sipi_ibis::{
    lift_series_pin_group_threshold_record_v1, SERIES_PIN_TABLE_GROUP_THRESHOLDS_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03ac_series_pin_table_group_thresholds_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let output = match lift_series_pin_group_threshold_record_v1(gn, vthresh, rseries, cseries, lseries) {
        Ok(rec) => {
            serde_json::json!({
                "policy": SERIES_PIN_TABLE_GROUP_THRESHOLDS_POLICY_V1,
                "valid": true,
