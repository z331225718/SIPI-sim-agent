...
use sipi_ibis::{
    SERIES_PIN_TABLE_GROUP_THRESHOLDS_POLICY_V1, lift_series_pin_group_threshold_record_v1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4a_03ac_series_pin_table_group_thresholds_runner --input <json> [--report <path>]"
        );
        return;
    };
    ...
    let output =
        match lift_series_pin_group_threshold_record_v1(gn, vthresh, rseries, cseries, lseries) {
            Ok(rec) => {
                serde_json::json!({
