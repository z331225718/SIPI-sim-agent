//! External-only typed series pin Model Selector group threshold table cross-check runner (P4A-03au).
//!
...
use sipi_ibis::{
    lift_series_pin_selector_group_threshold_record_v1, SERIES_PIN_TABLE_SELECTOR_THRESHOLDS_GROUP_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03au_series_pin_table_selector_thresholds_group_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let output = match lift_series_pin_selector_group_threshold_record_v1(pf, ps, ms, gn, vthresh, rseries, cseries, lseries) {
        Ok(rec) => {
            serde_json::json!({
                "policy": SERIES_PIN_TABLE_SELECTOR_THRESHOLDS_GROUP_POLICY_V1,
                "valid": true,
