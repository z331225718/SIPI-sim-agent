//! External-only typed series pin group Model Selector threshold table cross-check runner (P4A-03bd).
//!
//! Lifts one typed [Series Pin Mapping] group Model Selector threshold table record from input parameters
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03bd_series_pin_table_selector_group_thresholds_runner --input <json> [--report <path>]");
        return;
    };
