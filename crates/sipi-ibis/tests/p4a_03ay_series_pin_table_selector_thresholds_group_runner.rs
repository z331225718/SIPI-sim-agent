//! External-only typed series pin Model Selector group threshold table cross-check runner (P4A-03ay).
//!
//! Lifts one typed [Series Pin Mapping] Model Selector group threshold table record from input parameters
...
use sipi_ibis::{
    lift_series_pin_group_selector_threshold_binding_record_v1, SERIES_PIN_TABLE_SELECTOR_THRESHOLDS_GROUP_BINDING_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03ay_series_pin_table_selector_thresholds_group_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let gn = value.get("group_name").and_then(|v| v.as_str());
    ...
    let output = match lift_series_pin_group_selector_threshold_binding_record_v1(pf, ps, ms, gn, vthresh, rseries, cseries, lseries) {
        Ok(rec) => {
            serde_json::json!({
                "policy": SERIES_PIN_TABLE_SELECTOR_THRESHOLDS_GROUP_BINDING_POLICY_V1,
                "valid": true,
