...
use sipi_ibis::{
    SERIES_PIN_TABLE_SELECTOR_BINDING_POLICY_V1, lift_series_pin_selector_binding_record_v1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4a_03aq_series_pin_table_selector_runner --input <json> [--report <path>]"
        );
        return;
    };
