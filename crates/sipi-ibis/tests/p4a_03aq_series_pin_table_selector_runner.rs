//! External-only typed series pin Model Selector binding cross-check runner (P4A-03aq).
//!
...
use sipi_ibis::{
    lift_series_pin_selector_binding_record_v1, SERIES_PIN_TABLE_SELECTOR_BINDING_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03aq_series_pin_table_selector_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let output = match lift_series_pin_selector_binding_record_v1(pf, ps, ms, ftg) {
        Ok(rec) => {
            serde_json::json!({
                "policy": SERIES_PIN_TABLE_SELECTOR_BINDING_POLICY_V1,
                "valid": true,
