...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03at_series_pin_thresholds_group_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let output = match lift_series_pin_table_group_thresholds_v1(gn, vthresh, rseries, cseries, lseries) {
        Ok(rec) => {
            serde_json::json!({
