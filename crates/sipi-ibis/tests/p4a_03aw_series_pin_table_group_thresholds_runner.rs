...
fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4a_03aw_series_pin_table_group_thresholds_runner --input <json> [--report <path>]"
        );
        return;
    };
    ...
    let output = match lift_series_pin_group_model_threshold_record_v1(
        pf, ps, mn, gn, vthresh, rseries, cseries, lseries,
    ) {
        Ok(rec) => {
            serde_json::json!({
