//! External-only typed series pin group model threshold table cross-check runner (P4A-03aw).
//!
//! Lifts one typed [Series Pin Mapping] group model threshold table record from input parameters
...
use sipi_ibis::{
    lift_series_pin_group_model_threshold_record_v1, SERIES_PIN_TABLE_GROUP_MODEL_THRESHOLDS_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03aw_series_pin_table_group_thresholds_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let output = match lift_series_pin_group_model_threshold_record_v1(pf, ps, mn, gn, vthresh, rseries, cseries, lseries) {
        Ok(rec) => {
            serde_json::json!({
                "policy": SERIES_PIN_TABLE_GROUP_MODEL_THRESHOLDS_POLICY_V1,
                "valid": true,
