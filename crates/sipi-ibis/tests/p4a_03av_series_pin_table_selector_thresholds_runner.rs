//! External-only typed series pin table selector threshold parameters cross-check runner (P4A-03av).
//!
...
use sipi_ibis::{
    lift_series_pin_selector_threshold_record_v1, SERIES_PIN_TABLE_SELECTOR_THRESHOLDS_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03av_series_pin_table_selector_thresholds_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let output = match lift_series_pin_selector_threshold_record_v1(pf, ps, ms, vthresh, rseries, cseries, lseries) {
        Ok(rec) => {
            serde_json::json!({
                "policy": SERIES_PIN_TABLE_SELECTOR_THRESHOLDS_POLICY_V1,
                "valid": true,
