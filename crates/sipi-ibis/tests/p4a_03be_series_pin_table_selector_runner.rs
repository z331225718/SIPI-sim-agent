//! External-only typed series pin Model Selector binding cross-check runner (P4A-03be).
//!
...
use sipi_ibis::{
    lift_series_pin_selector_binding_record_be_v1, SERIES_PIN_TABLE_SELECTOR_BINDING_BE_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03be_series_pin_table_selector_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let output = match lift_series_pin_selector_binding_record_be_v1(pf, ps, ms, ftg) {
        Ok(rec) => {
            serde_json::json!({
                "policy": SERIES_PIN_TABLE_SELECTOR_BINDING_BE_POLICY_V1,
                "valid": true,
