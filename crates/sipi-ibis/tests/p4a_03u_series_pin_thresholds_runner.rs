//! External-only typed series pin thresholds cross-check runner (P4A-03u).
//!
//! Lifts one typed [Series Pin Mapping] threshold declaration from input parameters
...
use sipi_ibis::{SERIES_PIN_THRESHOLDS_POLICY_V1, lift_series_pin_thresholds_v1};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4a_03u_series_pin_thresholds_runner --input <json> [--report <path>]");
        return;
    };
    ...
    let output = match lift_series_pin_thresholds_v1(vthresh, rseries, cseries, lseries) {
        Ok(thresh) => {
            serde_json::json!({
                "policy": SERIES_PIN_THRESHOLDS_POLICY_V1,
                "valid": true,
