...
use sipi_ibis::{
    SERIES_PIN_TABLE_SELECTOR_THRESHOLDS_BINDING_POLICY_V1,
    lift_series_pin_selector_threshold_binding_record_v1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4a_03bg_series_pin_table_selector_thresholds_runner --input <json> [--report <path>]"
        );
        return;
    };
    ...
    let output = match lift_series_pin_selector_threshold_binding_record_v1(
        pf, ps, ms, vthresh, rseries, cseries, lseries,
    ) {
        Ok(rec) => {
            serde_json::json!({
