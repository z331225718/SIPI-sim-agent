//! Typed IBIS [Series Pin Mapping] group threshold table binding core (P4A-03ac).
//!
//! Lifts and validates IBIS [Series Pin Mapping] group threshold table binding records
...
pub const SERIES_PIN_TABLE_GROUP_THRESHOLDS_POLICY_V1: &str =
    "sipi.p4a-03ac.series-pin-table-group-thresholds-v1.typed-group-thresholds";

/// Fail-closed errors during series pin table group thresholds lifting.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum SeriesPinTableGroupThresholdsErrorV1 {
    EmptyGroupName,
    NonAsciiName,
    InvalidName,
    NonFiniteValue,
...
/// A typed IBIS [Series Pin Mapping] group threshold table record.
...
impl TypedSeriesPinGroupThresholdRecordV1 {
    pub fn try_new(
    ...
    ) -> Result<Self, SeriesPinTableGroupThresholdsErrorV1> {
        let gn = group_name.into().trim().to_string();

        if gn.is_empty() {
            return Err(SeriesPinTableGroupThresholdsErrorV1::EmptyGroupName);
        }
        if !gn.is_ascii() {
            return Err(SeriesPinTableGroupThresholdsErrorV1::NonAsciiName);
        }
        if !gn.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-' || c == '.') {
            return Err(SeriesPinTableGroupThresholdsErrorV1::InvalidName);
        }

        let validate_non_negative = |val: Option<f64>, kind: &'static str| -> Result<Option<FiniteF64>, SeriesPinTableGroupThresholdsErrorV1> {
            match val {
                None => Ok(None),
                Some(v) => {
                    if !v.is_finite() {
                        return Err(SeriesPinTableGroupThresholdsErrorV1::NonFiniteValue);
                    }
                    if v < 0.0 {
                        return Err(SeriesPinTableGroupThresholdsErrorV1::NegativeThresholdParameter);
                    }
                    let finite = FiniteF64::try_new(v, kind)
                        .map_err(|_| SeriesPinTableGroupThresholdsErrorV1::NonFiniteValue)?;
                    Ok(Some(finite))
                }
            }
        };

        let validate_finite = |val: Option<f64>, kind: &'static str| -> Result<Option<FiniteF64>, SeriesPinTableGroupThresholdsErrorV1> {
            match val {
                None => Ok(None),
                Some(v) => {
                    if !v.is_finite() {
...
pub fn lift_series_pin_group_threshold_record_v1(
    ...
    TypedSeriesPinGroupThresholdRecordV1::try_new(
        group_name,
        vthreshold_v,
        rseries_ohm,
        cseries_farad,
...
mod tests {
    ...
    fn policy_fixed() {
        assert_eq!(
            SERIES_PIN_TABLE_GROUP_THRESHOLDS_POLICY_V1,
            "sipi.p4a-03ac.series-pin-table-group-thresholds-v1.typed-group-thresholds"
        );
    }
    ...
    fn valid_full_group_threshold_record() {
        let rec = lift_series_pin_group_threshold_record_v1(
            "SERIES_GRP1",
            Some(1.2),
            Some(50.0),
            Some(1e-12),
    ...
    fn rejects_empty_group_name() {
        assert_eq!(
            lift_series_pin_group_threshold_record_v1("", None, None, None, None),
            Err(SeriesPinTableGroupThresholdsErrorV1::EmptyGroupName)
        );
    }
    ...
    fn rejects_negative_rseries() {
        assert_eq!(
            lift_series_pin_group_threshold_record_v1("SERIES_GRP1", None, Some(-50.0), None, None),
            Err(SeriesPinTableGroupThresholdsErrorV1::NegativeThresholdParameter)
        );
    }
}
