...
The following upstream paths are copied byte-for-byte to the same basename in
`src/`: `analysis.rs`, `bathtub.rs`, `capabilities.rs`, `channel.rs`,
`decoder.rs`, `equalization.rs`, `error.rs`, `event.rs`, `input.rs`,
`jitter.rs`, `output.rs`, `pattern.rs`, `pipeline.rs`, `receiver.rs`,
`response.rs`, `signal.rs`, `simulation.rs`, `statistical_eye.rs`, and
`units.rs`.
