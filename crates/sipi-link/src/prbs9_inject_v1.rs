...
use super::prbs9_v1::{PRBS9_OWNER_SEED_BITS, Prbs9V1};
...
pub fn prbs9_inject_waveform_v1(
    ...
    if samples_per_ui == 0 {
        return Err(InjectWaveformErrorV1::ZeroSamplesPerUi);
    }
    if bit_count == 0 {
        return Err(InjectWaveformErrorV1::ZeroBitCount);
    }
    if !amplitude_v.is_finite() {
        return Err(InjectWaveformErrorV1::NonFiniteAmplitude);
    }
    ...
    for _ in 0..bit_count {
        ...
        waveform.extend(std::iter::repeat_n(value, samples_per_ui));
    }
    Ok(waveform)
}
...
mod tests {
    ...
    fn policy_fixed() {
        assert_eq!(
            ...
            "sipi.p3b-05d.prbs9-inject.v1.tx-waveform"
        );
    }
    ...
    fn rejects_zero_spu() {
        assert_eq!(
            prbs9_inject_waveform_v1(1, 4, 0, 1.0).err(),
            Some(InjectWaveformErrorV1::ZeroSamplesPerUi)
        );
    }
    ...
    fn rejects_zero_bits() {
        assert_eq!(
            prbs9_inject_waveform_v1(1, 0, 1, 1.0).err(),
            Some(InjectWaveformErrorV1::ZeroBitCount)
        );
    }
}
