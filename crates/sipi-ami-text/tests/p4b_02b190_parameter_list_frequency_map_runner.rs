//! External-only parameter list frequency map cross-check runner (P4B-02b190).
//!
//! Reads one JSON case {name, type, value}, builds the validated value, computes the
//! frequency map and reports it (or the fail-closed error) for hash-only comparison
...
use sipi_ami_text::{
    parameter_list_frequency_map_v1, AmiParameterValueV1, ParameterListFrequencyMapErrorV1,
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4b_02b190_parameter_list_frequency_map_runner --input <path> [--report <path>]");
        return;
    };
    ...
    let parameter = match AmiParameterValueV1::try_new(name, type_token, value_token) {
        ...
        Err(_) => {
            let output = serde_json::json!({
                "policy": PARAMETER_LIST_FREQUENCY_MAP_POLICY_V1,
                "valid": false,
    ...
    let output = match parameter_list_frequency_map_v1(&parameter) {
        Ok(map) => {
            ...
            serde_json::json!({
                "policy": PARAMETER_LIST_FREQUENCY_MAP_POLICY_V1,
                "valid": true,
                "frequency_map": map_json,
            })
        }
        Err(ParameterListFrequencyMapErrorV1::NotAList) => serde_json::json!({
            "policy": PARAMETER_LIST_FREQUENCY_MAP_POLICY_V1,
            "valid": false,
            "error": "NotAList",
        }),
        Err(ParameterListFrequencyMapErrorV1::MalformedList) => serde_json::json!({
            "policy": PARAMETER_LIST_FREQUENCY_MAP_POLICY_V1,
            "valid": false,
            "error": "MalformedList",
        }),
