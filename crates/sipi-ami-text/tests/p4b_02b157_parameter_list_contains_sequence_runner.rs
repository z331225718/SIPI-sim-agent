//! External-only parameter list sequence containment cross-check runner (P4B-02b157).
//!
//! Reads one JSON case {name, type, value, query_name, query_type, query_value}, builds both
//! validated values, checks whether the host's trimmed items contain the query's trimmed items
//! as a contiguous window and reports the boolean (or the fail-closed error) for hash-only
...
pub(crate) fn main() {
    ...
    let host = match AmiParameterValueV1::try_new(name, type_token, value_token) {
        ...
        Err(_) => {
            let output = serde_json::json!({
                "policy": PARAMETER_LIST_CONTAINS_SEQUENCE_POLICY_V1,
                "valid": false,
    ...
    let output = match list_contains_sequence_v1(&host, &query) {
        Ok(contains) => serde_json::json!({
            "policy": PARAMETER_LIST_CONTAINS_SEQUENCE_POLICY_V1,
            "valid": true,
            "contains": contains,
        }),
        Err(ParameterListContainsSequenceErrorV1::NotAList) => serde_json::json!({
            "policy": PARAMETER_LIST_CONTAINS_SEQUENCE_POLICY_V1,
            "valid": false,
            "error": "NotAList",
        }),
        Err(ParameterListContainsSequenceErrorV1::MalformedList) => serde_json::json!({
            "policy": PARAMETER_LIST_CONTAINS_SEQUENCE_POLICY_V1,
            "valid": false,
            "error": "MalformedList",
        }),
