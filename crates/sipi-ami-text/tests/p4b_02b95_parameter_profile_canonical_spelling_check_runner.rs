//! External-only parameter profile canonical spelling check cross-check runner (P4B-02b95).
//!
//! Reads one JSON case {profile: {name: {type, value}}}, builds the validated
//! profile, checks the canonical spelling of every entry's value, and reports
//! {entries, canonical, non_canonical} for hash-only comparison against an
...
use sipi_ami_text::{
    check_parameter_profile_spellings_canonical_v1, AmiParameterValueV1,
    PARAMETER_PROFILE_CANONICAL_SPELLING_CHECK_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4b_02b95_parameter_profile_canonical_spelling_check_runner --input <path> [--report <path>]");
        return;
    };
    ...
    let profile = match value.get("profile").and_then(build_profile) {
        ...
        None => {
            let output = serde_json::json!({
                "policy": PARAMETER_PROFILE_CANONICAL_SPELLING_CHECK_POLICY_V1,
                "valid": false,
    ...
    let result = check_parameter_profile_spellings_canonical_v1(&profile);
