//! External-only parameter list last-occurrence map cross-check runner (P4B-02b187).
//!
//! Reads one JSON case {name, type, value}, builds the validated value, computes the
//! last-occurrence map and reports it (or the fail-closed error) for hash-only comparison
...
pub(crate) fn main() {
    ...
    let parameter = match AmiParameterValueV1::try_new(name, type_token, value_token) {
        ...
        Err(_) => {
            let output = serde_json::json!({
                "policy": PARAMETER_LIST_LAST_OCCURRENCE_MAP_POLICY_V1,
                "valid": false,
    ...
    let output = match parameter_list_last_occurrence_map_v1(&parameter) {
        Ok(map) => {
            ...
            serde_json::json!({
                "policy": PARAMETER_LIST_LAST_OCCURRENCE_MAP_POLICY_V1,
                "valid": true,
                "last_occurrence_map": map_json,
            })
        }
        Err(ParameterListLastOccurrenceMapErrorV1::NotAList) => serde_json::json!({
            "policy": PARAMETER_LIST_LAST_OCCURRENCE_MAP_POLICY_V1,
            "valid": false,
            "error": "NotAList",
        }),
        Err(ParameterListLastOccurrenceMapErrorV1::MalformedList) => serde_json::json!({
            "policy": PARAMETER_LIST_LAST_OCCURRENCE_MAP_POLICY_V1,
            "valid": false,
            "error": "MalformedList",
        }),
