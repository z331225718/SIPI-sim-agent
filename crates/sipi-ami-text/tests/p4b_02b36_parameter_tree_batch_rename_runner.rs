//! External-only parameter tree batch rename cross-check runner (P4B-02b36).
//!
//! Parses raw parenthesized AMI text, builds typed parameter tree hierarchies,
//! renames leaves per a caller-supplied old-name -> new-name map, and reports
//! the renamed tree for hash-only comparison against an independent reference.
...
use sipi_ami_text::{
    build_parameter_trees_v1, parse_ami_text_v1, rename_parameter_tree_leaves_v1,
    AmiParameterTreeNodeV1, ParseLimitsV1, PARAMETER_TREE_BATCH_RENAME_POLICY_V1,
};
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4b_02b36_parameter_tree_batch_rename_runner --input <path> [--report <path>]");
        return;
    };
    ...
    if let Some(renames_json) = value.get("renames").and_then(|v| v.as_object()) {
        for (old_name, new_name) in renames_json {
    ...
    let doc = match parse_ami_text_v1(text.as_bytes(), limits) {
        ...
        Err(e) => {
            let output = serde_json::json!({
                "policy": PARAMETER_TREE_BATCH_RENAME_POLICY_V1,
                "valid": false,
    ...
    let output = match rename_parameter_tree_leaves_v1(&trees[0], &renames) {
        Ok(renamed) => {
            serde_json::json!({
                "policy": PARAMETER_TREE_BATCH_RENAME_POLICY_V1,
                "valid": true,
