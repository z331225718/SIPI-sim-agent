//! External-only typed parameter tree leaf value type inference cross-check runner (P4B-02b33).
//!
//! Parses raw parenthesized AMI text, builds typed parameter tree hierarchies,
//! infers one AmiParameterTypeV1 per leaf from its value tokens, and reports the
...
fn main() {
    ...
    let output = match infer_parameter_tree_leaf_types_v1(&trees[0]) {
        Ok(inference) => {
            ...
            serde_json::json!({
                "policy": PARAMETER_TREE_VALUE_TYPE_INFERENCE_POLICY_V1,
                "valid": true,
                "leaves_inferred": inference.leaves_inferred(),
