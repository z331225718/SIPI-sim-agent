//! External-only parameter profile type statistics cross-check runner (P4B-02b73).
//!
//! Reads one JSON case {profile: {name: {type, value}}}, builds the validated
//! profile, and reports per-type counts for hash-only comparison against an
...
use sipi_ami_text::{
    compute_parameter_profile_type_stats_v1, AmiParameterValueV1,
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4b_02b73_parameter_profile_type_stats_runner --input <path> [--report <path>]");
        return;
    };
    ...
    let profile = match value.get("profile").and_then(build_profile) {
        ...
        None => {
            let output = serde_json::json!({
                "policy": PARAMETER_PROFILE_TYPE_STATS_POLICY_V1,
                "valid": false,
    ...
    let stats = compute_parameter_profile_type_stats_v1(&profile);
    let output = serde_json::json!({
        "policy": PARAMETER_PROFILE_TYPE_STATS_POLICY_V1,
        "valid": true,
        "total": stats.total(),
        "float_count": stats.float_count(),
        "integer_count": stats.integer_count(),
