//! External-only parameter list normalized frequency cross-check runner (P4B-02b181).
//!
//! Reads one JSON case {name, type, value}, builds the validated value, computes the
//! normalized per-item frequency map (count / total) of the trimmed-item distribution and
//! reports it (or the fail-closed error) for hash-only comparison against an independent
//! reference. Each proportion is formatted with 12 decimals. Ignored by default;
...
fn main() {
    ...
    let output = match parameter_list_frequency_normalized_v1(&parameter) {
        Ok(normalized) => {
        ...
        Err(ParameterListFrequencyNormalizedErrorV1::NotAList) => serde_json::json!({
            "policy": PARAMETER_LIST_FREQUENCY_NORMALIZED_POLICY_V1,
            "valid": false,
            "error": "NotAList",
        }),
        Err(ParameterListFrequencyNormalizedErrorV1::MalformedList) => serde_json::json!({
            "policy": PARAMETER_LIST_FREQUENCY_NORMALIZED_POLICY_V1,
            "valid": false,
            "error": "MalformedList",
        }),
