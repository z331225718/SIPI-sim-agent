//! External-only parameter list LCS length cross-check runner (P4B-02b142).
//!
//! Reads one JSON case {name, type, value, other_name, other_type, other_value}, builds both
//! validated values, computes the LCS length of their trimmed items and reports the length (or
...
fn main() {
    ...
    let output = match list_longest_common_subsequence_length_v1(&left, &right) {
        Ok(length) => serde_json::json!({
            "policy": PARAMETER_LIST_LONGEST_COMMON_SUBSEQUENCE_LENGTH_POLICY_V1,
            "valid": true,
            "length": length,
        }),
        Err(ParameterListLongestCommonSubsequenceLengthErrorV1::NotAList) => serde_json::json!({
            "policy": PARAMETER_LIST_LONGEST_COMMON_SUBSEQUENCE_LENGTH_POLICY_V1,
            "valid": false,
            "error": "NotAList",
        }),
