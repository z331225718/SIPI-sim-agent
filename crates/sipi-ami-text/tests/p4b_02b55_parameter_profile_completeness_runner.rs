...
use sipi_ami_text::{
    AmiParameterValueV1, PARAMETER_PROFILE_COMPLETENESS_POLICY_V1,
    check_parameter_profile_completeness_v1,
};
...
pub(crate) fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4b_02b55_parameter_profile_completeness_runner --input <path> [--report <path>]"
        );
        return;
    };
