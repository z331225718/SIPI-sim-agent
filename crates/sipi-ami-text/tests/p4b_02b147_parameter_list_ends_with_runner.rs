...
use sipi_ami_text::{
    AmiParameterValueV1, PARAMETER_LIST_ENDS_WITH_POLICY_V1, ParameterListEndsWithErrorV1,
    list_ends_with_v1,
};
...
pub(crate) fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4b_02b147_parameter_list_ends_with_runner --input <path> [--report <path>]"
        );
        return;
    };
    ...
    let suffix_name = value
        .get("suffix_name")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let suffix_type = value
        .get("suffix_type")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let suffix_value = value
        .get("suffix_value")
        .and_then(|v| v.as_str())
        .unwrap_or("");
