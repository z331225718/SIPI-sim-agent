//! External-only parameter list overlap coefficient cross-check runner (P4B-02b156).
//!
//! Reads one JSON case {name, type, value, other_name, other_type, other_value}, builds both
//! validated values, computes the overlap coefficient of their distinct trimmed item sets and
...
use sipi_ami_text::{
    AmiParameterValueV1, PARAMETER_LIST_OVERLAP_COEFFICIENT_POLICY_V1,
    ParameterListOverlapCoefficientErrorV1, list_overlap_coefficient_v1,
};
...
pub(crate) fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4b_02b156_parameter_list_overlap_coefficient_runner --input <path> [--report <path>]"
        );
        return;
    };
    ...
    let left = match AmiParameterValueV1::try_new(name, type_token, value_token) {
        ...
        Err(_) => {
            let output = serde_json::json!({
                "policy": PARAMETER_LIST_OVERLAP_COEFFICIENT_POLICY_V1,
                "valid": false,
    ...
    let output = match list_overlap_coefficient_v1(&left, &right) {
        Ok(index) => serde_json::json!({
            "policy": PARAMETER_LIST_OVERLAP_COEFFICIENT_POLICY_V1,
            "valid": true,
            "index": format!("{index:.12}"),
        }),
        Err(ParameterListOverlapCoefficientErrorV1::NotAList) => serde_json::json!({
            "policy": PARAMETER_LIST_OVERLAP_COEFFICIENT_POLICY_V1,
            "valid": false,
            "error": "NotAList",
        }),
        Err(ParameterListOverlapCoefficientErrorV1::MalformedList) => serde_json::json!({
            "policy": PARAMETER_LIST_OVERLAP_COEFFICIENT_POLICY_V1,
            "valid": false,
            "error": "MalformedList",
        }),
