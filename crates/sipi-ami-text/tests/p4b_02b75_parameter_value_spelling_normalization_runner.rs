//! External-only parameter value spelling normalization cross-check runner (P4B-02b75).
//!
//! Reads one JSON case {name, type, value}, builds the validated value, and
//! reports its canonical spelling for hash-only comparison against an
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4b_02b75_parameter_value_spelling_normalization_runner --input <path> [--report <path>]");
        return;
    };
    ...
    let parameter = match AmiParameterValueV1::try_new(name, type_token, value_token) {
        ...
        Err(_) => {
            let output = serde_json::json!({
                "policy": PARAMETER_VALUE_SPELLING_NORMALIZATION_POLICY_V1,
                "valid": false,
