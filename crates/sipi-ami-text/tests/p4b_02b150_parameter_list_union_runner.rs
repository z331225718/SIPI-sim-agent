//! External-only parameter list union cross-check runner (P4B-02b150).
//!
//! Reads one JSON case {name, type, value, other_name, other_type, other_value}, builds both
//! validated values, computes the multiset union of their trimmed items and reports the
...
fn main() {
    ...
    let Some(input) = input else {
        println!("usage: p4b_02b150_parameter_list_union_runner --input <path> [--report <path>]");
        return;
    };
    ...
    let left = match AmiParameterValueV1::try_new(name, type_token, value_token) {
        ...
        Err(_) => {
            let output = serde_json::json!({
                "policy": PARAMETER_LIST_UNION_POLICY_V1,
                "valid": false,
    ...
    let output = match list_union_v1(&left, &right) {
        Ok(token) => serde_json::json!({
            "policy": PARAMETER_LIST_UNION_POLICY_V1,
            "valid": true,
            "token": token,
        }),
        Err(ParameterListUnionErrorV1::NotAList) => serde_json::json!({
            "policy": PARAMETER_LIST_UNION_POLICY_V1,
            "valid": false,
            "error": "NotAList",
        }),
        Err(ParameterListUnionErrorV1::MalformedList) => serde_json::json!({
            "policy": PARAMETER_LIST_UNION_POLICY_V1,
            "valid": false,
            "error": "MalformedList",
        }),
