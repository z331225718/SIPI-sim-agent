//! External-only typed parameter tree diff filter cross-check runner (P4B-02b21).
//!
//! Parses two raw parenthesized AMI texts, computes diff entries between them,
//! filters diff entries matching a target exclude path, and reports the filtered diff list
    ...
    let output = match filter_parameter_tree_diffs_v1(&diffs, |path, _| path != exclude_path) {
        Ok(filtered) => {
            ...
            serde_json::json!({
                "policy": PARAMETER_TREE_DIFF_FILTER_POLICY_V1,
                "valid": true,
                "filtered_diff_count": filtered.len(),
                "filtered_diffs": diff_json_list,
            })
        }
        Err(e) => {
            serde_json::json!({
