...
use sipi_ami_text::{
    AmiParameterValueV1, PARAMETER_LIST_DISTINCT_COUNT_POLICY_V1,
    ParameterListDistinctCountErrorV1, count_distinct_parameter_list_items_v1,
};
...
pub(crate) fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4b_02b107_parameter_list_distinct_count_runner --input <path> [--report <path>]"
        );
        return;
    };
