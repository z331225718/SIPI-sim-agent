//! External-only parameter profile canonicalization cross-check runner (P4B-02b77).
//!
//! Reads one JSON case {profile: {name: {type, value}}}, builds the validated
//! profile, canonicalizes every value spelling, and reports {entries,
//! canonicalized, parameters} for hash-only comparison against an independent
...
use sipi_ami_text::{
    AmiParameterValueV1, PARAMETER_PROFILE_CANONICALIZATION_POLICY_V1,
...
pub(crate) fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4b_02b77_parameter_profile_canonicalization_runner --input <path> [--report <path>]"
        );
        return;
    };
    ...
    let profile = match value.get("profile").and_then(build_profile) {
        ...
        None => {
            let output = serde_json::json!({
                "policy": PARAMETER_PROFILE_CANONICALIZATION_POLICY_V1,
                "valid": false,
    ...
    let result = canonicalize_parameter_profile_v1(&profile);
