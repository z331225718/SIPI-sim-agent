...
use sipi_ami_text::{
    AmiParameterValueV1, PARAMETER_LIST_STARTS_WITH_POLICY_V1, ParameterListStartsWithErrorV1,
    list_starts_with_v1,
};
...
pub(crate) fn main() {
    ...
    let Some(input) = input else {
        println!(
            "usage: p4b_02b146_parameter_list_starts_with_runner --input <path> [--report <path>]"
        );
        return;
    };
    ...
    let prefix_name = value
        .get("prefix_name")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let prefix_type = value
        .get("prefix_type")
        .and_then(|v| v.as_str())
        .unwrap_or("");
