...
| M5B-03 | 消费 M4 channel resolver 并做 Link parity（含原 M3-08b 数值等价接线） | **RFM current-drive artifact bridge 已接受（2026-08-08）**：`876d380`/`ce3069a` 与 clean source `7fb6182`/`93c8c9e` 使 hash-verified `meta.json`/`response.bin` 直入现有 Rust receiver，不重跑 Agent-Spice；sign、ports、dt、FFT shape 和 artifact provenance fail-closed。OMP `msg_0ad1e6785cf8` 与交叉审查 `msg_acedd84f2ef2` 均为 0 P1 / 0 P2。**S2P external-boundary admission 与固定 fixture Link handoff parity 已接受（2026-08-08）**：SIPI `029c09d`/`58971aa`/`677f8af`/`437d8bc`/`221ac0c`/`2adbcc3` 与 clean source `244f6db`/`f6ba031` 将既有 Python `calc_chnl_h` 的离散 `V/sample` impulse 仅在 DTO 边界换为 `V/s`，再经唯一一次 M4 `resolve_channel` 注入真实 `sim-native` CLI；同配置 `channel_16ghz_3db.s2p` 的 impulse、channel/rx/CTLE/DFE arrays、DFE decision/clock、BER/sampled-eye/jitter compare 均通过。OMP `msg_f355f8f9b021` 与交叉审查 `msg_e09811d1e784` 均为 0 P1 / 0 P2。此项仅证明该外部 S2P fixture 的 Link handoff parity，**不证明 M4 计算或独立验证 S2P conditioning/IFFT/termination/resampling，也不认证 ADS/AMI/S4P/通用 S 参数；visual-eye 与 statistical-eye width 归 M5B-05**。**RFM same-config evidence currently blocked（2026-08-08）**：clean source tracked `block_2.rfm` is `894b5d...`, while its immutable response golden declares `5716f6...`; the original bytes and `response.bin` are unavailable. The lock-pinned executable (`111ff6...`) still reproduces the golden response hash `ce80d6...`, but the identity mismatch makes that result non-acceptance diagnostic evidence only. Do not rewrite the golden without separate user approval. M5B-03 remains open for a provenance-valid RFM current-domain reference-to-native compare report (current sign/ports/FFT mapping/provenance); prohibit a second converter in `sipi-link`. |
| M5B-04 | 完整 host-driven AMI contract | **Candidate transport 与 PyBERT 薄适配器已接受，主任务仍 open（2026-08-09）**：`c51029d` 新增 Windows-only 显式 `ami-host-candidate` v1 process transport，以 `sipi-ami` 的唯一 `AmiDll`/`AmiModel` Init/GetWave/Close 路径承接原始 ABI buffers；`a14120b` 将 result 的 `requestSha256` 绑定到解析前原始 manifest bytes。clean PyBERT `d4af5e0` 隔离地新增显式 `RustAmiHostCandidate` process adapter：钉定 executable/build-info/request/result/model 身份，临时 sidecar 仅在成功后消费，timeout/cancel/partial output 全部 fail-closed，并把未改写 waveform 仅交给既有 native DFE receiver；它未进入任何 Web/default/auto/compare registry，且不加载 Python DLL host。上述工作不写入 `engine.lock`，不进入生产 resolver/default/fallback，也不宣称真实 DLL、candidate bundle、AMI numerical parity 或跨平台认证。动态 clean-room ABI stub 与 synthetic child 分别仅是 transport/lifecycle evidence；公共 `example_rx.dll` 仍受 fixture-distribution compliance blocker 约束。OMP `msg_681bb805f087`/`msg_d7dc6b2cccb8`/`msg_28c75f0e5a27` 与交叉审查 `msg_6c88a2451f5a`/`msg_b3f4a53f44d7`/`msg_c61ba4387555` 均为 0 P1 / 0 P2。此前 metadata-layout prerequisite 亦已由 `e204d0b` clean-room parser coverage 补齐（OMP `msg_5cbd6c6bd68e`、交叉审查 `msg_bc9c7a1ac800`）。后续仍需 loader dependency-closure 可复现 candidate bundle/许可分类、显式 lock promotion、真实授权 DLL runtime 与 AMI parity；Python host 保留为 reference，直至另行完成 AMI parity。详见 `docs/m5b-ami-host-preflight.v1.md` 与 `docs/m5b-ami-host-candidate.v1.md`。 |
