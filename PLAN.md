...
- [ ] **P3B-04** 已接受 external-only same-source reference-bit provenance 与 Windows RFM receive-waveform 到 test-only product `ReceiverInputV1` handoff replay：两次 fresh external replay 的 waveform/bit hash 与单次 product boundary validation 均通过，Orca 审计 `0 P1 / 0 P2`。现已实现独立、stdlib-only 的 charter evaluator；下一步须用它与唯一产品 receiver 对同一双重 replay sidecar 做逐项 compare。当前仍为 `required_pending_receiver_compare`，不宣称 receiver 或 stage parity，禁止复用 retained/old-Rust receiver。证据见 [2026-08-10-p3b-rfm-reference-bits-preflight.md](docs/baselines/audits/2026-08-10-p3b-rfm-reference-bits-preflight.md) 和 [2026-08-10-p3b-rfm-receiver-handoff-replay.md](docs/baselines/audits/2026-08-10-p3b-rfm-receiver-handoff-replay.md)。
