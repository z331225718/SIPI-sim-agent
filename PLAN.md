...
| M5B-04 | 完整 host-driven AMI contract | **Candidate transport、PyBERT 薄适配器、candidate-bundle policy 与 PE/twin-build assurance 已接受，主任务仍 open（2026-08-09）**：`c51029d` 新增 Windows-only 显式 `ami-host-candidate` v1 process transport，以 `sipi-ami` 的唯一 `AmiDll`/`AmiModel` Init/GetWave/Close 路径承接原始 ABI buffers；`a14120b` 将 result 的 `requestSha256` 绑定到解析前原始 manifest bytes。clean PyBERT `d4af5e0` 隔离地新增显式 `RustAmiHostCandidate` process adapter；它未进入任何 Web/default/auto/compare registry，且不加载 Python DLL host。`c1d4d97` 新增外部目录 candidate bundle builder/verifier，拒绝 vendor `.dll/.ami/.ibs`、third-party closure、`engine.lock` promotion 和 production-resolvable 声称。`405dfb1` 解析 PE32+ normal/delay imports，记录 section/system hash 与 API-set contracts，并从两个新建外部 target 的 `--locked --release`、`CARGO_INCREMENTAL=0` build 得到 `byteReproducible=false`；因此 `promotionEligible=false`，不得 lock promotion。动态 clean-room ABI stub 与 synthetic child 分别仅是 transport/lifecycle evidence；公共 `example_rx.dll` 仍受 fixture-distribution compliance blocker 约束。OpenCode `msg_7949d9ab636f`/`msg_0a88f7642e48` 均为 0 P1 / 0 P2；按用户 2026-08-09 OMP 订阅受限时单审继续的决策记录，OMP `msg_d9d9f01901e0`/`msg_1f30b1467f95` 留待恢复补审。此前 metadata-layout prerequisite 亦已由 `e204d0b` clean-room parser coverage 补齐。后续仍需解释并消除 twin-build `.rdata`/timestamp 漂移、显式 lock promotion、真实授权 DLL runtime 与 AMI parity；Python host 保留为 reference，直至另行完成 AMI parity。 |
