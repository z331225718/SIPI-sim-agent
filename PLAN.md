...
- [ ] **P6-04** **P6-04a/04b 已完成：** 唯一 fixed TRAN→causal-FIR edge 可在调用者提供的协作式 `RunContext` 中执行 attempt `1`，共享 TRAN/FIR checkpoint 和 checked work/byte admission；其唯一 composite project node 也已严格绑定一个 in-memory request/consumer policy、project id 与同一 runtime policy。取消、deadline、资源或计算失败不返回 result/record。P6-04b Orca 独立审计为 `0 P1 / 0 P2`，证据见 [2026-08-11-p6-fixed-project-attempt.md](docs/baselines/audits/2026-08-11-p6-fixed-project-attempt.md)。generic dispatch、multi-edge、retry、cache、持久状态、artifact 与 CLI project executor 仍未实现。
