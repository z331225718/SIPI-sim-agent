...
| M5B-04 | 完整 host-driven AMI contract | **Candidate transport 已接受，主任务仍 open（2026-08-09）**：`c51029d` 新增 Windows-only 显式 `ami-host-candidate` v1 process transport，以 `sipi-ami` 的唯一 `AmiDll`/`AmiModel` Init/GetWave/Close 路径承接原始 ABI buffers；`a14120b` 将 result 的 `requestSha256` 绑定到解析前原始 manifest bytes，防止 adapter 将成功输出误归属到另一请求。严格校验 request/result schema、model/sidecar hash、metadata、finite/路径约束，并在 Close 成功后才原子发布。它不写入 `engine.lock`，不进入生产 resolver/default/fallback，也不宣称真实 DLL、candidate bundle、PyBERT route、AMI numerical parity 或跨平台认证。动态 clean-room ABI stub 是 transport/lifecycle 证据；公共 `example_rx.dll` 仍受 fixture-distribution compliance blocker 约束。OMP `msg_681bb805f087`/`msg_d7dc6b2cccb8` 与交叉审查 `msg_6c88a2451f5a`/`msg_b3f4a53f44d7` 均为 0 P1 / 0 P2。此前 metadata-layout prerequisite 亦已由 `e204d0b` clean-room parser coverage 补齐（OMP `msg_5cbd6c6bd68e`、交叉审查 `msg_bc9c7a1ac800`）。后续仍需 loader dependency-closure 可复现 candidate bundle/许可分类、显式 lock promotion，以及 PyBERT 薄 process adapter 的 clock/lock/cancel/timeout/malformed/partial-output fail-closed 契约；Python host 保留为 reference，直至另行完成 AMI parity。详见 `docs/m5b-ami-host-preflight.v1.md` 与 `docs/m5b-ami-host-candidate.v1.md`。 |
