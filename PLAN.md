...
| ID | 任务 | 说明 |
| --- | --- | --- |
| M5B-01 | 继续现有 Task 12/13 | 完整 result parity、RSS、nightly、故障演练 |
| M5B-02 | 解锁 `auto` 前完成 strict NRZ/RLGC 门禁 | 当前 blocked 状态不得被平台绕过 |
...
| M5B-06 | 修改现有 Task 14 目标 | 最终目的地从 Agent-Spice 改为 `SIPI-sim-agent`，避免二次搬迁 |
