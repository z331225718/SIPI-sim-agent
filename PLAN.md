...
- [ ] **P4B-02** clean-room 实现 `.ami` 参数语义，固定 raw UTF-8 bytes、binding 和 validation。
  ...
  - [x] **P4B-02b0** raw-parameter-text identity/binding foundation：`parse_and_bind_v1` 只将 caller-provided raw UTF-8 bytes 与其 structural AST 以精确内存字节绑定；`verify_binding_v1` 要求同一 bytes 并重新 structural parse。无任何 line ending/Unicode/case/escape/token spelling 归一化；不定义 parameter/default/reserved/model-specific/IBIS/DLL/ABI 语义。真正 P4B-02 继续等待 profile、IBIS binding 与 parameter contract。
