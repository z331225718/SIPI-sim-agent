...
| M5B-03 | 消费 M4 channel resolver 并做 Link parity（含原 M3-08b 数值等价接线） | **RFM current-drive artifact bridge 已接受（2026-08-08）**：`876d380`/`ce3069a` 与 clean source `7fb6182`/`93c8c9e` 使 hash-verified `meta.json`/`response.bin` 直入现有 Rust receiver，不重跑 Agent-Spice；sign、ports、dt、FFT shape 和 artifact provenance fail-closed。OMP `msg_0ad1e6785cf8` 与交叉审查 `msg_acedd84f2ef2` 均为 0 P1 / 0 P2。**S2P external-boundary admission 已接受（2026-08-08）**：SIPI `029c09d`/`58971aa`/`677f8af`/`437d8bc` 与 clean source `244f6db`/`f6ba031` 将既有 Python `calc_chnl_h` 的离散 `V/sample` impulse 仅在 DTO 边界换为 `V/s`，再经唯一一次 M4 `resolve_channel` 注入 native Link；sentinel、hash、dt、port/sign 与 lineage 全部 fail-closed。OMP `msg_14c2c2edc93b` 与交叉审查 `msg_fe0137aaaf4f` 为 0 P1 / 0 P2。此项只证明 external handoff/admission，**不证明 M4 计算 S2P，也不证明 S2P 数值 parity**。M5B-03 仍需同配置的 Python reference 与 native Link 数组/指标 compare report，锁定 S2P/S4P 的 pulse、termination、port intent 和 FFT scaling；禁止在 `sipi-link` 再写一套转换。**验收含原 M3-08b：RFM→PyBERT 数值等价（current sign/端口/FFT mapping + compare report/provenance）**——RFM 频域响应→impulse 转换经 Python 外部边界（`utility/sparam.py`/pybert `agent_spice_channel`）产出 DTO，平台不写第二套 resolver |
