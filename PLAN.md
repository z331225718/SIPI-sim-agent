...
| M5B-04 | 完整 host-driven AMI contract | **Preflight blocked（2026-08-09）**：M5A-09 的 clean-room `sipi-ami` 已有 Rust `libloading` Init/GetWave/Close library contract，但目前唯一获 `native/crates/sipi-circuit/engine.lock` 批准的 Windows executable (`111ff6...`) 仅声明 `build-info`/`rfm-response`，没有可锁定的版本化 AMI host process subcommand；不能以本地重建 binary 或 Python host 冒充该路由。此前的 metadata-layout prerequisite 已由 clean-room parser coverage 补齐：`ami-inspect` 现可读取 public `example_rx.ami` 的 model-wrapper/`Reserved_Parameters` 元数据，但这不是 DLL runtime、candidate bundle 或 PyBERT route 证据；OMP `msg_5cbd6c6bd68e` 与交叉审查 `msg_bc9c7a1ac800` 均为 0 P1 / 0 P2。详见 `docs/m5b-ami-host-preflight.v1.md`。此前 preflight 审计 OMP `msg_9e2b50c17592` 与交叉审查 `msg_a0a312a5409a` 均为 0 P1 / 0 P2。后续需候选 bundle + request/response protocol + lock promotion + Python 薄 process adapter，补 clock/lock/cancel/timeout/partial-output fail-closed fixture；Python host 保留为 reference 直到 parity 通过。 |
