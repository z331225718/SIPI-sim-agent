...
- [ ] **P4A-04** 实现 required I-V/V-T/ramp/package 语义与显式 interpolation/extrapolation policy。
  ...
  - [ ] **P4A-04d** two-fresh-custody external compare：oracle-only orchestrator 将两份 external-only asset materialization 分别交给独立 raw-table observer 与 test-only Rust runner；仅保留 hashes/aggregate metrics 的外部报告。实现和 external run 已具备，待单审与 acceptance evidence 收口。
