...
| M5B-04 | 完整 host-driven AMI contract | **Candidate transport、PyBERT 薄适配器和 candidate-bundle policy 已接受，主任务仍 open（2026-08-09）**：`c51029d` 新增 Windows-only 显式 `ami-host-candidate` v1 process transport，以 `sipi-ami` 的唯一 `AmiDll`/`AmiModel` Init/GetWave/Close 路径承接原始 ABI buffers；`a14120b` 将 result 的 `requestSha256` 绑定到解析前原始 manifest bytes。clean PyBERT `d4af5e0` 隔离地新增显式 `RustAmiHostCandidate` process adapter：钉定 executable/build-info/request/result/model 身份，临时 sidecar 仅在成功后消费，timeout/cancel/partial output 全部 fail-closed，并把未改写 waveform 仅交给既有 native DFE receiver；它未进入任何 Web/default/auto/compare registry，且不加载 Python DLL host。`c1d4d97` 新增外部目录 candidate bundle builder/verifier：它绑定 clean SIPI commit、Cargo.lock、`agent-spice@2cc92316` Git-object license evidence、candidate build-info capability、exe hash/size 和 system-only declared closure；它拒绝 vendor `.dll/.ami/.ibs`、third-party closure、`engine.lock` promotion 和 production-resolvable 声称。2026-08-09 外部 clean release artifact 的 manifest SHA-256 为 `50f4a4f2...ae00cb`，但它只标 input-pinned，尚未声称 binary reproducibility。动态 clean-room ABI stub 与 synthetic child 分别仅是 transport/lifecycle evidence；公共 `example_rx.dll` 仍受 fixture-distribution compliance blocker 约束。OpenCode `msg_7949d9ab636f` 为 0 P1 / 0 P2（P3：`--source-root` 与 system DLL list 是显式 operator trust boundary）；按用户 2026-08-09 OMP 订阅受限时单审继续的决策记录，OMP `msg_d9d9f01901e0` 留待恢复补审。此前 metadata-layout prerequisite 亦已由 `e204d0b` clean-room parser coverage 补齐。后续仍需经 PE/loader 实测的 dependency closure、独立 clean rebuild 可复现观测、显式 lock promotion、真实授权 DLL runtime 与 AMI parity；Python host 保留为 reference，直至另行完成 AMI parity。详见 `docs/baselines/m5b-ami-candidate-bundle-preflight.v1.md`。 |
