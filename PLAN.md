...
- [ ] **P3C-04ah** strict-index time-domain residual diagnostic core：为定位不合格的 waveform-only NRMSE，`sipi-compare` 已实现 exact selected-v3 诊断 leaf，固定分割三个 16,352-sample period，记录未经变换的 reference/candidate/residual RMS、signed mean、NRMSE、residual digest与最大误差；第三周期再固定 511 个 UI 的 residual-energy digest/最小索引 tie-break。第三周期 NRMSE 必须 bit-exact 等于 v3 结果。新增 v2 external-only runner，以 owner-confirmed finite-edge source v2 独立重放两次 sealed selected S4P intake、candidate与同一 ADS RX reference；外部 hash-only evidence仍待 preparation clean archive 实跑。无 CLI、外部读取、FFT、lag/phase/fold search、对齐、resample、gain/DC/polarity 修正或 adjusted acceptance；OpenCode 审查仍因本地平台启动失败而 pending，不能以此替代审计。
