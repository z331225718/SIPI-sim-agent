...
| M5B-03 | 消费 M4 channel resolver 并做 Link parity（含原 M3-08b 数值等价接线） | **RFM current-drive artifact bridge 已接受（2026-08-08）**：`876d380`/`ce3069a` 与 clean source `7fb6182`/`93c8c9e` 使 hash-verified `meta.json`/`response.bin` 直入现有 Rust receiver，不重跑 Agent-Spice；sign、ports、dt、FFT shape 和 artifact provenance fail-closed。OMP `msg_0ad1e6785cf8` 与交叉审查 `msg_acedd84f2ef2` 均为 0 P1 / 0 P2。M5B-03 仍需同一 production resolver 锁 S2P/S4P 的 pulse、termination、port intent 和 FFT scaling；禁止在 `sipi-link` 再写一套转换。**验收含原 M3-08b：RFM→PyBERT 数值等价（current sign/端口/FFT mapping + compare report/provenance）**——RFM 频域响应→impulse 转换经 Python 外部边界（`utility/sparam.py`/pybert `agent_spice_channel`）产出 DTO，平台不写第二套 resolver |
