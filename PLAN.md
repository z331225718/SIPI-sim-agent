...
- **P7-06g 前置 P0 修复已完成：** 已检测到并以 `.gitattributes` 修复普通 Windows checkout 对 byte-hashed 根治理文件的 CRLF 表现与 Git blob/`git archive` LF 表现分裂；P7 current chain 必须以新 canonical-LF P0 candidate 重新观察，不能继承此前 checkout-representation 拒绝。
