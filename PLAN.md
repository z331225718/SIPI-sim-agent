...
- [ ] **P4A-04** 实现 required I-V/V-T/ramp/package 语义与显式 interpolation/extrapolation policy。
  ...
  - [x] **P4A-04b** typed DC clamp evaluator core：`sipi-ibis` 以产品自有 `DcIvTableV1`、`InputClampDcModelV1` 和显式双驱动 probe 实现两张 signed I-V 表的线性插值、越界拒绝与 branch/total DC current；`C_comp` current 固定为零。它不解码 IBIS text 或外部资产，未实现 package/pin/PVT/V-T/ramp/differential/AMI/network。外部 profile decoder 与 compare 仍待执行。
