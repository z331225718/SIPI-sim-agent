...
**2026-08-24 v7 upstream-first 治理快照：** 新增
[`upstream-integration-ledger.v7`](docs/baselines/upstream-integration-ledger.v7.yaml)，绑定当前
clean candidate `246a285f`、tree `86248d92` 与显式
`git -c core.autocrlf=true archive --format=tar` archive
`bfc50b05f8fdc7338017052ebff2e21f75ef8e7e9465bbcb13012e80fb3f900b`（44738560 bytes）；v1-v6
保持 immutable。AS-04 仅绑定 relative source-map/NOTICE，HSPICE 仍 external blocker，numeric parity
...
仅为 internal staged DC/ACCM leaf；完整 public S4P + legal `portable.search` E2E 仍因
`Chain(Equalizer)` open，pinned numeric/canonical output、full parity 与 release 均 open；不做
DC/ACCM public parity 或 S-parameter fit，channel 仍 impulse-only。15 行仍 open，release-ready=0。

**2026-08-24 v8 upstream-first 治理快照：** 新增
[`upstream-integration-ledger.v8`](docs/baselines/upstream-integration-ledger.v8.yaml)，绑定当前
clean candidate `b2de8ed6`、tree `b7903ef2` 与显式
`git -c core.autocrlf=true archive --format=tar` archive
`3b59eb25ce7b8bd942d3d6e82d666da369136fb48d56154cd390228474a78b88`（44963840 bytes）；v1-v7
保持 immutable。AS-03 仅绑定 source-map/NOTICE，v2 global mismatch 与 v3 fixed-fixture
scope 仍 open，无 SI fit、numeric parity 或 release；PB-02 仅绑定 43c formal v2 的
single fixed-fixture environment-local numeric observation，不代表 branch completion、global
parity、bit-reproducible build、pure-Python legacy、promotion 或 release。COM 保持 v7 的
internal staged DC/ACCM leaf，完整 public S4P + legal-search ACCM E2E 仍因
...
明确迁入的 admission 子集，且不执行 solver。COM 与 PB 已分别从不可变 preparation
commit 的 clean archive 完成两次独立 current-candidate replay，并由 additive bound
evidence 记录；这些观察仍只覆盖各自冻结 corpus/profile。不得据此声称
branch-complete、admission-complete、完整输入分支、产品 direct route、waveform
parity、许可结论或 release promotion。对应账本与候选证据见
`docs/baselines/upstream-rust-parity-ledger.v1.yaml`、
`docs/baselines/com-03-direct-port-bound.v2.yaml`、
...
- [x] **P2-06** 对 required Agent-Spice TRAN 例子生成 stage compare：parsed circuit、time grid、waveforms、measurements。**P2-06g 已完成（current-surface stage-compare coverage record）：** 以 `sipi.p2-06.stage-compare-coverage.v1` 机械记录当前 bounded 产品面的四 stage 覆盖状态：`time_grid`/`waveforms`（time/v(in)/v(out) 冻结 indexed gates）已由 v4 current evidence（clean archive `538b5dd`、oracle `2cc92316`、双次 replay、report `854e34fd...`）覆盖，v1-v3 保留 source-drift historical；`parsed_circuit` 明确 `not_applicable_current_surface`（当前面无 netlist parser，P2-03 冻结 arbitrary_netlist rejected）；`measurements` 明确 `not_implemented`（P2-03 freeze）。泛化 stage compare（parsed circuit/measurements）待未来 netlist 面与 P2-04 tolerance owner 决策，见 [audit](docs/baselines/audits/2026-08-16-p2-06-stage-compare-coverage.md)。
  ...
  - [x] **P2-06k** exact-profile parsed measurement consumer：`sipi-tran` 仅接受固定七行 title/RC/PULSE/`.tran`/单一 `MAX V(out)`/`.end` deck，复用既有 one-node RC/PULSE 内核并对实际输出样本求 MAX；`.op`、`.ac`、额外 device、windowed measurement 与泛化 netlist 均 fail-closed。clean detached Agent-Spice oracle 与产品各双次运行通过冻结误差门，该观察未读取 Agent-Spice measurement 输出且不是同输入 parser compare，因此仅完成 bounded product prerequisite，P2-06 保持 open；不声称 generic SPICE/MNA/OP/AC/general measurements。见 [evidence](docs/baselines/p2-06-exact-rc-measurement-implementation.v1.yaml) 与 [audit](docs/baselines/audits/2026-08-21-p2-06-exact-rc-measurement-implementation.md)。
  - [x] **P2-06l** same-deck measurement replay：同一 143-byte 七行 deck 分别由 pinned Agent-Spice 与产品 fresh process 各运行两次；oracle 值直接取 `SimulationResult.measurements[0]`，产品 `MAX V(out)` 误差 `9.963737488231927e-7 V`，通过冻结门。该 exact-profile scoped close 不扩展 generic SPICE/MNA、OP/AC 或 general measurement，见 [evidence](docs/baselines/p2-06-exact-parser-measurement-replay.v2.yaml)。
...
- [ ] **P3B-02** **P3B-02a/02b 已完成：** `sipi-link` 仅实现 product-owned causal FIR full linear convolution，`sipi link run --stdin` 将同一严格 direct-launch/bypass contract 发布为不可变 artifact；显式限制/数值溢出 fail-closed，CTLE/FFE 仍仅 bypass。equalizer 子集待明确 Link profile 与独立语义后开始；P3B-03 的 required RFM receiver charter 阻塞不受本切片影响。证据见 [2026-08-10-p3b-causal-fir-convolution.md](docs/baselines/audits/2026-08-10-p3b-causal-fir-convolution.md)、[2026-08-10-p3b-causal-fir-cli-run.md](docs/baselines/audits/2026-08-10-p3b-causal-fir-cli-run.md)。**P3B-02c 已完成（link kernel-singleton & bypass-only gate）：** 以 `sipi.p3b-02.link-kernel-singleton.v1` 机械固化：3 个 causal-FIR 卷积符号单点定义于 `crates/sipi-link/src/lib.rs`，`sipi-cli`/`sipi-pipeline` 仅经 `sipi_link` import 调用、无复制内核；bypass-only 断言（`RxStagesV1::bypass().ctle(), CtleStageV1::Bypass`）必须保留，任何非 bypass CTLE/FFE stage selector（equalizer 实现）fail-closed。该 gate 不开始 equalizer 子集、不接受 Link profile，CTLE/FFE 保持 bypass-only 直至 owner 明确 Link profile 与独立语义，见 [audit](docs/baselines/audits/2026-08-16-p3b-02-link-kernel-singleton.md)。
  - [x] **P3B-02d** equalizer semantics gap：绑定 owner 已冻结的 `RX CTLE -> RX FFE` 顺序与 live bypass-only contract，精确列出 CTLE transfer/state、FFE tap/order/units/state、timebase/output、tolerance/oracle/provenance 缺口；不新增 runtime 或 profile claim。见 [evidence](docs/baselines/p3b-02-equalizer-semantics-gap.v1.yaml) 与 [audit](docs/baselines/audits/2026-08-21-p3b-02-equalizer-semantics-gap.md)。
  - [x] **P3B-02e** named causal-FIR prerequisite：新增固定 CTLE 命名槽到 FFE 命名槽的显式 `Bypass | ExplicitCausalFir` 组合，复用唯一 full-linear convolution 内核并对累计 output/MAC 预算 fail-closed；v1 wire 继续 bypass-only。该能力只冻结次序和有界组合，未定义 CTLE transfer/grid/state 或 FFE UI/cursor/profile 语义，因此 P3B-02 保持 open。
  - [x] **P3B-02f** PyBERT profile source audit：pinned source 中 CTLE default 同时出现 1.7 dB 与 4.0 dB，FFE 还存在 15/20 taps 与 disabled/empty 的冲突形态；未按 oracle 表现择优，也未改 wire/runtime。P3B-02 继续等待唯一权威 profile，见 [evidence](docs/baselines/p3b-02-pybert-profile-source-audit.v1.yaml)。
...
- [ ] **P3C-03** 现有 aligned-array、metric profile、C4 compare 与 exact ADS replay 均已完成；当前 selected high-loss waveform 的 strict same-index third-period NRMSE 为 `0.02911956313297956`，超过固定 1% gate。**P3C-03h 已完成（原项目 channel semantics observation）：** 从 retained ADS dataset/netlist 与 PyBERT/PyAMI 原项目逐对象绑定 seed、strobe、interpolation、causality、delay/trimming 语义；ADS seed/首 bits `1101` 与 finite-edge phase0/phase1 source-only interior 一致，但完整 channel mismatch 仍存在，因此本观察不识别或排除 root cause。ADS/PyBERT 的 interpolation/causality/delay/trimming 与当前产品 policy 不同，但本观察不授权移植、alignment、fit、resample 或 tolerance relaxation。见 [evidence](docs/baselines/p3c-original-project-channel-semantics-observation.v1.yaml) 与 [audit](docs/baselines/audits/2026-08-21-p3c-original-project-channel-semantics-observation.md)。
  - [x] **P3C-03l** required COM metric bundle prerequisite：`sipi-compare` 新增固定 COM/ERL/TD-ILN 三标量 dB bundle，三项 tolerance 必须由调用者显式提供且无默认、alias、alignment、单位换算或 oracle。主项继续等待 authoritative reference、TD-ILN value、input provenance、checkpoint alignment 与 per-metric acceptance tolerance，不能用 C4 的 ICN 替代 TD-ILN。见 [evidence](docs/baselines/p3c-03-com-metric-bundle-capability.v1.yaml)。
  - [x] **P3C-03m** strict checkpoint compare boundary：新增固定 COM/ERL/TD-ILN 三指标的 strict same-checkpoint、no-alignment 比较，三项均采用 owner 冻结的 0.1 dB absolute tolerance；该 core 不提供 oracle、checkpoint 名或 acceptance。P3C-03 继续等待可用 TD-ILN 与 exact clean checkpoint/input provenance，见 [evidence](docs/baselines/p3c-03c-com-metric-checkpoint-boundary.v1.yaml)。
  - [x] **P3C-03i** exact metric surface hardening + current gap：`compare_metric_profile_v1` 现在拒绝 profile 外 candidate metric；同时绑定 required `COM/ERL/TD-ILN` 与现有 C4 `COM/ICN/ERL` 的不等价事实，alignment、provenance、tolerance 与 authoritative reference 继续阻塞。见 [evidence](docs/baselines/p3c-03-current-semantics-gap.v1.yaml) 与 [audit](docs/baselines/audits/2026-08-21-p3c-03-current-semantics-gap.md)。
...
`sipi.owner-recommended-bundle-authorization.v1`. P2-06 is now scoped-closed by
same-deck Agent-Spice measurement replay, and P4A-02 is scoped-closed as
static/declaration-only with dynamic endpoint explicitly deferred. The
remaining-items ledger now has 20 entries: 5 product/domain blockers, 6
external asset/oracle blockers, and 9 release gates. This authorization does
not create third-party rights, select missing profiles, or promote P7.

- [ ] **P4A-01** **Current owner-input reconciliation v2：** 冻结 exact IBIS 5.0 file `fixtures/ibis/as4c512m16md4v-053bin.ibs` 与已观察 structural inventory；不猜 model/corner behavior，behavior selection 转 P4A-02/P4A-03。当前 tracked 文件已确认是官方对象的严格截断前缀且无 `[End]`，因此主项转为 exact full asset/profile/rights 的 external blocker，不再把截断对象当语义输入。
  ...
  - [x] **P4A-01h** official object rights recheck：fresh 取得的完整 Alliance 对象精确复现 6,138,000 bytes/SHA-256，但仅观察到 disclaimer/copyright 与 customer modification wording，未找到可验证的再分发许可；完整 bytes 继续 external-only，P4A-01 保持 external blocker，见 [evidence](docs/baselines/p4a-01-official-object-rights-recheck.v1.yaml)。
- [x] **P4A-02** 观察侧基于公开 IBIS 标准和授权 black-box 形成行为规格。 **P4A-02b 已完成（authorized Gen5 behavior-spec 观察）：** authorized Gen5 behavior-spec observation（owner C1 决策将 behavior-spec 形成约束于公开标准与授权 black-box 观察侧），以 `sipi.p4a-02b.gen5-behavior-spec-observation.v1` 机械绑定，证据 docs/baselines/p4a-02b-gen5-behavior-spec-observation.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p4a-02b-gen5-behavior-spec.md)
  ...
  - [x] **P4A-02c** static/dynamic scope split：exact authorized Gen5 结果仅作为 observation，现有 static/declaration inventory 只关闭其有界子范围；动态 endpoint 仍缺 reference/supply/initial-state/integration/timebase/grid/stimulus/resource/tolerance/acceptance 等外部权威事实，admission=false。其当时的 open disposition 已由 02d 的 owner scope decision 加法式取代。
  - [x] **P4A-02d** current disposition：owner 选择 E3=A，本阶段明确只保留 static/declaration scope，dynamic endpoint 不实现且 11 项缺口原样保留为未来独立能力。该 scope closure 不构成 transient admission，见 [evidence](docs/baselines/p4a-02-dynamic-endpoint-current-disposition.v2.yaml)。
- [ ] **P4A-03** clean-room Rust parser、typed AST 与严格 diagnostics。03a structural foundation 与 03d..03bg 的 typed 声明/keyword 微切片已落库；本次质量审计确认这些后续切片主要由产品 Rust 与同算法 Python 自测支撑，不能据此宣称完整 IBIS 语义、外部标准一致性或 required-profile acceptance。在完成规范追溯、生产调用可达性、资源预算与独立 evidence 重绑前，主项保持 open，停止继续扩张微切片。
  - [x] **P4A-03bh** typed inventory consumer：新增单一 bounded consumer 串联 structural parser、semantic envelope、typed Model/Selector/Pin 与 caller-owned marker 分类；selected asset 在 `DQ_PIN -> DQ_60OHM_60OHM_PREEMP_ON` 缺失 Model block 处 fail-closed，不选择 branch/corner/PVT 或电气行为。P4A-01/02 剩余字段见 [gap audit](docs/baselines/audits/2026-08-21-p4a-01-02-semantics-gap.md)；实现边界见 [evidence](docs/baselines/p4a-03-typed-inventory-consumer.v1.yaml) 与 [audit](docs/baselines/audits/2026-08-21-p4a-03-typed-inventory-consumer.md)。
  - [x] **P4A-03bi** complete-document/external inventory closure：typed consumer 现要求唯一、无 payload、最终 `[End]`；仓内 selected object 因截断而在 `missing_end` fail-closed。对 exact Alliance external object 的 test-only 观察得到 95 models/4 selectors/200 pins，并仅关闭 complete-document declaration/linkage scope；rights、model/corner/PVT、电气与动态行为均未接受。
  - [x] **P4A-03bj** selected-model blocker：95 个 model 全部被 direct/selector inventory 覆盖，但 selector 分支为 9/9/9/67，owner 未选 signal role、model/selector、branch、corner/PVT 或 table family；scoped parser 不得猜首项或 CKE，P4A-03 保持 open，见 [evidence](docs/baselines/p4a-03-selected-model-missing-selection-blocker.v1.yaml)。
...
<!-- Historical verifier anchors only; all records are product-owned self-crosschecks without independent hash binding: **P4B-02b1** **P4B-02b2** **P4B-02b3** **P4B-02b4** **P4B-02b5** **P4B-02b6** **P4B-02b7** **P4B-02b8** **P4B-02b9** **P4B-02b10** **P4B-02b11** **P4B-02b12** **P4B-02b13** **P4B-02b14** **P4B-02b15** **P4B-02b16** **P4B-02b17** **P4B-02b18** **P4B-02b19** **P4B-02b20** **P4B-02b21** **P4B-02b22** **P4B-02b23** **P4B-02b24** **P4B-02b25** **P4B-02b26** **P4B-02b27** **P4B-02b28** **P4B-02b29** **P4B-02b30** **P4B-02b31** **P4B-02b32** **P4B-02b33** **P4B-02b34** **P4B-02b35** **P4B-02b36** **P4B-02b37** **P4B-02b38** **P4B-02b39** **P4B-02b40** **P4B-02b41** **P4B-02b42** **P4B-02b43** **P4B-02b44** **P4B-02b45** **P4B-02b46** **P4B-02b47** **P4B-02b48** **P4B-02b49** **P4B-02b50** **P4B-02b51** **P4B-02b52** **P4B-02b53** **P4B-02b54** **P4B-02b55** **P4B-02b56** **P4B-02b57** **P4B-02b58** **P4B-02b59** **P4B-02b60** **P4B-02b61** **P4B-02b62** **P4B-02b63** **P4B-02b64** **P4B-02b65** **P4B-02b66** **P4B-02b67** **P4B-02b68** **P4B-02b69** **P4B-02b70** **P4B-02b71** **P4B-02b72** **P4B-02b73** **P4B-02b74** **P4B-02b75** **P4B-02b76** **P4B-02b77** **P4B-02b78** **P4B-02b79** **P4B-02b80** **P4B-02b81** **P4B-02b82** **P4B-02b83** **P4B-02b84** **P4B-02b85** **P4B-02b86** **P4B-02b87** **P4B-02b88** **P4B-02b89** **P4B-02b90** **P4B-02b91** **P4B-02b92** **P4B-02b93** **P4B-02b94** **P4B-02b95** **P4B-02b96** **P4B-02b97** **P4B-02b98** **P4B-02b99** **P4B-02b100** **P4B-02b101** **P4B-02b102** **P4B-02b103** **P4B-02b104** **P4B-02b105** **P4B-02b106** **P4B-02b107** **P4B-02b108** **P4B-02b109** **P4B-02b110** **P4B-02b111** **P4B-02b112** **P4B-02b113** **P4B-02b114** **P4B-02b115** **P4B-02b116** **P4B-02b117** **P4B-02b118** **P4B-02b119** **P4B-02b120** **P4B-02b121** **P4B-02b122** **P4B-02b123** **P4B-02b124** **P4B-02b125** **P4B-02b126** **P4B-02b127** **P4B-02b128** **P4B-02b129** **P4B-02b130** **P4B-02b131** **P4B-02b132** **P4B-02b133** **P4B-02b134** **P4B-02b135** **P4B-02b136** **P4B-02b137** **P4B-02b138** **P4B-02b139** **P4B-02b140** **P4B-02b141** **P4B-02b142** **P4B-02b143** **P4B-02b144** **P4B-02b145** **P4B-02b146** **P4B-02b147** **P4B-02b148** **P4B-02b149** **P4B-02b150** **P4B-02b151** **P4B-02b152** **P4B-02b153** **P4B-02b154** **P4B-02b155** **P4B-02b156** **P4B-02b157** **P4B-02b158** **P4B-02b159** **P4B-02b160** **P4B-02b161** **P4B-02b162** **P4B-02b163** **P4B-02b164** **P4B-02b165** **P4B-02b166** **P4B-02b167** **P4B-02b168** **P4B-02b169** **P4B-02b170** **P4B-02b171** **P4B-02b172** **P4B-02b173** **P4B-02b174** **P4B-02b175** **P4B-02b176** **P4B-02b177** **P4B-02b178** **P4B-02b179** **P4B-02b180** **P4B-02b181** **P4B-02b182** **P4B-02b183** **P4B-02b184** **P4B-02b185** **P4B-02b186** **P4B-02b187** **P4B-02b188** **P4B-02b189** **P4B-02b190** **P4B-02b191** **P4B-02b192** **P4B-02b193** -->
  ...
  - [x] **P4B-02f production adapter selection**：默认 worker/host 的实际参数路径固定为 `parse_and_bind_v1 -> verify_binding_v1 -> AmiTextBindingV1 -> AMI_Init`；193 helper 仍 keep=0/quarantine=34/delete-candidate=159，未提升 typed helper 或执行删除。该记录只关闭生产 adapter 选择子范围；`.ami` 参数语义仍缺 authorized profile/contract，P4B-02 保持 open。
  - [x] **P4B-02g/P4B-08d/P4B-09a** dual-AMI lane admission：四资产 identity 与唯一 TX+RX composite profile 已复验，但没有 hash-bound vendor runtime grant、唯一参数树消费证据、动态/sidecar closure 或批准的安全隔离边界；worker 不是 security sandbox，`AMI_Init/GetWave` 均未调用。三主项继续阻塞，见 [evidence](docs/baselines/p4b-ads-pcie-gen5-dual-ami-lane3-admission.v1.yaml)。
...
- [ ] **P5-02** 冻结 r4.80 canonical parameter JSON、默认值、消费审计和 warning contract。 **P5-02e 已完成（canonical R480 参数键参考）：** canonical R480 parameter key reference，机械绑定；defaults/warning 契约仍待，证据 docs/baselines/p5-r480-canonical-parameter-reference.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02e-canonical-parameter-reference.md) **P5-02f 已完成（canonical 参数 JSON draft）：** canonical parameter JSON draft，机械绑定；reference-default 解析仍待，证据 docs/baselines/p5-r480-canonical-parameter-json-draft.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02f-parameter-json-draft.md) **P5-02g 已完成（canonical 参数 JSON v1）：** canonical parameter JSON v1，机械绑定；算术默认值解析边界明确，证据 docs/baselines/2026-08-16-p5-02g-canonical-json.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02g-canonical-json.md) **P5-02h 已完成（canonical 参数 JSON v2）：** bounded static arithmetic on defaults（v2），机械绑定；caller-dependent defaults 仍待，证据 docs/baselines/2026-08-16-p5-02h-canonical-json-v2.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02h-canonical-json-v2.md) **P5-02i 已完成（warning-call 观察）：** r4.80 源中 warning calls 观察，机械绑定；product warning contract 仍待，证据 docs/baselines/p5-r480-warning-observation.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02i-warning-observation.md) **P5-02j 已完成（scalar 默认值消费/解析）：** scalar default-rule resolution（_resolve_default port），产品 vs agent-com Python oracle crosscheck 一致，证据 docs/baselines/p5-02j-default-resolution-crosscheck-evidence.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02j-value-consumption.md) **P5-02k 已完成（MATLAB 数值字面量/向量-矩阵默认值）：** 完整 parse_matlab_literal port（vector/matrix），产品 vs agent-com Python oracle crosscheck 一致，证据 docs/baselines/p5-02k-matlab-literal-crosscheck-evidence.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02k-matlab-literal.md) **P5-02l 已完成（默认表达式集合解析器）：** default-expression set resolver，产品 vs oracle _resolve_default crosscheck 一致，证据 docs/baselines/p5-02l-resolve-parameters-crosscheck-evidence.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02l-resolve-parameters.md) **P5-02m 已完成（确定性 warning 检测核心）：** deterministic warning detector core，产品 vs 独立 reference crosscheck 一致，证据 docs/baselines/p5-02m-warning-detector-crosscheck-evidence.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02m-warning-detector.md) **P5-02n 已完成（确定性 warning 报告聚合器）：** deterministic warning-report aggregator（sipi-com warning_report_v1.rs），产品 vs 独立 reference crosscheck 一致，证据 docs/baselines/p5-02n-warning-report-crosscheck-evidence.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-02n-warning-report.md)
  ...
  - [x] **P5-02q** clean default/warning observation：对 pinned Agent-COM clean object 重验可物化 defaults 与 warning subset，只形成外部 source observation；完整 warning/default contract、required profile/input 与产品 acceptance 仍缺失，见 [evidence](docs/baselines/p5-02q-agent-com-clean-default-warning-observation.v1.yaml)。
  - [x] **P5-02r** original-13 参数/default/消费收口：用户选定的 13 workbook 路线以 MATLAB 参数槽矩阵、Rust 与 pinned materializer 的实际运行消费 projection、303 final scalar 和 TDILN named vectors 共同验收。line 9715 warning 精确映射；line 6337 仅为平台近零残差，不伪造为等价 warning。该收口不提升完整 warning catalog、未选 profile、generalized COM、IEEE 或 release。见 [evidence](docs/baselines/p5-02-06-original13-execution-closure.v1.yaml)。
...
- [x] **P5-05** workbook importer 与核心参数 DTO 分离，未使用字段仍进入消费报告。 **P5-05a 已完成（workbook importer xlsx stage）：** workbook importer xlsx stage（sipi-com workbook 解析），产品 vs 独立 reference crosscheck 一致，证据 docs/baselines/2026-08-16-p5-05a-workbook-importer.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-05a-workbook-importer.md) **P5-05b 已完成（CSV 配置读取 stage）：** CSV configuration reader stage，产品 vs 独立 reference crosscheck 一致，证据 docs/baselines/2026-08-16-p5-05b-csv-reader.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-05b-csv-reader.md) **P5-05c 已完成（MATLAB v5 配置读取 stage）：** MATLAB v5 configuration reader stage（mat_reader_v1.rs），产品 vs 独立 reference crosscheck 一致，证据 docs/baselines/2026-08-16-p5-05c-mat-reader.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-05c-mat-reader.md) **P5-05d 已完成（参数面报告 stage）：** parameter surface report stage（parameter_surface_v1.rs），产品 vs 独立 reference crosscheck 一致，证据 docs/baselines/2026-08-16-p5-05d-parameter-surface.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-05d-parameter-surface.md) **P5-05e 已完成（COM typed 参数 DTO merge 核心）：** COM typed parameter DTO merge core（com_parameters_v1.rs），产品 vs 独立 reference crosscheck 一致，证据 docs/baselines/p5-05e-com-parameters-crosscheck-evidence.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-05e-com-parameters.md) **P5-05f 已完成（COM 参数面解析与准入核心）：** `sipi-com` 新增 `com_parameter_resolver_v1.rs`：`resolve_com_parameter_controls_v1` 将合并后的 `ComParametersV1` DTO 解析为类型化的 `ComChainControlsV1` 控制参数，支持别名键提取与标量/向量类型校验，缺失必需键或类型不符 fail-closed；6 Rust 单测 + crosscheck（3 组 DTO 用例：产品 vs 独立 Python 参考 **3/3 matched_hash_bound**），证据 `docs/baselines/p5-05f-parameter-resolver-crosscheck-evidence.v1.yaml`，见 [audit](docs/baselines/audits/2026-08-16-p5-05f-parameter-resolver.md)。
  - [x] **P5-05g** workbook-to-DTO ingestion consumer：校验 surface 与同一 workbook，使用 caller-supplied canonical consumed keys、既有 case-insensitive lookup 和 resolved-default fallback，返回 typed DTO 与 consumed/workbook/defaulted/unconsumed 报告；不选择 COM profile 或 oracle。见 [evidence](docs/baselines/p5-05g-com-parameter-ingestion-stage.v1.yaml) 与 [audit](docs/baselines/audits/2026-08-21-p5-05g-com-parameter-ingestion.md)。
- [ ] **P5-06** 对 required COM 示例比较 normalized input、选择证据、intermediate arrays、COM/ERL/TDILN 等指标和 warnings。 **P5-06a 已完成（MATLAB oracle 首次运行）：** authorized MATLAB R2024b oracle 首次运行（621.6s，26.56 GHz target，hash-bound 证据 p5-06-matlab-oracle-first-run-evidence.v1.yaml：matlab_oracle.mat dd47cb3d... / summary.json 151d8a03...），required COM compare matrix 仍待，证据 docs/baselines/2026-08-16-p5-06a-matlab-oracle-first-run.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-06a-matlab-oracle-first-run.md) **P5-06b 已完成（oracle metric surface）：** oracle metric surface observation（14 项输出指标含 COM_dB/ICN_mV/ERL 与 internal checkpoints），机械绑定；compare matrix pending，证据 docs/baselines/2026-08-16-p5-06b-oracle-metric-surface.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-06b-oracle-metric-surface.md) **P5-06c 已完成（normalized input surface）：** normalized input surface（sipi-com canonical_input_keys_v1.rs），机械绑定；compare matrix pending，证据 docs/baselines/2026-08-16-p5-06c-normalized-input-surface.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-06c-normalized-input-surface.md) **P5-06d 已完成（canonical normalized-input key signature）：** canonical normalized-input key signature core，产品 vs 独立 reference crosscheck 一致，证据 docs/baselines/p5-06d-canonical-input-keys-crosscheck-evidence.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-06d-canonical-input-keys.md) **P5-06e 已完成（COM oracle metric reference 绑定）：** COM oracle metric reference binding（p5-06e-com-oracle-metric-reference.v1.yaml，digest f7b30359... sha256 验证、aggregate + case references 绑定），compare matrix 现可按 oracle 驱动，证据 docs/baselines/p5-06e-com-oracle-metric-reference.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-06e-oracle-metric-reference.md) **P5-06f 已完成（fixed-tap COM 链组合核心）：** `sipi-com` 新增 `com_chain_v1.rs`：将已移植的 R480 阶段核心（cursor P5-04i、DFE 抵消残差 PDF P5-04g、噪声 PDF 构建 P5-04h、组合噪声 P5-04e、COM/VEC/VEO 指标 P5-04c）串成产品固定-tap 链，typed 控制 + 逐阶段 checkpoint；equalizer search（P5-04t）、频域接收机噪声（P5-04o）、crosstalk（P5-04p）、TX-FFE 合成（P5-04j）仍为 caller-supplied 输入，不组合、不猜测；任何阶段拒绝或非有限控制 fail-closed。4 Rust 测试 + crosscheck（2 组合成双极脉冲：产品 vs 独立 Python 参考逐 checkpoint **2/2 matched_hash_bound**），证据 `docs/baselines/p5-06f-com-chain-crosscheck-evidence.v1.yaml`，见 [audit](docs/baselines/audits/2026-08-16-p5-06f-com-chain-core.md) **P5-06g 已完成（产品端 COM 链 oracle 检查点参数运行与 C4 容差对比）：** 以 `p5-06a` 证据 summary_content 中的 oracle 检查点参数（case_1/case_2 TXLE/DFE taps、sigma_N、spec_ber 等）驱动产品 `sipi-com` `run_com_chain_v1` 链路运行，并使用 P5-06e 绑定的 oracle 参考值与 C4 1% 相对容差对比引擎 `compare_metric_profile_v1` 执行 C4 profile 比对；独立 Python 参考逐 checkpoint 重算 2/2 matched_hash_bound，证据 `docs/baselines/p5-06g-oracle-chain-compare-crosscheck-evidence.v1.yaml`，见 [audit](docs/baselines/audits/2026-08-16-p5-06g-oracle-chain-compare.md)。。**非 product-vs-oracle compare**：真实 S4P/config 仍为外部资产、链尚未以 oracle 输入运行。
  ...
  - [x] **P5-06q** clean oracle observation：pinned clean Agent-COM 两次 fresh run 只观察到可用 COM scalar，ERL 为 non-finite 且 TD-ILN scalar 缺失；因此未生成三指标 reference，也未用 ICN/FOM 替代。strict checkpoint/no-alignment/0.1 dB policy 已冻结，但 P5-06 继续 blocked，见 [evidence](docs/baselines/p5-06q-agent-com-clean-oracle-observation.v1.yaml)。
  - [x] **P5-06s** additive original-13 reclassification：历史 P5-06k/q/r、P3C-03 与 result-surface 证据保持不变；pinned r4.80 original-13 的 immutable Rust `0e72786c` 已对两次 fresh MATLAB R2024b 和两次 fresh Rust final-surface replay 通过 13 workbooks、3 role channels、28 cases、303 scalar slots。finite scalar `1e-9`、MATLAB repeat `1e-12`、Inf 同符号同位置、Rust exact repeat、每 workbook 及总时间 Rust 不慢于 MATLAB 均已通过；见 `p5-06-original13-rust-matlab-r2024b-acceptance.v1.yaml`。**P5-06t 已完成（current candidate scalar/performance rebind）：** current Rust `83e9cb6c` 又以 2x fresh MATLAB R2024b 和 2x fresh Rust replay 重绑同一 13-workbook/28-case/303-slot final scalar matrix；MATLAB 固定 `-singleCompThread`、Rust 固定 `RAYON_NUM_THREADS=16` 的部署 wall-clock 下，逐 workbook 和总时间均不慢于 MATLAB（Rust worst total `149.7176113s`，MATLAB best total `1566.7051462s`；C2M index 2 为 `82.5354424s` vs `664.0101237s`）。这是明确多线程部署性能结论，不声称单线程算法比较；见 `p5-06-original13-rust-matlab-r2024b-current-acceptance.v1.yaml`。数组 checkpoint 在 instrumentation 等价与逐数组 tolerance 冻结前仍仅 diagnostic，故 P5-06 不关闭；S 参数 no-fit、channel one-final FD-to-TD impulse 不变。
  - [x] **P5-06u** current DFE source-visible checkpoint：current `83e9cb6c` 对 original-13 的 `output_args.DFE_taps` 完成 2x MATLAB R2024b 与 2x Rust fresh matrix。MATLAB 收据传输 exact IEEE-754 f64 bytes，避免 JSON 显示精度伪造 drift；25 个 eligible DFE checkpoint 的 MATLAB repeat `1e-12`、Rust exact repeat 与两对 MATLAB/Rust `1e-9` 都通过。固定 MATLAB 单线程/Rust 16 Rayon 部署 wall-clock 下逐 workbook 和总时间 Rust 不慢于 MATLAB，best MATLAB `1613.4832605s` / worst Rust `157.375059s`（至少 `10.252471203203806x`）；仅接受该 named checkpoint，不接受完整 array/result graph、不关闭 P5-06 或 release，见 `docs/baselines/p5-06-stage2b-dfe-checkpoint-acceptance.v1.yaml`。
  - [x] **P5-06v** current TXLE source-visible checkpoint：同一 current candidate 对 `output_args.TXLE_taps` 完成 2x MATLAB/Rust original-13 matrix；25 个 eligible checkpoint 的 MATLAB repeat `1e-12`、Rust exact repeat 与两对 cross `1e-9` 均通过。TXLE raw f64 receipt 保留为观测，shape/signed-zero 严格、数值按冻结 tolerance 比较；当前 best MATLAB `1612.3855592s` / worst Rust `156.0124371s`，固定部署至少 `10.34215302697813x`。该 scoped acceptance 不覆盖完整 result graph、warnings、TD-ILN、P5-06 main 或 release，见 `docs/baselines/p5-06-stage2a-txle-aggregate.v1.json`。
  - [x] **P5-06w** public-root scalar/performance rebind：候选 `b456e9d5` 通过真实 `sipi com run` 根 CLI 对 original-13 做 2x Rust 与 2x MATLAB R2024b 全矩阵重放。13 个 workbook、28 个 package case、303 个 final scalar slot 的 MATLAB repeat (`1e-12`)、Rust exact repeat 与双向 Rust/MATLAB (`1e-9`) 均通过；逐 workbook 和总 wall-clock 均满足 Rust 不慢于 MATLAB。Rust 最慢总计 `113.0148048s`，MATLAB 最快总计 `1568.9991522s`，即至少 `13.883129338467x`。此项同时绑定 root CLI 收据及三个输出工件的 hash/bytes，且保留 raw FD-to-TD impulse/no-fit 策略。它仅接受 final scalar 和部署性能，不接受完整数组、warnings、TD-ILN/ERL profile conformance、IEEE 认证或 release，故 P5-06 主项仍 open；见 `docs/baselines/p5-06-original13-root-matrix-v5-acceptance.v1.yaml`。
  - [x] **P5-06w1** original-13 source warning observation：以 immutable `agent-com@5272ffe` archive 在 MATLAB R2024b 对实际会发出 source warning 的五个 original-13 workbook 重放。line 9715 的三个 `MaxFreqTooLow` occurrence 全部与 Rust source-mapped warning 对齐；line 6337 anti-causal warning 只由 2.27e-17 至 4.55e-17 的 MATLAB 近零复数残差触发，Rust 对应输入为 signed-zero，故保持 `roundoff_sensitive_trace_mismatch` diagnostic，绝不伪装为 warning parity。该 observation 不改变 P5-06w 的 scalar/performance acceptance，也不关闭 complete warning catalog、P5-06 main 或 release；见 `docs/baselines/p5-06w-source-warning-observation.v1.yaml`。
  - [x] **P5-06w2** sparse TDILN PDF current checkpoint：`b456e9d5` 的 sparse PAM-PDF 优化在原 full-matrix 最慢 Rust TDILN workbook 10 上，从 fresh candidate/upstream archive 重放三项 package case。六个 named TDILN arrays 全部通过（time exact f64le，其余使用冻结的 `5e-12 + 1e-9 relative`）；production Rust 19.07 s 对 MATLAB source-core 182.32 s，即 `9.56x`，未用 S-parameter fit。它是 current optimization regression checkpoint，不能替代 2x 全 original-13 TDILN rebind 或关闭 P5-06；见 `docs/baselines/p5-06w-tdiln-sparse-pdf-checkpoint.v1.yaml`。
  - [x] **P5-06w3** full current TDILN rebind：`b456e9d5` 的 sparse PAM-PDF 后端在 immutable candidate/upstream archive 上完成 2x original-13 MATLAB R2024b/Rust 重放。13 个 workbook、28 个 TDILN package case 的六个 named array sidecar 全部通过（time exact f64le，其余 `5e-12 + 1e-9 relative`）；Rust semantic repeat exact，逐 workbook 与总耗时均不慢于 MATLAB。best MATLAB `1753.3723475s` / worst Rust `195.89563529996667s`，速度下限 `8.950543205391662x`，最弱单 workbook `5.785408685678222x`。该项验证优化没有牺牲已定义的中间数组一致性，也不声称 full result graph、complete warning catalog、public TDILN wire、release 或关闭 P5-06；见 `docs/baselines/p5-06-tdiln-array-current-acceptance.v1.yaml`。
  - [x] **P5-06w4** named normal-ERL TDR trace checkpoint：`b456e9d5` 对 original-13 ERL-only workbook 3/4/5 的两个差分端口完成 2x fresh MATLAB R2024b/Rust replay。临时 instrumented MATLAB copy 先与未 instrumented source 的 final scalar/warning semantics 对齐，再以 exact f64le hash/length 比较 `time_s`、`impedance_ohm`、raw `ptdr` 和 gated TDR；六个 port/workbook pair 全部 identity。直接 normal-ERL 测量的保守速度下限为 `72.43344098541151x`，且不使用 instrumented MATLAB timing；S 参数仍是 raw FD-to-TD impulse/no-fit。该项只覆盖一采样 named profile，不是一般 finite-ERL/full result graph/warning catalog/public wire/release，P5-06 仍 open；见 `docs/baselines/p5-06-normal-erl-trace-acceptance.v1.yaml`。
  - [x] **P5-06w5** public-root current-candidate scalar/performance rebind：修正 zero-reflection normal-TDR observation window 后，候选 `b9b195a1` 以 immutable archive 重跑 2x MATLAB R2024b 与 2x Rust public `sipi com run` original-13 matrix。13 个 workbook、28 个 package case、303 个 final scalar slots 的 MATLAB repeat (`1e-12`)、Rust exact repeat 与双向 Rust/MATLAB (`1e-9`) 均通过；逐 workbook 和总 wall-clock 均满足 Rust 不慢于 MATLAB。Rust 最慢总计 `114.2562865s`，MATLAB 最快总计 `1587.0214218s`，即至少 `13.8900140238673x`。它绑定 root CLI 收据和三个输出工件的 hash/bytes，保持 raw FD-to-TD impulse/no-fit；仅接受 final scalar 和部署性能，不接受完整数组、warnings、TD-ILN/ERL profile conformance、IEEE 认证或 release，P5-06 主项仍 open；见 `docs/baselines/p5-06-original13-root-matrix-v6-acceptance.v1.yaml`。
  - [x] **P5-06w6** original-13 scalar reference custody：在不改写历史 `com-r480-acceptance.v1` 三指标契约的前提下，将当前 user-selected original-13 final scalar surface 固化为严格 MATLAB reference record。v6 的 2x MATLAB R2024b 与 2x public-root Rust 报告分别产生 repeat-stable、domain-separated scalar bundle；记录绑定 13 workbook、3 role channel、28 package case、303 scalar slot、parameter-slot digest、MATLAB/Python/harness/upstream identity、无对齐 `1e-9` cross compare 与 Rust 不慢于 MATLAB的 v6 aggregate。它明确不接受 arrays、中间 checkpoint、warnings/full result graph、generalized COM、IEEE 认证或 release，故 P5-02/P5-06 主项仍 open；见 `docs/baselines/p5-06-original13-scalar-reference-custody.v2.yaml`。
  - [x] **P5-06w7** current TDILN named-array rebind：zero-reflection normal-TDR 修复后的当前候选 `b9b195a1` 已完成 2x original-13 TDILN replay。13 workbook 的 6 个 named vector（time、ILN/reference/fitted pulse、PDF axis/probability）全部通过既有 exact/tolerance 规则，Rust semantic repeat exact；默认 production Rust 二进制在每 workbook 与总计均不慢于 MATLAB R2024b source core。MATLAB best total `1758.8355904s`、Rust worst total `190.41138859995408s`，速度下限 `9.237029377981356x`；无 S-parameter fit。此项仍只是 named intermediate checkpoint，不接受 full result graph、complete warning catalog、public sidecar wire、IEEE 认证或 release，P5-06 主项继续 open；见 `docs/baselines/p5-06-tdiln-array-b9-acceptance.v1.yaml`。
  - [x] **P5-06w8** original-13 MATLAB execution surface 收口：将当前 13 workbook、28 package case 的 MATLAB R2024b 参数槽、runtime-consumed projection、303 final scalar、六个 TDILN named vector 和性能门整合为单一 owner-selected execution contract。每 workbook 与总耗时均要求 Rust 不慢于 MATLAB，最弱已绑定下限为 `9.237029377981356x`。line 6337 平台近零 warning 异常明确不计功能 mismatch；不声称完整结果图、warning catalog、泛化 COM、IEEE 或 release。见 [evidence](docs/baselines/p5-02-06-original13-execution-closure.v1.yaml)。
  - [x] **P5-07** 建立 synthetic/property/negative fixtures，避免 MATLAB oracle 成为唯一真相。
- [x] **P5-08** 接入 `sipi com run`、工件和 provenance。 **P5-08a 已完成（COM run-request admission preflight）：** COM run-request admission preflight core（sipi-com com_run_admission_v1.rs），产品 vs 独立 reference crosscheck 一致；完整 `sipi com run` conformance stack 仍 deferred（owner 小切片决策），证据 docs/baselines/p5-08a-com-run-admission-crosscheck-evidence.v1.yaml，见 [audit](docs/baselines/audits/2026-08-16-p5-08a-com-run-admission.md) **P5-08b 已完成（COM 运行执行与结果包头核心）：** `sipi-com` 新增 `com_run_execution_v1.rs`：`execute_com_run_v1` 将 `sipi.com.run-request.v1` 请求准入（P5-08a）、控制参数解析（P5-05f）与 COM 链路执行（P5-06f）连贯串联为 `sipi.com.run-result.v1` 结果包头，未准入请求输出包含 invalid_reason 的结构化失败包头；6 Rust 单测 + crosscheck（3 组请求用例：产品 vs 独立 Python 参考 **3/3 matched_hash_bound**），证据 `docs/baselines/p5-08b-com-run-execution-crosscheck-evidence.v1.yaml`，见 [audit](docs/baselines/audits/2026-08-16-p5-08b-com-run-execution.md)。主项现以 P5-08h 的 additive scoped replacement 收口为 public bounded `com.run-artifact` specified/non-oracle route；generalized COM conformance、external provenance/acceptance 与 release 仍在本范围之外。
  ...
  - [x] **P5-08e** bounded artifact execution composition：exact sealed `pulse.f64le` 经 P5-05g parameter ingestion、既有 COM execution、immutable `result.json` publication 与 08d bounded report 形成真实 library consumer；request 在解析前受 64 KiB 上限约束，scalar params 必须与 ingestion DTO 同名值一致。该 library prerequisite 不等于 public `com.run`，authoritative oracle、profile/metric acceptance、hostile-writer/external provenance 与 release仍 blocked，P5-08 保持 open。
   - [x] **P5-08f** specified/non-oracle artifact CLI：新增 `sipi com run-artifact --stdin --pulse-root`，仅接收显式 caller-owned 参数分区和 exact sealed pulse artifact，发布 immutable result；legacy `com.run` 仍 unavailable。该 route 不读取 workbook、不声称 Agent-COM parity/acceptance/hostile-writer safety，P5-08 保持 open，见 [evidence](docs/baselines/p5-08f-specified-com-artifact-route.v1.yaml)。
  - [x] **P5-08h** main scoped replacement：在不改写历史 P5-08g 的前提下，以 additive evidence supersede 当时的 `p5_08_main_item_closed: false` disposition；D6=A 的 public `com.run-artifact` 保持 available/specified/non-oracle，legacy `com.run` 保持 unavailable。预算、schema、threat model 与 non-claims 原样继承，P5-02/P5-06/P7 继续 blocked，见 [evidence](docs/baselines/p5-08h-com-run-main-scoped-closure.v1.yaml) 与 [audit](docs/baselines/audits/2026-08-21-p5-08h-com-run-main-scoped-closure.md)。
- [x] **P5-09** 明确“行为复刻、非 IEEE 官方认证”的实际 report 与 capability 文案；真实 bounded artifact result 现发出固定 capability label、claim、disclaimer 和 blocked statements，外部 parity、认证、acceptance、public CLI 与 release仍不成立。
  ...
  - [x] **P5-09b** candidate wording retirement：不改写 09a 历史合同，新增加法式 retirement/evidence 绑定 `ComRunArtifactExecutionReportV1`；public `com.run` 继续 unavailable，外部 COM parity 与认证保持 blocked。
...
- [ ] **P7-07** `release candidate` 仍被 strict license/NOTICE、fresh-machine、四域/跨域 certified profile 等门阻断，禁止创建或命名 `rc` tag。**P7-07a 已完成（non-release evidence anchor）：** 以 `record_commit=aae3f9f` 与 `candidate_source_commit=5edced5` 明确区分外部证据记录和被测源码，固定 P7-01/02/03/04/06b、policy、binary/locked-input hashes 与全局 blocker；它不是 release candidate 或 promotion。**P7-07b 已完成（Cargo 许可材料离线闭包拒绝观察）：** 从 preparation commit `601f561` 的 clean detached worktree 对 immutable `f498499` 重跑两份 Windows `sipi-cli` locked/offline build，均在 Cargo metadata 阶段以 `cargo_metadata_failed` fail-closed；hash-only evidence 仅记录该一致拒绝，不联网补包、不产生 package inventory，也不作 SPDX、NOTICE、法律兼容性、依赖/第一方批准或 release 判定。**P7-07c 已完成（actual-build closure v2，non-conclusive）：** 从 `02fffad` 的 clean detached worktree 仅解析 fixed `sipi-cli` Windows build 的 `compiler-artifact` 与 required `manifest_path`，两份 fresh locked/offline builds 一致观察到 86 个实际编译包（13 workspace、73 registry），且每个 registry archive 均与 immutable candidate lock checksum 精确匹配。v1拒绝保留为 historical metadata-superset evidence；v2只记录 declared metadata/material identity，不联网、不产 SPDX/NOTICE/法律决定，也不提升 dependency/first-party/release gate。**P7-07d 已完成（compiled metadata structural normalization，non-conclusive）：** 仅对07c的精确 hash-only report执行无网络、无重建的确定性结构归一；保留73条未解析 license string，13条 workspace record仍标为 conflicting-or-unbound，不将其改写为 SPDX、NOTICE、法律兼容性或 approval 结论。dependency snapshot、license/NOTICE/first-party approval、SBOM 与 release gate 继续 false/blocked。
  ...
  - [x] **P7-07g** current candidate `167b9714` 的 clean twin/layout-v2/composition/archive/isolated-install/performance 链已按既有工具 fresh 复跑；twin/layout/archive/install 均取得对应阶段观察，composition 仍 `incomplete` 且 promotion blocked。固定 TRAN performance observation 为 `observed_pending_owner_budget`，但 evaluation 因 policy 所绑定的历史 baseline raw custody 不可用而真实返回 `rejected/evidence_json_invalid`，没有生成 evaluation report；不重试、不替换 baseline、不提升 performance 或 release gate。仅以 hash/length/aggregate 记录外部证据，见 `docs/baselines/p7-current-candidate-chain-rejected-evaluation.v1.yaml` 与对应 audit。
...
replay；PB-01/PB-03 的 archive-built 双 replay 已记录在
[`pb-01-03-source-corpus.v1`](docs/baselines/pb-01-03-source-corpus.v1.yaml)。
