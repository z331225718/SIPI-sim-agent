...
- [ ] **P3B-04** 已完成 external-only same-source reference-bit provenance preflight：128 个 direct launch-current bits 的 Git-object/clean-checkout/polarity/UI/dt 与双重临时重放 hash 已审计；下一步仍须重放并 hash receive waveform，再运行 approved fixed receiver。当前仅 `required_pending_receiver_compare`，不宣称 receiver 或 stage parity。证据见 [2026-08-10-p3b-rfm-reference-bits-preflight.md](docs/baselines/audits/2026-08-10-p3b-rfm-reference-bits-preflight.md)。
