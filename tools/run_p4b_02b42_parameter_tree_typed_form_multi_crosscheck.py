# -*- coding: utf-8 -*-
"""P4B-02b42 AMI parameter tree multi-tree typed-form extraction cross-check (product vs independent ref).

Drives the product multi-tree typed-form runner (p4b_02b42_parameter_tree_typed_form_multi_runner)
over parenthesized AMI texts (one tree per text). An independent Python reference replicates the
tree build and the per-tree (name Type value) typed-form rules, then merges by name.
...
EVIDENCE_SCHEMA = "sipi.p4b-02b42-parameter-tree-typed-form-multi-crosscheck-evidence.v1"
...
def ref_multi(texts: list[str]) -> dict[str, Any]:
    if not texts:
        return {"valid": False, "multi_form_error": "EmptyTreeList"}
    ...
    for index, text in enumerate(texts):
        ...
        if error:
            return {"valid": False, "multi_form_error": f'TreeError({index}, {error})'}
        for name, parameter in tree_params.items():
            if name in parameters:
                return {"valid": False, "multi_form_error": f'DuplicateAcrossTrees("{name}")'}
            parameters[name] = parameter
        leaves_consumed[0] += tree_consumed[0]
    return {"valid": True, "leaves_consumed": leaves_consumed[0], "parameters": parameters}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b42_parameter_tree_typed_form_multi_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b42_parameter_tree_typed_form_multi_runner-*.exe"))[-1]

    cases = [
        {
            "label": "two_trees_merge",
            "texts": ["(root (gain Float 0.5))", "(root (steps Integer 7))"],
        },
        {
            "label": "duplicate_across",
            "texts": ["(root (gain Float 0.5))", "(root (gain Float 1.0))"],
        },
        {
            "label": "error_in_second",
            "texts": ["(root (gain Float 0.5))", "(root (steps Real 7))"],
        },
        {
            "label": "empty_list",
            "texts": [],
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b42-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_multi(case["texts"])
            ...
            if product.get("valid") != reference.get("valid"):
                ...
                prod_err = (product.get("multi_form_error") or product.get("parse_error"))
                ref_err = (reference.get("multi_form_error") or reference.get("parse_error"))
