# -*- coding: utf-8 -*-
"""Tests for the P4B-02b95 parameter profile canonical spelling check verifier."""
...
import verify_p4b_02b95_parameter_profile_canonical_spelling_check as GATE


class ParameterProfileCanonicalSpellingCheckTests(unittest.TestCase):
    ...
    def test_implementation_tokens_present(self) -> None:
        ...
        self.assertIn("pub fn check_parameter_profile_spellings_canonical_v1", source)
        self.assertIn(GATE.POLICY, source)

    def test_plan_row_present(self) -> None:
        self.assertIn("**P4B-02b95", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_rejects_evidence_mismatch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "EVIDENCE", tmp_path):
                with self.assertRaises(GATE.ParameterProfileCanonicalSpellingCheckError):
                    GATE.validate(ROOT)
