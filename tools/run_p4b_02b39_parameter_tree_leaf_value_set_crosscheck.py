# -*- coding: utf-8 -*-
"""P4B-02b39 AMI parameter tree leaf value set cross-check (product vs independent ref).

Drives the product parameter tree leaf value set runner (p4b_02b39_parameter_tree_leaf_value_set_runner)
over parenthesized AMI text inputs plus a path, a type map, and a new value token. An independent
Python reference replicates the tree build, path resolution, and typed value rules. Fail closed on
any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b39-parameter-tree-leaf-value-set-crosscheck-evidence.v1"
...
def ref_set_value(text: str, path: list[str], types: dict[str, str], value_token: str) -> dict[str, Any]:
    ...
    if not path:
        return {"valid": False, "set_error": "EmptyPath"}
    if path[0] != "root":
        return {"valid": False, "set_error": "RootMismatch"}
    segments = path[1:]

    def set_node(n: dict[str, Any], segs: list[str]) -> dict[str, Any] | str:
        if n["kind"] != "branch":
            return f'PathNotFound("{n["name"]}")'
        target = segs[0]
        if target not in n["children"]:
            return f'PathNotFound("{target}")'
        child = n["children"][target]
        if len(segs) == 1:
            if child["kind"] != "leaf":
                return f'TargetNotLeaf {{ name: "{child["name"]}" }}'
            leaf_name = child["name"]
            if leaf_name not in types:
                return f'MissingType("{leaf_name}")'
            type_token = types[leaf_name]
            rule_error = ref_value_rule(leaf_name, type_token, value_token)
            if rule_error == "InvalidName":
                return f'InvalidLeafName("{leaf_name}")'
            if rule_error is not None:
                return f'InvalidValue {{ leaf: "{leaf_name}", error: {rule_error} }}'
            new_child = {"kind": "leaf", "name": leaf_name, "value_tokens": [value_token]}
        else:
            new_child = set_node(child, segs[1:])
            if isinstance(new_child, str):
                return new_child
        new_children = dict(n["children"])
    ...
    new_root = set_node(node, segments)
    if isinstance(new_root, str):
        return {"valid": False, "set_error": new_root}
    return {"valid": True, "root_name": "root", "tree": ref_node_json(new_root)}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b39_parameter_tree_leaf_value_set_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b39_parameter_tree_leaf_value_set_runner-*.exe"))[-1]

    cases = [
        {
            "label": "set_float_value",
        ...
        },
        {
            ...
            "text": "(root (sub (deep Float 1.0) (other Integer 2)))",
            "path": ["root", "sub", "deep"],
            "types": {"deep": "Float", "other": "Integer"},
            "value": "2.5",
        },
        {
            "label": "missing_type",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b39-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_set_value(case["text"], case["path"], case["types"], case["value"])
            ...
            if product.get("valid") != reference.get("valid"):
                ...
                prod_err = (product.get("set_error") or product.get("build_error")
                            or product.get("parse_error"))
                ref_err = (reference.get("set_error") or reference.get("build_error")
