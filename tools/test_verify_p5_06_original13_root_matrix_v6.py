...
from tools.run_p5_06_original13_root_matrix_v6 import CANDIDATE, HARNESS, MATLAB_RECEIPT, MATLAB_RELEASE, ROOT_RECEIPT_SCHEMA, SCHEMA, TIMING_SCOPE, WORKER, file_identity, metric_values, read_root_receipt
from tools.verify_p5_06_original13_fresh_matrix import WORKBOOKS
from tools.verify_p5_06_original13_root_matrix_v6 import VerificationError, verify
...
def report(engine: str, nonce: str, duration: int = 100) -> dict:
    ...
    return {
        "schema": SCHEMA, "engine": engine, "run_id": f"original13-root-v6-{engine}-{nonce}", "nonce": nonce, "root_id": ("a" if engine == "matlab" else "b") + nonce[1:],
        ...
        "source": {
            ...
            "gate_tools": {"runner": file_identity(Path(__file__).with_name("run_p5_06_original13_root_matrix_v6.py")), "worker": file_identity(WORKER), "matlab_harness": file_identity(HARNESS)},
        },
...
class V5Tests(unittest.TestCase):
    ...
    def test_aggregate_rejects_one_slow_workbook(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = [sys.executable, str(Path(__file__).with_name("aggregate_p5_06_original13_root_matrix_v6.py")), "--matlab-report", str(root / "matlab-a.json"), "--matlab-report", str(root / "matlab-b.json"), "--rust-report", str(root / "rust-a.json"), "--rust-report", str(root / "rust-b.json"), "--output", str(root / "accepted.json")]
            ...
            blocked = [
                sys.executable, str(Path(__file__).with_name("aggregate_p5_06_original13_root_matrix_v6.py")),
