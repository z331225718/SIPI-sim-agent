# -*- coding: utf-8 -*-
"""Tests for the P5-02m warning detector verifier."""
...
import verify_p5_02m_warning_detector as GATE


class WarningDetectorTests(unittest.TestCase):
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-02m", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_detectors_trigger(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        for entry in evidence["entries"]:
            if entry["id"] == "anti_causal":
                self.assertTrue(entry["product"]["anti_causal"])
            if entry["id"] == "non_decay":
                self.assertTrue(entry["product"]["non_decay"])

    def test_rejects_contract_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["full_warning_contract"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.WarningDetectorError):
                    GATE.validate(ROOT)
