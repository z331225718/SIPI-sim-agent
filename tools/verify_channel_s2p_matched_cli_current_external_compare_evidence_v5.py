"""Verify the P3A-16 current-candidate CLI evidence record."""
...
SCHEMA = "sipi.channel.s2p-matched.cli-current-external-compare-evidence.v5"
EVIDENCE = ROOT / "docs" / "baselines" / "channel-s2p-matched-cli-current-external-compare-evidence.v5.yaml"
...
def _verify_historical_core_anchor(document: object, **_: object) -> dict[str, bool]:
    ...
    The v5 observer independently rebuilds the standard DFT kernel from the
    external Git object. Its fresh CLI result is the current evidence; v2 is
    retained only as a hash-bound historical lineage anchor.
    """
