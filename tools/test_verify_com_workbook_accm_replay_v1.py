...
def valid_report(run_id: str, nonce: str) -> dict[str, object]:
    ...
    return {"schema": "sipi.com.workbook-accm-replay-prep.v2", "run_id": run_id, "nonce": nonce, "candidate": {"commit": "0f38e3e796b2312f476c6c5a181dd83d2752bb43", "tree": "241d86cc898616aa656f50ac47536fe169098115", "archive": {"command": "git -c core.autocrlf=false archive --format=tar <candidate>", "exit": 0, "bytes": 44492800, "sha256": "9d627a57821db2f66e522eedacbefbca06a11eaa4e5b5f4c476b6ea95590cb01"}, "binary_pre": binary, "binary": copy.deepcopy(binary)}, "upstream": {**UPSTREAM, "source_inventory": inventory}, "fixtures": fixtures, "toolchain": {"pre": {"cargo": tool("cargo"), "rustc": tool("rustc"), "linker": tool("linker")}, "post": {"cargo": tool("cargo"), "rustc": tool("rustc"), "linker": tool("linker")}}, "build": {"command": "cargo build --manifest-path crates/sipi-agent-com-direct/Cargo.toml --bin sipi-com-direct-run --release --locked --offline", "exit": 3, "stdout_sha256": "e" * 64, "stderr_sha256": "f" * 64, "timeout_s": 900, "env_policy": "explicit_rustc_wrappers_cleared_offline_incremental_zero", "env_receipt": receipt}, "execution": {"runtime_timeout_s": 180, "source_inventory_before": {}, "source_inventory_after": {}, "source_inventory_equal": True}, "controls": {"ac_cm_rms_vectors": [[0.0, 0.0], [0.0, 0.001]], "source": "pinned workbook vector override", "no_sparam_fit": True, "channel_policy": "single_fd_to_td_impulse"}, "runs": runs, "parity": {"status": "blocked", "matched": False, "acceptance": False}, "non_claims": ["no S-parameter fit", "single FD-to-TD impulse", "no upstream numeric parity", "no global/product/release claim", "no final consumer proof while blocked"]}
...
class WorkbookAccmPreparationTests(unittest.TestCase):
    ...
    def test_build_env_rejects_inherited_flags(self) -> None:
        ...
        try:
            with self.assertRaises(RuntimeError): build_env(Path("rustc.exe"), Path("link.exe"))
        finally:
    ...
    def test_runner_does_not_overwrite_report(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(FileExistsError): run_one(Path(directory), Path(directory), Path("cargo.exe"), Path("rustc.exe"), Path("link.exe"), "1" * 64, output)
    ...
    def test_runner_state_machine_all_success_mixed_and_blocked(self) -> None:
        ...
        for mode in ("success", "mixed", "blocked", "invalid"):
            with self.subTest(mode=mode), tempfile.TemporaryDirectory() as directory, mock.patch.object(runner, "archive", fake_archive), mock.patch.object(runner, "safe_extract", fake_extract), mock.patch.object(runner, "candidate_inventory", lambda _: {}), mock.patch.object(runner, "pinned_blob", fake_blob), mock.patch.object(runner, "source_inventory", lambda _: {}), mock.patch.object(runner, "tool_identity", fake_tool), mock.patch.object(runner, "build_env", lambda *_: {}), mock.patch.object(runner, "bounded_run", fake_run):
                report = run_one(Path(directory), Path(directory), Path("cargo.exe"), Path("rustc.exe"), Path("link.exe"), "1" * 64, Path(directory) / "report.json")
