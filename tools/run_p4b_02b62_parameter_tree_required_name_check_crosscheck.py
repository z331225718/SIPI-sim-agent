# -*- coding: utf-8 -*-
"""P4B-02b62 parameter tree required-name check cross-check (product vs independent ref).

Drives the product required-name runner (p4b_02b62_parameter_tree_required_name_check_runner) over
parenthesized AMI text inputs plus a required-name set. An independent Python reference replicates
the tree build and the leaf presence check. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b62-parameter-tree-required-name-check-crosscheck-evidence.v1"
...
def ref_check(text: str, required: list[str]) -> dict[str, Any]:
    ...
    if not required:
        return {"valid": False, "required_error": "EmptyRequired"}
    ...
    collect(node)
    missing = sorted(name for name in required if name not in leaf_names)
    present = len(required) - len(missing)
    return {"valid": True, "required_count": len(required), "present": present,
            "missing": missing, "complete": not missing}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b62_parameter_tree_required_name_check_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b62_parameter_tree_required_name_check_runner-*.exe"))[-1]
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b62-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_check(case["text"], case["required"])

            matched = True
            if product.get("valid") != reference.get("valid"):
                matched = False
            elif product.get("valid"):
                if (product.get("required_count") != reference.get("required_count") or
                ...
                prod_err = (product.get("required_error") or product.get("build_error")
                            or product.get("parse_error"))
                ref_err = (reference.get("required_error") or reference.get("build_error")
