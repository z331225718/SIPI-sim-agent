# -*- coding: utf-8 -*-
"""P4A-03af typed IBIS series pin mapping block keywords cross-check (product vs independent ref).

Drives the product series pin mapping block runner (p4a_03af_series_pin_mapping_keywords_runner) over valid
and invalid series pin block inputs. An independent Python reference recomputes the lifting rules.
...
EVIDENCE_SCHEMA = "sipi.p4a-03af.series-pin-mapping-keywords-crosscheck-evidence.v1"
POLICY = "sipi.p4a-03af.series-pin-mapping-keywords-v1.typed-series-keywords"


def ref_lift_series_pin_mapping_block(pf: str, ps: str, mn: str, ftg: str | None,
    ...
    if not pf or not ps:
        return {"valid": False, "record_error": "EmptyPinName"}
    if not mn:
        return {"valid": False, "record_error": "EmptyModelName"}
    ...
    if pf == ps:
        return {"valid": False, "record_error": "IdenticalPins"}
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ibis", "--test", "p4a_03af_series_pin_mapping_keywords_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4a_03af_series_pin_mapping_keywords_runner-*.exe"))[-1]

    cases = [
        {
            "label": "full_series_pin_block",
            "pin_first": "P1",
            "pin_second": "P2",
        ...
        {
            "label": "minimal_series_pin_block",
            "pin_first": "P1",
            "pin_second": "P2",
            "model_name": "R_SERIES_50",
        ...
        {
            "label": "negative_rseries_threshold",
            "pin_first": "P1",
            "pin_second": "P2",
            "model_name": "R_SERIES_50",
            "function_table_group": None,
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4a-03af-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_lift_series_pin_mapping_block(
                case["pin_first"], case["pin_second"], case["model_name"], case["function_table_group"],
            ...
            if product.get("valid") != reference["valid"]:
                ...
                if (product.get("pin_first") != reference["pin_first"] or
                    ...
                    product.get("function_table_group") != reference["function_table_group"] or
