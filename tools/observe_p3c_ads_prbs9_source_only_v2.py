"""Observe the owner-confirmed finite-edge PRBS9 source projection in ADS."""
...
def _digest(domain: bytes, values: Iterable[float]) -> str:
    ...
    payload.extend(len(items).to_bytes(8, "big"))
...
def projected_loaded_differential_v2() -> list[float]:
    symbols = [-0.5 if bit == "0" else 0.5 for bit in runner.prbs9_period()]
    values: list[float] = []
    total_uis = runner.PERIOD_BITS * 3
    for ui in range(total_uis):
        current = symbols[ui % runner.PERIOD_BITS]
        if ui == 0:
            values.extend([current] * runner.SAMPLES_PER_UI)
            continue
        prior = symbols[(ui - 1) % runner.PERIOD_BITS]
        values.append(prior)
        values.extend([current] * (runner.SAMPLES_PER_UI - 1))
    if len(values) != runner.TOTAL_SAMPLES:
...
def read_canonical_payload(path: Path) -> tuple[list[float], list[float], str]:
    ...
    for index in range(runner.TOTAL_SAMPLES):
        ...
        expected_time = index * runner.SAMPLE_INTERVAL_SECONDS
        tolerance = 8 * math.ulp(max(abs(time), abs(expected_time)))
        if not all(math.isfinite(value) for value in (time, loaded, common)) or abs(time - expected_time) > tolerance:
...
def summary(expected: list[float], observed: list[float], common_mode: list[float], indexes: range | Iterable[int]) -> dict[str, object]:
    ...
    common_values = [common_mode[index] for index in selected]
    count = len(selected)
    return {
        "sample_count": count,
        "nrmse_bits": _hex_bits(math.sqrt(residual_energy / reference_energy)),
        "residual_sha256": _digest(b"sipi.p3c.ads-source-only-v2.residual.v1\0", residual),
        "max_absolute_residual_bits": _hex_bits(abs(max_residual)),
        "max_absolute_residual_first_index": selected[max_index],
        "common_mode_sha256": _digest(b"sipi.p3c.ads-source-only-v2.common-mode.v1\0", common_values),
        "common_mode_max_absolute_bits": _hex_bits(max(abs(value) for value in common_values)),
    }
...
def run_once(root: Path, index: int) -> dict[str, object]:
    ...
    try:
        ...
        expected = projected_loaded_differential_v2()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        work = Path(tempfile.mkdtemp(prefix="sipi-p3c-source-only-v2-", dir=output_root))
        try:
                ...
                json.dumps(
                    {
                        ...
                        "ads_inclusive_samples": runner.ADS_INCLUSIVE_SAMPLES,
                        "canonical_half_open_samples": runner.TOTAL_SAMPLES,
