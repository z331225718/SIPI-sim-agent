"""Aggregate two R2024b and two public-root Rust original-13 reports."""
...
try:
    from .run_p5_06_original13_extended_scalar_matrix_v1 import digest
    from .run_p5_06_original13_root_matrix_v7 import METRICS, file_identity
    from .verify_p5_06_original13_root_matrix_v7 import verify
except ImportError:
    from run_p5_06_original13_extended_scalar_matrix_v1 import digest
    from run_p5_06_original13_root_matrix_v7 import METRICS, file_identity
    from verify_p5_06_original13_root_matrix_v7 import verify
...
def comparable(left: dict, right: dict) -> bool:
    ...
    left_source.pop("root_cli_binary")
    left_source.pop("config_validator_binary")
    right_source.pop("root_cli_binary")
    right_source.pop("config_validator_binary")
    return (
...
def main() -> int:
    ...
    aggregate = {
        "schema": "sipi.p5-06.original13-root-aggregate.v7",
        ...
        "source": {
            key: value
            for key, value in reports[0]["source"].items()
            if key not in {"root_cli_binary", "config_validator_binary"}
        },
        "host_fingerprint": reports[0]["host_fingerprint"], "timing_clock": reports[0]["timing_clock"], "timing_scope": reports[0]["timing_scope"],
        "matrix": {"workbook_count": 13, "case_count": 28, "scalar_slot_count": sum(len(case) for record in matlab_a["records"] for case in record["metrics"])},
        ...
        "gate_tools": {"aggregate": file_identity(Path(__file__)), "verifier": file_identity(Path(__file__).with_name("verify_p5_06_original13_root_matrix_v7.py"))},
