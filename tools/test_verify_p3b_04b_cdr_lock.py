# -*- coding: utf-8 -*-
"""Tests for the P3B-04b CDR lock semantics verifier."""
...
import verify_p3b_04b_cdr_lock as GATE


class CdrLockTests(unittest.TestCase):
    ...
    def test_implementation_tokens_present(self) -> None:
        ...
        self.assertIn("pub fn track_cdr_lock_v1", source)
        self.assertIn(GATE.POLICY, source)

    def test_plan_row_present(self) -> None:
        self.assertIn("**P3B-04b", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_rejects_evidence_mismatch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "EVIDENCE", tmp_path):
                with self.assertRaises(GATE.CdrLockError):
                    GATE.validate(ROOT)
