"""Strict verifier for one public-root original-13 v6 replay."""
...
try:
    ...
    from .run_p5_06_original13_root_matrix_v6 import CANDIDATE, HARNESS, MATLAB_RECEIPT, MATLAB_RELEASE, RAYON_THREADS, ROOT_BINARY, ROOT_MANIFEST, ROOT_RECEIPT_SCHEMA, SCHEMA, TIMING_SCOPE, WORKER, file_identity
...
def verify(report: object) -> bool:
    ...
    require(report["run_id"] == f"original13-root-v6-{report['engine']}-{report['nonce']}", "run id")
    ...
    require(source["gate_tools"] == {"runner": file_identity(Path(__file__).with_name("run_p5_06_original13_root_matrix_v6.py")), "worker": file_identity(WORKER), "matlab_harness": file_identity(HARNESS)}, "gate tools")
