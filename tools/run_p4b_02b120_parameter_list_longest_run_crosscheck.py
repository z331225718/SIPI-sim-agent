# -*- coding: utf-8 -*-
"""P4B-02b120 parameter list longest-run cross-check (product vs independent ref).

Drives the product longest-run runner (p4b_02b120_parameter_list_longest_run_runner) over
validated (name, type, value) inputs. An independent Python reference replicates the run rule:
List values trim items and report the (item, length) of the longest consecutive equal run, ties
to the earliest; non-List values fail closed. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b120-parameter-list-longest-run-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b120.parameter-list-longest-run-v1.longest-consecutive-run"


def ref_longest_run(type_token: str, value_token: str) -> dict[str, Any]:
    if type_token != "List":
    ...
    return {"valid": True, "item": best_item, "length": best_len}
...
def main() -> int:
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b120-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_longest_run(case["type"], case["value"])
