# -*- coding: utf-8 -*-
"""Tests for the P3C-02f bathtub opening-width verifier."""
...
import verify_p3c_02f_bathtub as GATE
...
class BathtubTests(unittest.TestCase):
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P3C-02f", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_widths_exact(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        for entry in evidence["entries"]:
            if entry["product_width"] is not None:
                self.assertIsNotNone(entry["reference_width"])
                self.assertAlmostEqual(entry["product_width"], entry["reference_width"], places=9)

    def test_rejects_curve_fit_admission(self) -> None:
        import copy
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.BathtubError):
                    GATE.validate(ROOT)
