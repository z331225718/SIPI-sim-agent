# -*- coding: utf-8 -*-
"""P4A-03ay typed IBIS series pin Model Selector group threshold parameters cross-check (product vs independent ref).

Drives the product series pin Model Selector group threshold runner (p4a_03ay_series_pin_table_selector_thresholds_group_runner) over valid
and invalid series pin selector group threshold inputs. An independent Python reference recomputes the lifting rules.
...
EVIDENCE_SCHEMA = "sipi.p4a-03ay.series-pin-table-selector-thresholds-group-crosscheck-evidence.v1"
POLICY = "sipi.p4a-03ay.series-pin-table-selector-thresholds-group-v1.typed-selector-thresholds-group"


def ref_lift_series_pin_selector_group_threshold(pf: str, ps: str, ms: str, gn: str | None, vthresh: float | None, rseries: float | None,
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ibis", "--test", "p4a_03ay_series_pin_table_selector_thresholds_group_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4a_03ay_series_pin_table_selector_thresholds_group_runner-*.exe"))[-1]

    cases = [
        {
            "label": "full_selector_group_threshold_record",
            "pin_first": "P1",
            "pin_second": "P2",
        ...
        {
            "label": "minimal_selector_group_threshold_record",
            "pin_first": "P1",
            "pin_second": "P2",
            "model_selector_name": "SEL_SERIES_RES",
            "group_name": None,
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4a-03ay-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_lift_series_pin_selector_group_threshold(
                case["pin_first"], case["pin_second"], case["model_selector_name"], case["group_name"],
