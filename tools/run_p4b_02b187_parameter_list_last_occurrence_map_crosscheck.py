# -*- coding: utf-8 -*-
"""P4B-02b187 parameter list last-occurrence map cross-check (product vs ref).

Drives the product last-occurrence-map runner over one validated list value.
An independent Python reference replicates the rule: the list trims items and
builds a mapping from each distinct item to its last 0-based occurrence index
(raw byte equality); non-List values fail closed.  Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b187-parameter-list-last-occurrence-map-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b187.parameter-list-last-occurrence-map-v1.last-occurrence-map"


def ref_last_occurrence_map(type_token: str, value_token: str) -> dict:
    if type_token != "List":
    ...
    for idx, item in enumerate(items):
        mapping[item] = idx
    return {"valid": True, "last_occurrence_map": mapping}
...
def main() -> int:
    ...
    os.environ["SIPI_P4B_RUNNER"] = "p4b_02b187_parameter_list_last_occurrence_map_runner"
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b187-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_last_occurrence_map(case["type"], case["value"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("last_occurrence_map") == reference.get("last_occurrence_map")
