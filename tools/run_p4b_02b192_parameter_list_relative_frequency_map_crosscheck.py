# -*- coding: utf-8 -*-
"""P4B-02b192 parameter list relative-frequency map cross-check (product vs ref).

Drives the product relative-frequency-map runner over one validated list value.
An independent Python reference replicates the rule: the list trims items and
builds a mapping from each distinct item to its proportion count / total (raw
byte equality); each proportion is computed and formatted with 12 decimals;
...
EVIDENCE_SCHEMA = "sipi.p4b-02b192-parameter-list-relative-frequency-map-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b192.parameter-list-relative-frequency-map-v1.relative-frequency-map"


def ref_relative_frequency_map(type_token: str, value_token: str) -> dict:
    if type_token != "List":
    ...
    mapping = {item: f"{count / total:.12f}" for item, count in counts.items()}
    return {"valid": True, "relative_frequency_map": mapping}
...
def main() -> int:
    ...
    os.environ["SIPI_P4B_RUNNER"] = "p4b_02b192_parameter_list_relative_frequency_map_runner"
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b192-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_relative_frequency_map(case["type"], case["value"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("relative_frequency_map") == reference.get("relative_frequency_map")
