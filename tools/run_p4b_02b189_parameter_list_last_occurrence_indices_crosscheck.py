# -*- coding: utf-8 -*-
"""P4B-02b189 parameter list last-occurrence indices cross-check (product vs ref).

Drives the product last-occurrence-indices runner over one validated list value.
An independent Python reference replicates the rule: the list trims items and
collects the (item, last 0-based occurrence index) pairs in first-occurrence
order (raw byte equality); non-List values fail closed.  Fail closed on any
mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b189-parameter-list-last-occurrence-indices-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b189.parameter-list-last-occurrence-indices-v1.last-occurrence-index"


def ref_last_occurrence_indices(type_token: str, value_token: str) -> dict:
    if type_token != "List":
    ...
    for idx, item in enumerate(items):
        last_index[item] = idx
    pairs = [[item, last_index[item]] for item in dict.fromkeys(items)]
    return {"valid": True, "last_occurrence_indices": pairs}
...
def main() -> int:
    ...
    os.environ["SIPI_P4B_RUNNER"] = "p4b_02b189_parameter_list_last_occurrence_indices_runner"
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b189-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_last_occurrence_indices(case["type"], case["value"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("last_occurrence_indices") == reference.get("last_occurrence_indices")
