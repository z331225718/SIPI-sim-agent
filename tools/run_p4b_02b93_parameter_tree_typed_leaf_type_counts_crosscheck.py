# -*- coding: utf-8 -*-
"""P4B-02b93 parameter tree typed-leaf type counts cross-check (product vs independent ref).

Drives the product typed-leaf type counts runner (p4b_02b93_parameter_tree_typed_leaf_type_counts_runner)
over parenthesized AMI text inputs. An independent Python reference replicates the tree build and
the per-type counting: two-token leaves with a known type token count under their declared type,
everything else is skipped. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b93-parameter-tree-typed-leaf-type-counts-crosscheck-evidence.v1"
...
def ref_counts(text: str) -> dict[str, Any]:
    ...
    walk(node)
    return {"valid": True, "total_typed": total_typed,
            "float_count": counts["Float"], "integer_count": counts["Integer"],
            "boolean_count": counts["Boolean"], "string_count": counts["String"],
            "list_count": counts["List"], "skipped": skipped}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b93_parameter_tree_typed_leaf_type_counts_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b93_parameter_tree_typed_leaf_type_counts_runner-*.exe"))[-1]

    cases = [
        {"label": "typed_forms",
         "text": "(root (gain Float 0.5) (steps Integer 7) (on Boolean True) (mode String Linear) (channels List \"(a, b)\"))"},
        {"label": "mixed_skipped",
         "text": "(root (gain Float 0.5) (plain 5) (bad Nope 1.0))"},
        {"label": "nested",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b93-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_counts(case["text"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("total_typed") == reference.get("total_typed")
                       ...
                       and product.get("skipped") == reference.get("skipped"))
            if not matched:
                ok_all = False
