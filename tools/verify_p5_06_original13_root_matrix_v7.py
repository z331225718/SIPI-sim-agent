"""Strict verifier for one public-root extended-scalar original-13 v7 replay."""
...
try:
    from .run_p5_06_original13_extended_scalar_matrix_v1 import CHANNELS, CONFIG_PATHS, METRICS, UPSTREAM, digest
    from .run_p5_06_original13_root_matrix_v7 import CANDIDATE, HARNESS, MATLAB_RECEIPT, MATLAB_RELEASE, RAYON_THREADS, ROOT_BINARY, ROOT_MANIFEST, ROOT_RECEIPT_SCHEMA, SCHEMA, TIMING_SCOPE, WORKER, file_identity
...
def verify(report: object) -> bool:
    ...
    require(report["run_id"] == f"original13-root-v7-{report['engine']}-{report['nonce']}", "run id")
    ...
    require(source["gate_tools"] == {"runner": file_identity(Path(__file__).with_name("run_p5_06_original13_root_matrix_v7.py")), "worker": file_identity(WORKER), "matlab_harness": file_identity(HARNESS)}, "gate tools")
    ...
    # The base accepted surface has 303 slots. Source-visible optional
    # scalars vary by workbook, so preserve that lower bound rather than
    # pretending every profile emits all 18 names.
