# -*- coding: utf-8 -*-
"""P4A-03bb typed IBIS series pin group switch cross-check (product vs independent ref).

Drives the product series pin group switch runner (p4a_03bb_series_pin_table_group_switch_runner) over valid
and invalid series pin group switch inputs. An independent Python reference recomputes the lifting rules.
...
EVIDENCE = ROOT / "docs" / "baselines" / "p4a-03bb-series-pin-table-group-switch-crosscheck-evidence.v1.yaml"
EVIDENCE_SCHEMA = "sipi.p4a-03bb.series-pin-table-group-switch-crosscheck-evidence.v1"
POLICY = "sipi.p4a-03bb.series-pin-table-group-switch-v1.typed-group-switch"


def ref_lift_series_pin_group_switch(gn: str, on_g: str, off_g: str, ftg: str | None) -> dict[str, Any]:
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ibis", "--test", "p4a_03bb_series_pin_table_group_switch_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4a_03bb_series_pin_table_group_switch_runner-*.exe"))[-1]

    cases = [
        {
            "label": "full_group_switch_record",
        ...
        {
            "label": "minimal_group_switch_record",
            "group_name": "SERIES_GRP1",
            "on_group_name": "GROUP_ON_1",
            "off_group_name": "GROUP_OFF_1",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4a-03bb-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_lift_series_pin_group_switch(case["group_name"], case["on_group_name"], case["off_group_name"], case["function_table_group"])
