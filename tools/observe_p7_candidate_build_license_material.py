...
SCHEMA = "sipi.p7-candidate-build-license-material-observation.v1"
...
def _cargo_metadata(cargo: str, source_root: Path) -> dict[str, Any]:
    completed = _run(
        [cargo, "metadata", "--locked", "--offline", "--format-version", "1"],
        cwd=source_root,
        failure="cargo_metadata_failed",
    )
    try:
        value = json.loads(completed.stdout)
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ObservationError("cargo_metadata_invalid") from error
    if not isinstance(value, dict) or not isinstance(value.get("packages"), list):
        raise ObservationError("cargo_metadata_invalid")
    return value


def _cargo_build_artifacts(cargo: str, source_root: Path, target_root: Path) -> tuple[set[str], str]:
    completed = _run(
        [
    ...
    return package_ids, sha256(completed.stderr)
...
def _workspace_material(source_root: Path, package: dict[str, Any], boundary: dict[str, Any]) -> dict[str, Any]:
...
def _one_build(cargo: str, source_root: Path, target_root: Path, cache_root: Path) -> dict[str, Any]:
    ...
    metadata = _cargo_metadata(cargo, source_root)
    metadata_packages = metadata["packages"]
    by_id = {package.get("id"): package for package in metadata_packages if isinstance(package, dict) and isinstance(package.get("id"), str)}
    package_ids, stderr_sha = _cargo_build_artifacts(cargo, source_root, target_root)
    boundary = _candidate_boundary(source_root)
    materials: list[dict[str, Any]] = []
    for package_id in sorted(package_ids):
        package = by_id.get(package_id)
        if not isinstance(package, dict) or not all(isinstance(package.get(key), str) for key in ("name", "version", "manifest_path")):
            raise ObservationError("compiler_artifact_metadata_mismatch")
        source = package.get("source")
        if source is None:
            materials.append(_workspace_material(source_root, package, boundary))
        elif isinstance(source, str) and source.startswith("registry+"):
            registry_package = {"name": package["name"], "version": package["version"], "source": source}
...
def observe(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    if not _external(output) or output.exists() or not _external(scratch_root) or not repository.is_dir() or not cache_root.is_dir():
    ...
    try:
        ...
        for label in ("a", "b"):
            ...
            try:
                result = _one_build(arguments.cargo, source_root, target_root, cache_root)
