...
def verify_document(document: object) -> dict[str, object]:
    ...
    if document.get("status") != "specified_pending_owner_finite_edge_boundary_projection_confirmation":
        raise VerificationError("status_invalid")
    if document.get("authority") != {"actor": "user", "decision_ref": "user-continuation-2026-08-14", "scope": "selected_candidate_source_projection_v2_only_not_metric_or_release_policy"}:
    ...
    if confirmation != {"phase_0_prior_phase_1_current": False, "sample_zero_first_symbol_current": False, "product_amplitude_remains_plus_or_minus_one_volt": False, "v2_selected_policy_v1_historical_only": False}:
        raise VerificationError("owner_confirmation_gate")
    false_keys = {"finite_edge_boundary_projection_v2_specified", "selected_source_projection_v2_implemented", "product_projection_v2_source_strobe_match_observed", "current_v2_candidate_waveform_evaluated", "selected_highloss_waveform_only_profile_accepted", "source_explains_full_channel_residual", "causal_fir_admitted", "accepted_receiver", "acceptance_ready", "release_ledger_promoted"}
    admission = document.get("admission")
    if not isinstance(admission, dict) or set(admission) != false_keys or any(admission[key] is not False for key in false_keys):
    ...
    return {"valid": True, "owner_confirmation_pending": True, "v2_implemented": False, "release_admitted": False}
