# -*- coding: utf-8 -*-
"""P4B-02b185 parameter list is-balanced cross-check (product vs ref).

Drives the product is-balanced runner (p4b_02b185_parameter_list_is_balanced_runner)
over one validated list value.  An independent Python reference replicates the rule:
the list trims items and checks whether every maximal run of equal adjacent items
has the same length (raw byte equality); non-List values fail closed.  Fail closed
on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b185-parameter-list-is-balanced-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b185.parameter-list-is-balanced-v1.is-balanced"


def ref_is_balanced(type_token: str, value_token: str) -> dict:
    if type_token != "List":
    ...
    for idx in range(1, len(items)):
        if items[idx] == items[idx - 1]:
            ...
            current_len = 1
    run_lengths.append(current_len)
    balanced = len(set(run_lengths)) == 1
    return {"valid": True, "is_balanced": balanced}
...
def main() -> int:
    ...
    os.environ["SIPI_P4B_RUNNER"] = "p4b_02b185_parameter_list_is_balanced_runner"

    cases = [
        {"label": "balanced_two_runs", "name": "param", "type": "List", "value": "(a, a, b, b)"},
        {"label": "unbalanced_runs", "name": "param", "type": "List", "value": "(a, a, b)"},
        {"label": "all_equal_single_run", "name": "param", "type": "List", "value": "(x, x, x)"},
        {"label": "non_list", "name": "gain", "type": "Float", "value": "0.5"},
    ]

    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b185-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_is_balanced(case["type"], case["value"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("is_balanced") == reference.get("is_balanced")
