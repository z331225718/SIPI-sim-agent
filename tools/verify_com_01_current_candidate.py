...
DEFAULT_MANIFEST = ROOT / "docs/baselines/com-01-current-candidate-replay.v1.yaml"
REPORT_SCHEMA = "sipi.com-01.current-candidate-replay.v1"
AGGREGATE_SCHEMA = "sipi.com-01.current-candidate-replay.aggregate.v1"
...
def _validate_scenarios(value: Any) -> None:
    ...
    for item in value:
        ...
        if item["comparison"] == "values_equal_fingerprint_drift":
            require(item["fingerprint_match"] is False and item["difference_keys"] == [], "fingerprint drift not isolated")
        else:
...
def _validate_report(report: dict[str, Any], label: str, manifest: dict[str, Any]) -> None:
    ...
    require(report.get("blockers") == ["materialized_fingerprint_drift"], f"{label} blocker drift")
...
def verify(manifest_path: Path = DEFAULT_MANIFEST, *, repo_root: Path = ROOT) -> dict[str, Any]:
    ...
    require(document.get("schema") == "sipi.com-01.current-candidate-replay.v1", "manifest schema drift")
    ...
    require(harness.get("runner") == "tools/run_com_01_current_candidate.py" and is_sha(harness.get("runner_sha256")), "manifest runner drift")
    require(harness.get("aggregate_runner") == "tools/aggregate_com_01_current_candidate.py" and is_sha(harness.get("aggregate_runner_sha256")), "manifest aggregate runner drift")
    require(harness.get("semantic_runner") == "tools/run_com_01_direct_oracle.py" and is_sha(harness.get("semantic_runner_sha256")), "manifest semantic runner drift")
    ...
    require(aggregate.get("blockers") == ["materialized_fingerprint_drift"], "aggregate blocker drift")
