"""Tests for the P5-04s candidate-evaluation stage verifier."""
...
import verify_p5_04s_candidate_eval as GATE
...
class CandidateEvalTests(unittest.TestCase):
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-04s", GATE.PLAN.read_text(encoding="utf-8"))

    def test_evidence_matches(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        self.assertGreaterEqual(len(evidence["entries"]), 3)
    ...
    def test_rejects_candidate_admission_off(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.CandidateEvalError):
                    GATE.validate(ROOT)
