"""Mutation coverage for the v8 additive upstream ledger gate."""
...
from tools import verify_upstream_integration_ledger_v8 as gate

EXPECTED_VERIFIER_SHA256 = "e38be2c7a43bbd9c3d6fd460c1ae3860f861eb6000b4e58494f2e0317880e7a1"

class UpstreamIntegrationLedgerV8Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_current_snapshot_is_open_and_inherits_v7(self) -> None:
        self.assertEqual(gate.validate(self.document), {"valid": True, "rows": 15, "release_ready": 0})
        self.assertEqual(self.document["successor"]["predecessor_sha256"], gate.V7_SHA)
        self.assertEqual(self.document["summary"]["release_ready"], 0)
        verifier_path = gate.ROOT / "tools/verify_upstream_integration_ledger_v8.py"
    ...
    def test_as04_pb_inventory_and_com_public_claims_are_rejected(self) -> None:
        ...
        mutated = copy.deepcopy(self.document)
        mutated["rows"][2]["current_observation"]["notice"]["sha256"] = "0" * 64
