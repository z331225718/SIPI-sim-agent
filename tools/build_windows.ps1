...
.PARAMETER Channel
Include the local PyBERT native candidate in the same sipi.exe. Not a release.
#>
[CmdletBinding()]
param([switch]$Check, [switch]$Channel)
...
try {
    ...
    $requiredFiles = @('Cargo.toml', 'Cargo.lock', 'rust-toolchain.toml', 'crates/sipi-cli/Cargo.toml')
    if ($Channel) {
        $requiredFiles += @('crates/sipi-pybert-direct/Cargo.toml', 'examples/channel-native/metallic-line.json')
    }
    ...
    try {
        $buildArgs = @('build', '--locked', '--release', '-p', 'sipi-cli', '--bin', 'sipi', '--target', $target, '--target-dir', $targetDir)
        if ($Channel) { $buildArgs += @('--features', 'pybert-direct-integration') }
        & $cargoPath @buildArgs
        ...
        if ($Channel) {
            $smoke = Join-Path $root ("results/channel-native-smoke-" + [Guid]::NewGuid().ToString('N'))
            New-Item -ItemType Directory -Path $smoke -ErrorAction Stop | Out-Null
            $request = Join-Path $smoke 'request.json'
            $output = Join-Path $smoke 'run'
            $init = & $exe channel init $request | ConvertFrom-Json
            if ($LASTEXITCODE -ne 0 -or $init.status -ne 'ok') { throw 'Native channel init smoke failed.' }
            $simulation = & $exe channel simulate $request --output-dir $output | ConvertFrom-Json
            if ($LASTEXITCODE -ne 0 -or $simulation.status -ne 'ok' -or
                $simulation.result.backend -ne 'in_process_sipi_pybert_direct' -or
                $simulation.result.waveform_samples -ne 8192 -or
                $simulation.result.acceptance -ne $false -or
                -not (Test-Path -LiteralPath (Join-Path $output 'receipt.json'))) {
                throw 'In-process PyBERT channel simulation smoke failed.'
            }
            Write-Host "Native channel report: $(Join-Path $output 'report.html')"
            Write-Host 'Scope: opt-in local PyBERT candidate, no Python or second repository; not ADS parity or an accepted release.'
        } else {
            Write-Host 'Scope: default native development CLI. Use -Channel for the local PyBERT candidate; neither build is an accepted release.'
        }
    } finally {
        Pop-Location
    }
