...
SCHEMA = "sipi.com-01.current-candidate-replay.v2"
...
SEMANTIC_RUNNER_RELATIVE = Path("tools/run_com_01_direct_oracle_v2.py")
...
COM01_SCENARIO_CONTRACT = {
    ...
    "xlsx_materialized_json_default": ("passed", None, "materialized"),
...
COM01_MATERIALIZED_OBSERVATION = (
    ("6d1757e35ad2bfbb731c1281d2c427a6c518fa6132061d4357123fc6890eafa8", "d6963c122533d58273e4ebf3224bade1dc38b040f740c1cd92d4c9727e4483dc"),
    ("6d1757e35ad2bfbb731c1281d2c427a6c518fa6132061d4357123fc6890eafa8", "d6963c122533d58273e4ebf3224bade1dc38b040f740c1cd92d4c9727e4483dc"),
)
...
def _run_scenarios(module: ModuleType, upstream_root: Path, candidate_root: Path, binary: Path, fixture: Path, scratch: Path, tools: dict[str, Path], timeout: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    _derive_fixture(
        _safe_file(ROOT, SEMANTIC_RUNNER_RELATIVE),
        upstream_root,
        fixture,
        scratch,
        tools,
    ...
    for scenario in corpus["scenarios"]:
        ...
        scenarios.append(
            module.compare_scenario(
                scenario, scenario_fixture, upstream_root, binary, tools["uv"]
            )
        )
    receipts = []
...
def _validate_com01_scenarios(scenarios: Any) -> None:
    ...
    for scenario in scenarios:
        ...
        if summary_kind == "materialized":
            if item["fingerprint_match"] is not True:
                raise ValueError("COM-01 materialized fingerprint parity contract changed")
...
def _validate_com01_report(report: Any) -> None:
    ...
    _validate_harness_source(harness["source"], {"tools/run_com_01_current_candidate_v2.py", "tools/aggregate_com_01_current_candidate_v2.py", "tools/verify_com_01_current_candidate_v2.py", "tools/test_verify_com_01_current_candidate_v2.py", "tools/run_com_01_direct_oracle_v2.py"})
    if harness["runner"] != "tools/run_com_01_current_candidate_v2.py" or harness["semantic_runner"] != "tools/run_com_01_direct_oracle_v2.py" or harness["corpus"] != CORPUS_RELATIVE.as_posix() or not _hex(harness["scenario_set_sha256"]) or harness["archive_only_inputs"] is not True or type(harness["report_policy"]) is not str or harness["shared_helper_schema"] != SHARED_HELPER_SCHEMA or type(harness["scenario_count"]) is not int or harness["scenario_count"] != 14 or type(harness["timeout_seconds"]) is not int or harness["timeout_seconds"] <= 0:
    ...
    if type(item["outcomes"]) is not dict or any(type(value) is not int for value in item["outcomes"].values()) or item["outcomes"] != {"error_code_match": 7, "passed": 7} or item["values_aligned"] is not True:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    harness_source = _harness_receipt(
        ...
        args.harness_commit,
        (
            Path("tools/run_com_01_current_candidate_v2.py"),
            Path("tools/aggregate_com_01_current_candidate_v2.py"),
            Path("tools/verify_com_01_current_candidate_v2.py"),
            Path("tools/test_verify_com_01_current_candidate_v2.py"),
            Path("tools/run_com_01_direct_oracle_v2.py"),
        ),
    )
    with tempfile.TemporaryDirectory(prefix="sipi-com-01-current-") as directory:
        ...
        candidate_inventory = _inventory(candidate_root, (CRATE_RELATIVE, CORPUS_RELATIVE))
        ...
        if candidate_inventory != _git_inventory(candidate_repo, candidate_commit, (CRATE_RELATIVE, CORPUS_RELATIVE)):
        ...
        semantic_runner = _safe_file(candidate_repo, SEMANTIC_RUNNER_RELATIVE)
        ...
        harness = {
            "runner": "tools/run_com_01_current_candidate_v2.py",
            "semantic_runner": "tools/run_com_01_direct_oracle_v2.py",
        ...
        post_candidate_inventory = _inventory(candidate_root, (CRATE_RELATIVE, CORPUS_RELATIVE))
    ...
    mismatches = [item["id"] for item in scenarios if item.get("comparison") not in {"passed", "error_code_match"}]
