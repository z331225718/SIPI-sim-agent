"""Mutation coverage for the v9 additive upstream ledger gate."""
...
from tools import verify_upstream_integration_ledger_v9 as gate

EXPECTED_VERIFIER_SHA256 = "ff7cc3722fd07fbf57c7531f440c91e129ffc1b3ec93daac414973be96f273cd"

class UpstreamIntegrationLedgerV9Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_current_snapshot_is_open_and_inherits_v8(self) -> None:
        self.assertEqual(gate.validate(self.document), {"valid": True, "rows": 15, "release_ready": 0})
        self.assertEqual(self.document["successor"]["predecessor_sha256"], gate.V8_SHA)
        self.assertEqual(self.document["summary"]["release_ready"], 0)
        verifier_path = gate.ROOT / "tools/verify_upstream_integration_ledger_v9.py"
    ...
    def test_as04_pb_inventory_and_com_public_claims_are_rejected(self) -> None:
        ...
        mutated = copy.deepcopy(self.document)
        mutated["rows"][2]["current_observation"]["manifest"]["sha256"] = "0" * 64
        self.reject(mutated, "current_observation:AS-03")
        mutated = copy.deepcopy(self.document)
        mutated["rows"][7]["current_observation"]["run_02"]["sha256"] = "0" * 64
        self.reject(mutated, "current_observation:PB-02")
        mutated = copy.deepcopy(self.document)
    ...
    def test_source_predecessor_audit_and_harness_drift_are_rejected(self) -> None:
        ...
        mutated = copy.deepcopy(self.document)
        mutated["harness"]["verifier"]["path"] = "tools/verify_upstream_integration_ledger_v7.py"
    ...
    def test_exact_evidence_and_current_observation_maps_are_locked(self) -> None:
        for index, row in enumerate(self.document["rows"]):
            ...
            if "current_observation" in row:
                mutated = copy.deepcopy(self.document)
                target = mutated["rows"][index]["current_observation"]
                if row["id"] in {"AS-03", "PB-02"}:
                    target[next(iter(target))]["sha256"] = "0" * 64
                else:
                    target["sha256"] = "0" * 64
    ...
    def test_plan_binding_and_coordinated_drift_are_rejected(self) -> None:
        ...
        mutated = copy.deepcopy(self.document)
        mutated["rows"][7]["non_claims"].remove("single_fixed_fixture_numeric_observation")
