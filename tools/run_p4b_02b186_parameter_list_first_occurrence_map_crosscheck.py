# -*- coding: utf-8 -*-
"""P4B-02b186 parameter list first-occurrence map cross-check (product vs ref).

Drives the product first-occurrence-map runner over one validated list value.
An independent Python reference replicates the rule: the list trims items and
builds a mapping from each distinct item to its first 0-based occurrence index
...
EVIDENCE_SCHEMA = "sipi.p4b-02b186-parameter-list-first-occurrence-map-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b186.parameter-list-first-occurrence-map-v1.first-occurrence-map"


def ref_first_occurrence_map(type_token: str, value_token: str) -> dict:
    if type_token != "List":
    ...
    for idx, item in enumerate(items):
        if item not in mapping:
            mapping[item] = idx
    return {"valid": True, "first_occurrence_map": mapping}
...
def main() -> int:
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b186-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_first_occurrence_map(case["type"], case["value"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("first_occurrence_map") == reference.get("first_occurrence_map")
