...
def parse_report(path: Path) -> dict[str, Any]:
    ...
    expected_keys = {"manifest_sha256", "record_count", "impulse_sha256", "negative_energy_fraction_bits", "negative_peak_fraction_bits", "signed_peak_index"}
    for run in value["fresh_runs"]:
        ...
        if not isinstance(run["signed_peak_index"], int) or isinstance(run["signed_peak_index"], bool) or not -49_056 <= run["signed_peak_index"] <= 49_056: raise ObservationError("runner_peak_index_invalid")
