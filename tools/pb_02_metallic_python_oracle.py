"""Pinned PyBERT oracle for the PB-02 metallic CTLE payload contract."""
...
def _failure_artifact(output: Path, error: ValueError) -> None:
    if "The number of derivatives at boundaries does not match" not in str(error):
        raise error
    ...
    document = {
        ...
        "diagnostics": {
            "backend": "python",
            "failure_code": "pinned_channel_cubic_interp1d_two_point_boundary",
            "failure_stage": "channel",
        },
...
def run(config: Path, output: Path) -> dict[str, Any]:
    ...
    try:
        ...
        _failure_artifact(output, error)
        raise
    ...
    source_fields: dict[str, tuple[str, Any]] = {
        ...
        "rx_filter_impulse_v_per_v": ("legacy_ctle_h", getattr(pybert, "ctle_h", None)),
        "legacy_channel_frequency_hz": ("legacy_frequency_hz", getattr(pybert, "f", None)),
        "legacy_channel_raw_re": ("legacy_channel_raw_re", np.real(getattr(pybert, "chnl_H_raw", np.array([])))),
        "legacy_channel_raw_im": ("legacy_channel_raw_im", np.imag(getattr(pybert, "chnl_H_raw", np.array([])))),
        "legacy_channel_terminated_re": ("legacy_channel_terminated_re", np.real(getattr(pybert, "chnl_H", np.array([])))),
        "legacy_channel_terminated_im": ("legacy_channel_terminated_im", np.imag(getattr(pybert, "chnl_H", np.array([])))),
        "legacy_channel_trimmed_re": ("legacy_channel_trimmed_re", np.real(getattr(pybert, "chnl_trimmed_H", np.array([])))),
        "legacy_channel_trimmed_im": ("legacy_channel_trimmed_im", np.imag(getattr(pybert, "chnl_trimmed_H", np.array([])))),
        "legacy_stage_ctle_re": ("legacy_stage_ctle_re", np.real(getattr(pybert, "ctle_H", np.array([])))),
        "legacy_stage_ctle_im": ("legacy_stage_ctle_im", np.imag(getattr(pybert, "ctle_H", np.array([])))),
        "legacy_stage_ctle_out_re": ("legacy_stage_ctle_out_re", np.real(getattr(pybert, "ctle_out_H", np.array([])))),
        "legacy_stage_ctle_out_im": ("legacy_stage_ctle_out_im", np.imag(getattr(pybert, "ctle_out_H", np.array([])))),
    ...
    for candidate_name, (source_name, value) in source_fields.items():
        ...
        bindings[candidate_name] = {"source_field": source_name, "shape": list(array.shape), "dtype": "float64"}
    output.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(output / "arrays.npz", **arrays)
    selected_metadata = {key: metadata.get(key) for key in ("ber", "fom", "eye_width_ps", "eye_height_mv", "jitter", "effective_randomness", "eye_shape") if key in metadata}
    document = {
        "schema": "pybert.python-oracle-result.v1", "backend": "python", "source_command": "PythonSimulationBackend",
        "source_fields": bindings, "metadata": _jsonable(selected_metadata), "diagnostics": _jsonable(result.diagnostics), "arrays_file": "arrays.npz",
    }
    (output / "meta.json").write_text(json.dumps(document, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return document
