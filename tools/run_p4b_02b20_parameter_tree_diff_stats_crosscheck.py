# -*- coding: utf-8 -*-
"""P4B-02b20 AMI parameter tree diff stats cross-check (product vs independent ref).

Drives the product parameter tree diff stats runner (p4b_02b20_parameter_tree_diff_stats_runner) over parenthesized
AMI text inputs. An independent Python reference recomputes tree diff summary statistics.
...
EVIDENCE_SCHEMA = "sipi.p4b-02b20-parameter-tree-diff-stats-crosscheck-evidence.v1"
...
def ref_diff_stats_trees(text_left: str, text_right: str) -> dict[str, Any]:
    ...
    return {
        ...
        "missing_nodes_count": missing,
        "extra_nodes_count": extra,
        "kind_mismatches_count": kind_mm,
        "value_mismatches_count": val_mm,
        "is_identical": len(diffs) == 0,
    }


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b20_parameter_tree_diff_stats_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b20_parameter_tree_diff_stats_runner-*.exe"))[-1]
    ...
    cases = [
        ...
        {
            "label": "mixed_diff_stats",
            "text_left": "(root (node_a 1) (node_b 2) (val Float 0.5))",
            "text_right": "(root (node_a 1) (node_c 3) (val Float 0.9))",
        },
        {
            "label": "root_mismatch",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b20-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_diff_stats_trees(case["text_left"], case["text_right"])
            ...
            if product.get("valid") != reference["valid"]:
                ...
                if (product.get("total_diffs") != reference["total_diffs"] or
                    product.get("missing_nodes_count") != reference["missing_nodes_count"] or
                    product.get("extra_nodes_count") != reference["extra_nodes_count"] or
                    product.get("kind_mismatches_count") != reference["kind_mismatches_count"] or
                    product.get("value_mismatches_count") != reference["value_mismatches_count"] or
                    product.get("is_identical") != reference["is_identical"]):
                    matched = False
            else:
                prod_err = product.get("stats_error") or product.get("diff_error") or product.get("build_error") or product.get("parse_error")
                ref_err = reference.get("stats_error") or reference.get("diff_error") or reference.get("build_error")
