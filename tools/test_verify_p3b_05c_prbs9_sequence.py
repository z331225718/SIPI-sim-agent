# -*- coding: utf-8 -*-
"""Tests for the P3B-05c deterministic PRBS9 seq strong verifier."""
...
import verify_p3b_05c_prbs9_sequence as GATE


class Prbs9SequenceTests(unittest.TestCase):
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P3B-05c", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_evidence_matched(self) -> None:
        ...
        self.assertEqual(evidence["seed"], 1)
        self.assertTrue(evidence["entries"][0]["matched"])

    def test_rejects_injection_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["injection_position"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.Prbs9Error):
                    GATE.validate(ROOT)
