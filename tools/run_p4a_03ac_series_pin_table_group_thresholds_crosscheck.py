# -*- coding: utf-8 -*-
"""P4A-03ac typed IBIS series pin group threshold table cross-check (product vs independent ref).

Drives the product series pin group threshold table runner (p4a_03ac_series_pin_table_group_thresholds_runner) over valid
and invalid series pin group threshold inputs. An independent Python reference recomputes the lifting rules.
...
EVIDENCE_SCHEMA = "sipi.p4a-03ac.series-pin-table-group-thresholds-crosscheck-evidence.v1"
POLICY = "sipi.p4a-03ac.series-pin-table-group-thresholds-v1.typed-group-thresholds"


def ref_lift_series_pin_group_threshold(gn: str, vthresh: float | None, rseries: float | None,
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ibis", "--test", "p4a_03ac_series_pin_table_group_thresholds_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4a_03ac_series_pin_table_group_thresholds_runner-*.exe"))[-1]

    cases = [
        {
            "label": "full_group_threshold_record",
        ...
        {
            "label": "minimal_group_threshold_record",
            "group_name": "SERIES_GRP1",
            "vthreshold_v": None,
            "rseries_ohm": None,
            "cseries_farad": None,
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4a-03ac-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_lift_series_pin_group_threshold(
