# -*- coding: utf-8 -*-
"""P4B-02b60 parameter tree token frequencies cross-check (product vs independent ref).

Drives the product token frequency runner (p4b_02b60_parameter_tree_token_frequencies_runner)
over parenthesized AMI text inputs. An independent Python reference replicates the tree build and
the value-token counting. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b60-parameter-tree-token-frequencies-crosscheck-evidence.v1"
...
def ref_frequencies(text: str) -> dict[str, Any]:
    ...
    walk(node)
    return {"valid": True, "total_tokens": total, "frequencies": frequencies}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b60_parameter_tree_token_frequencies_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b60_parameter_tree_token_frequencies_runner-*.exe"))[-1]
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b60-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_frequencies(case["text"])

            matched = True
            if product.get("valid") != reference.get("valid"):
                matched = False
            elif product.get("valid"):
                if (product.get("total_tokens") != reference.get("total_tokens") or
                    product.get("frequencies") != reference.get("frequencies")):
                    matched = False
            else:
