...
SCHEMA = "sipi.agent-spice-as-01-bound-replay.v3"
EXPECTED_CANDIDATE_COMMIT = "9914a23dc747d94405f2fed9227538ae7aea8629"
EXPECTED_CANDIDATE_TREE = "94a25c6254fdec3802b4946ec777ce65f22fdc0c"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_root.mkdir()
    # Keep source and target paths byte-for-byte stable across fresh replays.
    # Rust remapping alone did not prevent PE differences when Cargo saw a
    # different extracted source path on Windows.
    stable_root = Path(tempfile.gettempdir()) / f"sipi-as01-bound-stable-{candidate_commit[:12]}"
    if stable_root.exists():
        shutil.rmtree(stable_root)
    stable_root.mkdir(parents=True)
    candidate_root = stable_root / "candidate"
    ...
    if not args.keep_work:
        shutil.rmtree(stable_root, ignore_errors=True)
