# -*- coding: utf-8 -*-
"""P4B-02b63 parameter tree distinct leaf names cross-check (product vs independent ref).

Drives the product distinct leaf names runner (p4b_02b63_parameter_tree_distinct_leaf_names_runner)
over parenthesized AMI text inputs. An independent Python reference replicates the tree build and
the sorted distinct name enumeration. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b63-parameter-tree-distinct-leaf-names-crosscheck-evidence.v1"
...
def ref_names(text: str) -> dict[str, Any]:
    ...
    return {"valid": True, "distinct_count": len(sorted_names), "names": sorted_names}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b63_parameter_tree_distinct_leaf_names_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b63_parameter_tree_distinct_leaf_names_runner-*.exe"))[-1]

    cases = [
        {
            "label": "cross_depth_dedup",
            "text": "(root (sub (gain 1)) (gain 2) (steps 3))",
        },
        {
            "label": "sorted_names",
            "text": "(root (beta 1) (alpha 2))",
        },
        {
            "label": "root_leaf",
            "text": "(root)",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b63-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_names(case["text"])

            matched = True
            if product.get("valid") != reference.get("valid"):
                matched = False
            elif product.get("valid"):
                if (product.get("distinct_count") != reference.get("distinct_count") or
                    product.get("names") != reference.get("names")):
                    matched = False
            else:
