# -*- coding: utf-8 -*-
"""P4B-02b91 parameter tree leaf value validity cross-check (product vs independent ref).

Drives the product value validity runner (p4b_02b91_parameter_tree_leaf_value_validity_runner)
over parenthesized AMI text inputs. An independent Python reference replicates the tree build and
the declared-type value validation: typed-form leaves validate their value token via the 02b1 rules
and report every invalid leaf with path, type, value, and error key; non-typed leaves are skipped.
Fail closed on any mismatch.
"""

from __future__ import annotations

import json
import re
import subprocess
import sys
import tempfile
from pathlib import Path
from typing import Any

import yaml

ROOT = Path(__file__).resolve().parents[1]
CARGO = Path.home() / ".cargo" / "bin" / "cargo.exe"
EVIDENCE = ROOT / "docs" / "baselines" / "p4b-02b91-parameter-tree-leaf-value-validity-crosscheck-evidence.v1.yaml"
EVIDENCE_SCHEMA = "sipi.p4b-02b91-parameter-tree-leaf-value-validity-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b91.parameter-tree-leaf-value-validity-v1.declared-type-value-validity"

KNOWN_TYPES = ("Float", "Integer", "Boolean", "String", "List")


def is_valid_identifier(name: str) -> bool:
    trimmed = name.strip()
    return bool(trimmed) and trimmed.isascii() and all(c.isalnum() or c in "_-." for c in trimmed)


def ref_build_node(node: Any) -> tuple[dict[str, Any] | None, str | None]:
    if not isinstance(node, list) or not node:
        return None, "InvalidNodeName"
    head = node[0]
    if not isinstance(head, str) or not is_valid_identifier(head):
        return None, "InvalidNodeName"
    name = head.strip()

    has_sublist = any(isinstance(item, list) for item in node[1:])
    if not has_sublist:
        tokens = []
        for item in node[1:]:
            if not isinstance(item, str) or not is_valid_identifier(item):
                return None, "InvalidNodeName"
            tokens.append(item)
        return {"kind": "leaf", "name": name, "value_tokens": tokens}, None
    else:
        children = {}
        for item in node[1:]:
            if isinstance(item, list):
                child, err = ref_build_node(item)
                if err:
                    return None, err
                cname = child["name"]
                if cname in children:
                    return None, f'DuplicateChild("{cname}")'
                children[cname] = child
        sorted_children = {k: children[k] for k in sorted(children.keys())}
        return {"kind": "branch", "name": name, "children": sorted_children}, None


def ref_list_items(value_token: str):
    if not (value_token.startswith("(") and value_token.endswith(")") and len(value_token) >= 2):
        return None
    inner = value_token[1:-1]
    if not inner:
        return None
    items = [item.strip() for item in inner.split(",")]
    if any(not item for item in items):
        return None
    return items


def ref_value_error(type_token: str, value_token: str) -> str | None:
    """Return the error key when the value violates its declared type rule."""
    if type_token == "Float":
        try:
            v = float(value_token)
        except ValueError:
            return "InvalidFloat"
        import math
        return None if math.isfinite(v) else "InvalidFloat"
    if type_token == "Integer":
        try:
            int(value_token)
            return None
        except ValueError:
            return "InvalidInteger"
    if type_token == "Boolean":
        return None if value_token in ("True", "False") else "InvalidBoolean"
    if type_token == "String":
        return None if bool(value_token) else "EmptyStringValue"
    if type_token == "List":
        return None if ref_list_items(value_token) is not None else "InvalidList"
    return None


def ref_validity(text: str) -> dict[str, Any]:
    text_clean = re.sub(r'\|.*', '', text)
    tokens = re.findall(r'\(|\)|"[^"]*"|[^\s()]+', text_clean)
    if not tokens:
        return {"valid": False, "parse_error": "EmptyDocument"}

    def parse_list(idx: int) -> tuple[list[Any], int]:
        items: list[Any] = []
        idx += 1
        while idx < len(tokens):
            if tokens[idx] == ')':
                return items, idx + 1
            elif tokens[idx] == '(':
                sub, idx = parse_list(idx)
                items.append(sub)
            else:
                items.append(tokens[idx])
                idx += 1
        return items, idx

    if tokens[0] != '(':
        return {"valid": False, "parse_error": "EmptyDocument"}
    ast, _ = parse_list(0)
    node, err = ref_build_node(ast)
    if err:
        return {"valid": False, "build_error": err}

    leaves_total = 0
    typed_leaves = 0
    valid_leaves = 0
    invalid = []

    def walk(n: dict[str, Any], prefix: str) -> None:
        nonlocal leaves_total, typed_leaves, valid_leaves
        current = n["name"] if not prefix else prefix + "." + n["name"]
        if n["kind"] == "leaf":
            leaves_total += 1
            tokens_list = n["value_tokens"]
            if len(tokens_list) == 2 and tokens_list[0] in KNOWN_TYPES:
                typed_leaves += 1
                error = ref_value_error(tokens_list[0], tokens_list[1])
                if error is None:
                    valid_leaves += 1
                else:
                    invalid.append({
                        "path": current,
                        "type_token": tokens_list[0],
                        "value_token": tokens_list[1],
                        "error": error,
                    })
            return
        for child in n["children"].values():
            walk(child, current)

    walk(node, "")
    return {"valid": True, "leaves_total": leaves_total, "typed_leaves": typed_leaves,
            "valid_leaves": valid_leaves, "invalid": invalid}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b91_parameter_tree_leaf_value_validity_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b91_parameter_tree_leaf_value_validity_runner-*.exe"))[-1]

    cases = [
        {"label": "all_valid",
         "text": "(root (gain Float 0.5) (steps Integer 7) (on Boolean True))"},
        {"label": "mixed_invalid",
         "text": "(root (gain Float abc) (steps Integer 7) (on Boolean nope))"},
        {"label": "nested",
         "text": "(root (sub (steps Integer abc)) (gain Float 0.5))"},
        {"label": "non_typed_skipped",
         "text": "(root (plain 5) (raw x y z) (bad Nope 1.0))"},
    ]

    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b91-") as tmp:
        work = Path(tmp)
        for case in cases:
            input_path = work / (case["label"] + "_in.json")
            input_path.write_text(json.dumps(case), encoding="utf-8")
            report_path = work / (case["label"] + "_rep.json")
            run = subprocess.run([str(runner), "--input", str(input_path), "--report", str(report_path)],
                                 capture_output=True, text=True, encoding="utf-8", errors="replace")
            if run.returncode != 0:
                raise SystemExit(f"runner failed for {case['label']}: " + run.stderr)
            product = json.loads(report_path.read_text(encoding="utf-8"))
            reference = ref_validity(case["text"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("leaves_total") == reference.get("leaves_total")
                       and product.get("typed_leaves") == reference.get("typed_leaves")
                       and product.get("valid_leaves") == reference.get("valid_leaves")
                       and product.get("invalid") == reference.get("invalid"))
            if not matched:
                ok_all = False

            entries.append({
                "label": case["label"],
                "matched": matched,
                "product": product,
                "reference": reference,
            })

    evidence = {
        "schema": EVIDENCE_SCHEMA,
        "status": "matched_hash_bound" if ok_all else "mis_match",
        "policy": POLICY,
        "matched_count": sum(1 for e in entries if e["matched"]),
        "case_count": len(entries),
        "entries": entries,
    }
    EVIDENCE.parent.mkdir(parents=True, exist_ok=True)
    EVIDENCE.write_text(yaml.safe_dump(evidence, sort_keys=False), encoding="utf-8")
    print(json.dumps({"schema": EVIDENCE_SCHEMA, "status": evidence["status"],
                      "matched_count": evidence["matched_count"], "case_count": evidence["case_count"]}, indent=2))
    return 0 if ok_all else 1


if __name__ == "__main__":
    raise SystemExit(main())
