"""Verify the COM-04 public API semantic replay contract and evidence shape."""
...
DEFAULT_MANIFEST = ROOT / "docs" / "baselines" / "com-04-direct-port.v1.yaml"
...
def verify(manifest_path: Path = DEFAULT_MANIFEST) -> dict[str, Any]:
    ...
    require(document.get("schema") == "sipi.com-04-direct-port.v1", "schema drift")
    require(document.get("status") == "direct_port_scoped_semantic_replay_open", "status overclaim")
    scope = document.get("scope")
    require(scope == {
        "work_item": "COM-04",
        "upstream_public_entrypoint": "load_config_run_com_write_artifacts",
        "target": "lane_local_direct_port_only",
        "product_capability_promoted": False,
        "global_migration_row_closed": False,
        "source_map": "crates/sipi-agent-com-direct/SOURCE-MAP-COM-04.md",
    }, "scope drift")
    require(document.get("audit") == "docs/baselines/audits/2026-08-23-com-04-direct-semantic-replay.md", "audit binding drift")
    ...
    require(isinstance(paths, list) and len(paths) == 26, "reachable source path set drift")
    ...
    require(contract.get("artifacts") == ["result.json", "report.html", "diagnostics.json"], "artifact set drift")
    require("metrics" in contract.get("semantic_result_payload", []), "metric payload semantics missing")
    ...
    require(isinstance(oracle, dict) and oracle.get("replay_count") == 2, "two replay policy drift")
    require(oracle.get("scenario_count") == 10, "portable replay scenario count drift")
    for claim in ("no_product_capability_promotion", "no_global_migration_row_close", "no_numeric_upstream_parity_claim", "no_workbook_or_matlab_source_vendoring", "no_matlab_engine_or_proprietary_golden_parity"):
    ...
    if evidence is not None:
        ...
        for item in reports:
            ...
            require(report.get("mode") == "com-04", "report mode drift")
