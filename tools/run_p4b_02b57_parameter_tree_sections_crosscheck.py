# -*- coding: utf-8 -*-
"""P4B-02b57 parameter tree sections listing cross-check (product vs independent ref).

Drives the product sections runner (p4b_02b57_parameter_tree_sections_runner) over parenthesized
AMI text inputs. An independent Python reference replicates the tree build and the top-level
section listing. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b57-parameter-tree-sections-crosscheck-evidence.v1"
...
def ref_sections(text: str) -> dict[str, Any]:
    ...
    if node["kind"] != "branch":
        return {"valid": False, "sections_error": "RootNotBranch"}
    sections = [
        {"name": child["name"], "kind": "Branch" if child["kind"] == "branch" else "Leaf"}
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b57_parameter_tree_sections_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b57_parameter_tree_sections_runner-*.exe"))[-1]

    cases = [
        {
            "label": "mixed_sections",
            "text": "(root (sub (deep Float 1.0)) (gain Float 0.5))",
        },
        {
            "label": "leaf_only_sections",
        ...
        },
        {
            ...
            "text": "(root (a (b 1)) (c (d 2)))",
        },
    ]

    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b57-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_sections(case["text"])

            matched = True
            if product.get("valid") != reference.get("valid"):
                matched = False
            elif product.get("valid"):
                if product.get("sections") != reference.get("sections"):
                    matched = False
            else:
                prod_err = (product.get("sections_error") or product.get("build_error")
                            or product.get("parse_error"))
                ref_err = (reference.get("sections_error") or reference.get("build_error")
