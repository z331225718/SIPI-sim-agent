# -*- coding: utf-8 -*-
"""P4B-02b11 AMI parameter tree diff cross-check (product vs independent ref).

Drives the product parameter tree diff runner (p4b_02b11_parameter_tree_diff_runner) over parenthesized
AMI text inputs. An independent Python reference recomputes tree structural diffs.
...
EVIDENCE_SCHEMA = "sipi.p4b-02b11-parameter-tree-diff-crosscheck-evidence.v1"
...
def ref_diff_trees(text_left: str, text_right: str) -> dict[str, Any]:
    ...
    return {"valid": True, "diff_count": len(diffs), "diffs": diffs}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b11_parameter_tree_diff_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b11_parameter_tree_diff_runner-*.exe"))[-1]
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b11-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_diff_trees(case["text_left"], case["text_right"])

            matched = True
            if product.get("valid") != reference["valid"]:
                matched = False
            elif product.get("valid"):
                if (product.get("diff_count") != reference["diff_count"] or
                    product.get("diffs") != reference["diffs"]):
                    matched = False
            else:
                if str(product.get("diff_error")) != str(reference.get("diff_error")):
                    matched = False
