...
from tools.verify_p5_06_original13_extended_scalar_matrix_v1 import Error, WORKBOOKS, verify
from tools.run_p5_06_original13_extended_scalar_matrix_v1 import ADAPTER, CANDIDATE, CHANNELS, CONFIG_PATHS, PROJECTION_SOURCES, UPSTREAM, inventory, materialize, runtime_projection, worker_environment
...
class Tests(unittest.TestCase):
    ...
    def test_final_surface_matlab_harness_is_scalar_only_and_staged(self):
        harness=Path(__file__).with_name("sipi_com_extended_scalar_oracle_v1.m").read_text(encoding="ascii")
        runner=Path(__file__).with_name("run_p5_06_original13_extended_scalar_matrix_v1.py").read_text(encoding="utf-8")
        ...
        self.assertIn("results = com_ieee8023_480(config_path, num_fext, num_next, varargin{:});",harness)
        self.assertIn("extended_scalar_metrics(results)",harness)
        self.assertNotIn("matlab_oracle.mat",harness)
        self.assertNotIn("save(",harness)
        self.assertIn("sipi_com_extended_scalar_oracle_v1",runner)
    ...
    def full(self, engine, marker):
        ...
        for index,count in enumerate(counts):
            ...
            for case_index in range(count): metrics.append({name:float(index+case_index) for name in (("ERL",) if count==1 else tuple(__import__('tools.run_p5_06_original13_extended_scalar_matrix_v1',fromlist=['METRICS']).METRICS))})
    ...
    def aggregate(self, reports):
        ...
        output=root/"aggregate.json";command=[sys.executable,str(Path(__file__).with_name("aggregate_p5_06_original13_extended_scalar_matrix_v1.py"))]
