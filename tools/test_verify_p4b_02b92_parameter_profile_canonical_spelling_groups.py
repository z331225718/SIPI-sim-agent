# -*- coding: utf-8 -*-
"""Tests for the P4B-02b92 parameter profile canonical spelling groups verifier."""
...
import verify_p4b_02b92_parameter_profile_canonical_spelling_groups as GATE


class ParameterProfileCanonicalSpellingGroupsTests(unittest.TestCase):
    ...
    def test_implementation_tokens_present(self) -> None:
        ...
        self.assertIn("pub fn group_parameter_profile_names_by_canonical_spelling_v1", source)
        self.assertIn(GATE.POLICY, source)

    def test_plan_row_present(self) -> None:
        self.assertIn("**P4B-02b92", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_rejects_evidence_mismatch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "EVIDENCE", tmp_path):
                with self.assertRaises(GATE.ParameterProfileCanonicalSpellingGroupsError):
                    GATE.validate(ROOT)
