...
from aggregate_com_01_current_candidate import VerificationError as AggregateError, aggregate
...
def prepared_report() -> dict:
    ...
    for scenario_id, (comparison, category, kind) in COM01_SCENARIO_CONTRACT.items():
        if kind == "structured":
            ...
            exit_code, output, fingerprint = 0, "materialized_json", False
        else:
    ...
    files = ["tools/run_com_01_current_candidate.py", "tools/aggregate_com_01_current_candidate.py", "tools/verify_com_01_current_candidate.py", "tools/test_verify_com_01_current_candidate.py"]
    ...
    return {"schema": "sipi.com-01.current-candidate-replay.v1", "status": "scoped_current_candidate_observed", "work_item": "COM-01", "leaf": "config-validate", "run_id": "d" * 64, "nonce": "e" * 64, "candidate": {"commit": CANDIDATE_COMMIT, "tree": CANDIDATE_TREE, "archive": {"bytes": 1, "sha256": "f" * 64, "command": "git archive", "path_redacted": True}, "source_mode": "git_archive_at_immutable_commit", "inventory": {"file_count": 1, "total_bytes": 1, "sha256": "0" * 64}, "cargo_lock_sha256": "1" * 64, "source_date_epoch": "1", "build": build}, "upstream": {"commit": UPSTREAM_COMMIT, "tree": UPSTREAM_TREE, "archive": {"bytes": 1, "sha256": "2" * 64, "command": "git archive", "path_redacted": True}, "source_mode": "git_archive_at_immutable_commit", "inventory": {"file_count": 1, "total_bytes": 1, "sha256": "3" * 64}, "source": source, "runtime": runtime, "uv_lock": {"bytes": 1, "sha256": "4" * 64}}, "toolchain": {"pre": tools, "post": copy.deepcopy(tools), "stable": True}, "harness": {"runner": files[0], "semantic_runner": "tools/run_com_01_direct_oracle.py", "corpus": "docs/baselines/com-01-direct-port-corpus.v1.json", "scenario_count": 14, "scenario_set_sha256": "5" * 64, "archive_only_inputs": True, "report_policy": "bounded", "timeout_seconds": 30, "source": harness_source, "shared_helper_schema": SHARED_HELPER_SCHEMA}, "fixtures": [{"role": "primary_xlsx", "basename": "fixture.xlsx", "extension": ".xlsx", "bytes": 1, "sha256": "6" * 64, "path_redacted": True}], "outcomes": {"error_code_match": 7, "passed": 6, "values_equal_fingerprint_drift": 1}, "values_aligned": True, "difference_scenario_ids": [], "scenarios": scenarios, "blockers": ["materialized_fingerprint_drift"], "matched": False, "acceptance": False, "non_claims": ["no_complete_com_parity"]}
