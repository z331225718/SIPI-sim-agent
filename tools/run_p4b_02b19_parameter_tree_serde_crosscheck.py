# -*- coding: utf-8 -*-
"""P4B-02b19 AMI parameter tree JSON Serde cross-check (product vs independent ref).

Drives the product parameter tree Serde runner (p4b_02b19_parameter_tree_serde_runner) over parenthesized
AMI text inputs. An independent Python reference recomputes tree JSON Serde and S-expression roundtrip formatting.
...
EVIDENCE_SCHEMA = "sipi.p4b-02b19-parameter-tree-serde-crosscheck-evidence.v1"
...
def ref_serde_tree(text: str) -> dict[str, Any]:
    text_clean = re.sub(r'\|.*', '', text)
    tokens = re.findall(r'\(|\)|"[^"]*"|[^\s()]+', text_clean)
    if not tokens:
        return {"valid": False, "serde_error": "EmptyDocument"}
    ...
    for form in forms:
        ...
        if err:
            return {"valid": False, "serde_error": err}
        formatted_roots.append(ref_format_node(node, 0))

    return {"valid": True, "roundtrip_formatted_text": "\n".join(formatted_roots)}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b19_parameter_tree_serde_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b19_parameter_tree_serde_runner-*.exe"))[-1]

    cases = [
        {
            "label": "standard_tree_serde",
            "text": "(Reserved_Parameters (tx_swing Float 0.5) (dfe (tap_1 Integer 2)))",
        },
        {
            "label": "nested_branches_serde",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b19-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_serde_tree(case["text"])

            matched = True
            if product.get("valid") != reference["valid"]:
                matched = False
            elif product.get("valid"):
                if product.get("roundtrip_formatted_text") != reference["roundtrip_formatted_text"]:
                    matched = False
            else:
                prod_err = product.get("serde_error") or product.get("build_error") or product.get("parse_error")
                ref_err = reference.get("serde_error") or reference.get("build_error")
