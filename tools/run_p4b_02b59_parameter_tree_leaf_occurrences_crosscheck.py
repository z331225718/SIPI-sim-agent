# -*- coding: utf-8 -*-
"""P4B-02b59 parameter tree leaf occurrence counts cross-check (product vs independent ref).

Drives the product leaf occurrence runner (p4b_02b59_parameter_tree_leaf_occurrences_runner) over
parenthesized AMI text inputs. An independent Python reference replicates the tree build and the
name counting. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b59-parameter-tree-leaf-occurrences-crosscheck-evidence.v1"
...
def ref_counts(text: str) -> dict[str, Any]:
    ...
    def walk(n: dict[str, Any]) -> None:
        ...
        if n["kind"] == "leaf":
            total += 1
            occurrences[n["name"]] = occurrences.get(n["name"], 0) + 1
        else:
            for c in n["children"].values():
                walk(c)

    walk(node)
    return {"valid": True, "total_leaves": total, "occurrences": occurrences}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b59_parameter_tree_leaf_occurrences_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b59_parameter_tree_leaf_occurrences_runner-*.exe"))[-1]

    cases = [
        {
            "label": "cross_depth_duplicates",
            "text": "(root (sub (gain 1)) (gain 2) (steps 3))",
        },
        {
            "label": "unique_names",
            "text": "(root (gain 1) (steps 2))",
        },
        {
            "label": "root_leaf",
            "text": "(root)",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b59-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            if product.get("valid") != reference.get("valid"):
                ...
                if (product.get("total_leaves") != reference.get("total_leaves") or
                    product.get("occurrences") != reference.get("occurrences")):
                    matched = False
            else:
