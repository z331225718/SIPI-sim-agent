# -*- coding: utf-8 -*-
"""P4B-02b32 AMI parameter tree leaf value validation cross-check (product vs independent ref).

Drives the product parameter tree value validation runner
(p4b_02b32_parameter_tree_value_validation_runner) over parenthesized AMI text inputs plus a
caller-supplied type map. An independent Python reference replicates the P4B-02b1 value rules
over the tree leaves. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b32-parameter-tree-value-validation-crosscheck-evidence.v1"
...
def ref_validate(text: str, types: dict[str, str]) -> dict[str, Any]:
    ...
    def walk(n: dict[str, Any]) -> bool:
        nonlocal leaves_checked, tokens_checked, error
        if n["kind"] == "branch":
        ...
        for token in value_tokens:
            ...
            if rule_error is not None:
                if rule_error in ("InvalidFloat", "InvalidInteger", "InvalidBoolean", "InvalidList"):
                    error = (f'{rule_error} {{ leaf: "{name}", token: "{token}" }}')
                elif rule_error == "EmptyStringValue":
                    error = f'EmptyStringValue {{ leaf: "{name}" }}'
                else:
                    error = rule_error
                return False
            tokens_checked += 1
        leaf_types[name] = type_token
        leaves_checked += 1
        return True
    ...
    return {"valid": False, "validation_error": error}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b32_parameter_tree_value_validation_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b32_parameter_tree_value_validation_runner-*.exe"))[-1]

    cases = [
        {
            "label": "mixed_valid_types",
        ...
        {
            "label": "missing_type_fails_closed",
            "text": "(root (gain Float 0.5) (steps Integer 7))",
            "types": {"gain": "Float"},
        },
        {
            "label": "invalid_float_token",
            "text": "(root (gain Float x1) (steps Integer 7))",
            "types": {"gain": "Float", "steps": "Integer"},
        },
        {
            "label": "invalid_boolean_token",
            "text": "(root (enabled Boolean maybe))",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b32-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_validate(case["text"], case["types"])
            ...
            if product.get("valid") != reference.get("valid"):
                ...
                if (product.get("leaves_checked") != reference.get("leaves_checked") or
                    product.get("tokens_checked") != reference.get("tokens_checked") or
                    product.get("leaf_types") != reference.get("leaf_types")):
                    matched = False
            else:
                prod_err = (product.get("validation_error") or product.get("build_error")
                            or product.get("parse_error") or product.get("types_error"))
                ref_err = (reference.get("validation_error") or reference.get("build_error")
