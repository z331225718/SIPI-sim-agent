...
CANDIDATE_COMMIT = "9914a23dc747d94405f2fed9227538ae7aea8629"
CANDIDATE_TREE = "94a25c6254fdec3802b4946ec777ce65f22fdc0c"
...
def tool_identity(path: Path, role: str, args: tuple[str, ...]) -> dict[str, Any]:
    ...
    return {"role": role, "executable": path.name, "file_sha256": sha(path), "version_sha256": sha_bytes(result.stdout + b"\0" + result.stderr), "exit_code": result.returncode}
...
def execute(row: str, *, run_id: str, report_path: Path, python: Path, cargo: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if row == "AS-02":
            ...
            metrics = {"upstream_cascade_mean_rms": up["cascade_mean_rms_error"], "candidate_cascade_mean_rms": cand["cascade_mean_rms_error"], "upstream_block_rms": [x["full_band_mean_rms_error"] for x in up["blocks"]], "candidate_block_rms": [x["gate"]["full_band_rms"] for x in cand["blocks"]]}
        else:
            ...
            up = json.loads(upstream_report.read_text(encoding="utf-8")); cand = json.loads(candidate_report.read_text(encoding="utf-8"))
            metrics = {"upstream_y_rms_siemens": up["y_rms_siemens"], "candidate_y_rms_siemens": cand["y_rms_siemens"], "upstream_y_mean_rms_siemens": up["y_mean_rms_siemens"], "candidate_y_mean_rms_siemens": cand["y_mean_rms_siemens"]}
        report = {"schema": "sipi.agent-spice-as-numeric-bound.v2", "workflow": row, "status": "completed_numeric_mismatch_open", "parity_claim": False, "numeric_parity": False, "run_id": run_id, "fresh_run_nonce": fresh_nonce, "candidate": {"commit": CANDIDATE_COMMIT, "tree": CANDIDATE_TREE, "archive_sha256": candidate_archive_sha}, "upstream": {"commit": UPSTREAM_COMMIT, "tree": UPSTREAM_TREE, "archive_sha256": upstream_archive_sha}, "build": {**build, "binary_sha256": sha(binary)}, "toolchain": {"cargo": tool_identity(cargo, "cargo", ("--version",)), "rustc": tool_identity(rustc, "rustc", ("--version",)), "python": tool_identity(python, "python", ("--version",))}, "fixture_sha256": sha(input_path), "execution": {"upstream": upstream_result, "candidate": candidate_result, "upstream_report_sha256": sha(upstream_report), "candidate_report_sha256": sha(candidate_report)}, "metrics": metrics, "non_claims": ["Numeric mismatch remains open.", "No parity or acceptance tolerance is claimed."]}
