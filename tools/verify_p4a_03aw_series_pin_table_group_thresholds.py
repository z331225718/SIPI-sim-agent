# -*- coding: utf-8 -*-
"""Fail closed on the P4A-03aw typed IBIS series pin group model threshold parameters core.

Charter fixes the series pin group model thresholds scope; cross-check evidence binds product
lifting against an independent reference. Verifier binds charter, source map,
evidence, Rust tokens, PLAN P4A-03aw.
"""
...
CHARTER = ROOT / "docs" / "baselines" / "p4a-03aw-series-pin-table-group-thresholds-stage.v1.yaml"
SOURCE_MAP = ROOT / "docs" / "baselines" / "p4a-03aw-mit-source-map.v1.yaml"
EVIDENCE = ROOT / "docs" / "baselines" / "p4a-03aw-series-pin-table-group-thresholds-crosscheck-evidence.v1.yaml"
SOURCE = ROOT / "crates" / "sipi-ibis" / "src" / "series_pin_mapping_table_group_model_thresholds_v1.rs"
PLAN = ROOT / "PLAN.md"
SCHEMA = "sipi.p4a-03aw.series-pin-table-group-thresholds-stage.v1"
...
def validate(root: Path = ROOT) -> dict[str, Any]:
    ...
    required_tokens = (
        ...
        "SeriesPinTableGroupModelThresholdsErrorV1",
        POLICY,
    )
    if any(token not in source for token in required_tokens):
        raise SeriesPinTableGroupThresholdsError("implementation_binding_drift")
    evidence = load_yaml(EVIDENCE)
    if evidence.get("schema") != "sipi.p4a-03aw.series-pin-table-group-thresholds-crosscheck-evidence.v1":
