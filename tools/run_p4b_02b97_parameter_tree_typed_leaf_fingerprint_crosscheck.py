# -*- coding: utf-8 -*-
"""P4B-02b97 parameter tree typed-leaf fingerprint cross-check (product vs independent ref).

Drives the product typed-leaf fingerprint runner
(p4b_02b97_parameter_tree_typed_leaf_fingerprint_runner) over parenthesized AMI text inputs. An
independent Python reference replicates the tree build, the typed-form leaf collection (canonical
path -> type/value with sorted keys and compact separators), and FNV-1a 64. Fail closed on any
mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b97-parameter-tree-typed-leaf-fingerprint-crosscheck-evidence.v1"
...
def ref_fingerprint(text: str) -> dict[str, Any]:
    ...
    def walk(n: dict[str, Any], prefix: str) -> None:
        ...
        if n["kind"] == "leaf":
            ...
            if len(tokens_list) == 2 and tokens_list[0] in KNOWN_TYPES:
                leaves[current] = {"type": tokens_list[0], "value": tokens_list[1]}
            return
        for child in n["children"].values():
            walk(child, current)

    walk(node, "")
    serialized = json.dumps(leaves, sort_keys=True, separators=(",", ":"),
                            ensure_ascii=False)
    return {"valid": True, "hash": ref_fnv1a64(serialized.encode("utf-8"))}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b97_parameter_tree_typed_leaf_fingerprint_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b97_parameter_tree_typed_leaf_fingerprint_runner-*.exe"))[-1]

    cases = [
        {"label": "typed_leaves",
         "text": "(root (gain Float 0.5) (steps Integer 7))"},
        {"label": "change_value",
         "text": "(root (gain Float 0.5001) (steps Integer 7))"},
        {"label": "skipped_ignored",
         "text": "(root (gain Float 0.5) (plain 5))"},
        {"label": "no_typed",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b97-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_fingerprint(case["text"])
