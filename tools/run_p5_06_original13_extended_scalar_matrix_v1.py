"""Run one source-visible extended-scalar original-13 MATLAB or Rust replay.
...
# Exact-name intersection of MATLAB ``output_args`` and the Rust result
# surface. A field is only recorded when both runtimes emit a value; source
# profiles legitimately omit optional values such as TD-ILN.
METRICS = ("COM_dB", "CTLE_DC_gain_dB", "ERL", "ERL11", "ERL22", "FOM", "FOM_ILD", "FOM_TDILN", "ICN_mV", "IL_dB_channel_only_at_Fnq", "MDFEXT_ICN_92_47_mV", "MDNEXT_ICN_92_46_mV", "Peak_ISI_XTK_and_Noise_interference_at_BER_mV", "VEC_dB", "VEO_mV", "available_signal_after_eq_mV", "channel_operating_margin_dB", "fitted_IL_dB_at_Fnq", "g_DC_HP", "itick", "sigma_N")
...
CANDIDATE=("bdc0dfd849a133cb04d871a7d32a73b34acebca1","9504de118a6f0e6197829a824c1315de83c3e34f","0902f6df6fe3666051719c8e624b258978a23c7a616d14bfbc67fa710e473108",59146240)
CHANNELS=(
...
def engine_worker(spec_path):
    ...
    harness=Path(__file__).with_name("sipi_com_extended_scalar_oracle_v1.m")
    ...
    try:
        ...
        engine.sipi_com_extended_scalar_oracle_v1(spec["upstream"],str(mat),str(output),float(1),float(1),spec["run_nonce"],*spec["channels"],nargout=0)
    finally:engine.quit()
...
def matlab_metrics(case_dir):
    ...
    return [{name:decode(value) for name,value in item.get("output_metrics",{}).items() if name in METRICS and value is not None} for item in source]
def rust_metrics(result): return [{name:decode(value) for name,value in item.get("metrics",{}).items() if name in METRICS and value is not None} for item in result.get("cases",[])]
