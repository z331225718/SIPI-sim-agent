# -*- coding: utf-8 -*-
"""P4B-02b190 parameter list frequency map cross-check (product vs ref).

Drives the product frequency-map runner over one validated list value.
An independent Python reference replicates the rule: the list trims items and
builds a mapping from each distinct item to its total occurrence count (raw
byte equality); non-List values fail closed.  Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b190-parameter-list-frequency-map-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b190.parameter-list-frequency-map-v1.frequency-map"


def ref_frequency_map(type_token: str, value_token: str) -> dict:
    if type_token != "List":
    ...
    return {"valid": True, "frequency_map": mapping}
...
def main() -> int:
    ...
    os.environ["SIPI_P4B_RUNNER"] = "p4b_02b190_parameter_list_frequency_map_runner"
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b190-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_frequency_map(case["type"], case["value"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("frequency_map") == reference.get("frequency_map")
