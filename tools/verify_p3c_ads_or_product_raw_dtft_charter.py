...
def verify(document: object) -> dict[str, object]:
    ...
    fail(document["status"] != "specified_owner_authorization_pending_external_diagnostic_only", "status")
    fail(document["authority"] != {"actor": "user", "decision_ref": "owner-authorization-pending-2026-08-15-cmp1-or-payload", "scope": "one_fixed_ads_cmp1_or_to_product_raw_periodic_dtft_observation_only"}, "authority")
    ...
    fail(document["external_help_allowlist"] != {"basename": "Transient_Simulation_Parameters.html", "byte_length": 150522, "sha256": "45372e9c7c79492bf6023a4e860f05a20caf0ef5e2a083407c119909afc187bf", "surface": "CMP1_OR", "required_authoritative_meaning": "original_spectrum_before_causality"}, "help")
    fail(document["ads_surface"] != {"matrix_dimension": 4, "members_per_surface": 16, "member_axes_must_match_exactly": True, "axis": "cmp1_or_native_all_finite_strictly_increasing_frequency_nodes", "selected_reduction": "s21_minus_s23_minus_s41_plus_s43_over_4", "source_payload_read_authorization": "pending"}, "ads_surface")
    ...
    fail(document["blockers"] != ["owner_authorization_to_read_cmp1_or_payload_missing", "cmp1_or_authoritative_precausality_semantics_must_be_revalidated", "two_fresh_external_cmp1_or_product_raw_observations_missing", "ads_or_product_raw_delta_not_cause_attribution"], "blockers")
    ...
    return {"valid": True, "external_payload_authorized": False, "admitted": False}
