# -*- coding: utf-8 -*-
"""Tests for the P4B-02b184 parameter list pairwise-distinct adjacent verifier."""
...
import verify_p4b_02b184_parameter_list_pairwise_distinct_adjacent as GATE


class ParameterListPairwiseDistinctAdjacentTests(unittest.TestCase):
    ...
    def test_implementation_tokens_present(self) -> None:
        ...
        self.assertIn("pub fn parameter_list_pairwise_distinct_adjacent_v1", source)
        self.assertIn(GATE.POLICY, source)

    def test_plan_row_present(self) -> None:
        self.assertIn("**P4B-02b184", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_rejects_evidence_mismatch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "EVIDENCE", tmp_path):
                with self.assertRaises(GATE.ParameterListPairwiseDistinctAdjacentError):
                    GATE.validate(ROOT)
