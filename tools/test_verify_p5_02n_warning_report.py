# -*- coding: utf-8 -*-
"""Tests for the P5-02n warning-report aggregator verifier."""
...
import verify_p5_02n_warning_report as GATE
...
class WarningReportTests(unittest.TestCase):
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-02n", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_evidence_exercises_both_paths(self) -> None:
        ...
        self.assertIn("union_sorted", labels)
        self.assertIn("empty_reports_fails", [e["label"] for e in failure])

    def test_union_ordering_is_sorted(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        union = [e for e in evidence["entries"] if e["label"] == "union_sorted"]
        self.assertEqual(union[0]["product_active"], ["anti_causal", "high_freq_non_decay"])

    def test_rejects_full_contract_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["mlse_der_cdr_contract"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.WarningReportError):
                    GATE.validate(ROOT)
