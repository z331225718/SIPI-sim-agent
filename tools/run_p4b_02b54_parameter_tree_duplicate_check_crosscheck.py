# -*- coding: utf-8 -*-
"""P4B-02b54 AMI parameter tree duplicate-name consistency check cross-check (product vs independent ref).

Drives the product duplicate check runner (p4b_02b54_parameter_tree_duplicate_check_runner) over
parenthesized AMI text inputs. An independent Python reference replicates the tree build and the
occurrence consistency classification. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b54-parameter-tree-duplicate-consistency-check-crosscheck-evidence.v1"
...
def ref_check(text: str) -> dict[str, Any]:
    ...
    def walk(n: dict[str, Any]) -> None:
        ...
        if n["kind"] == "leaf":
            leaves += 1
            occurrences.setdefault(n["name"], []).append(list(n["value_tokens"]))
        else:
    ...
    return {"valid": True, "leaves_checked": leaves, "duplicated_names": duplicated_names,
            "consistent": consistent, "inconsistent": inconsistent}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b54_parameter_tree_duplicate_check_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b54_parameter_tree_duplicate_check_runner-*.exe"))[-1]

    cases = [
        {
            "label": "consistent",
            "text": "(root (sub (gain Float 1.0)) (gain Float 1.0))",
        },
        {
            "label": "inconsistent",
        ...
        {
            "label": "unique",
            "text": "(root (gain Float 1.0) (steps Integer 7))",
        },
        {
            "label": "mixed",
            "text": "(root (sub (a 1) (b 2)) (a 1) (b 3))",
        },
    ]

    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b54-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            if product.get("valid") != reference.get("valid"):
                ...
                if (product.get("leaves_checked") != reference.get("leaves_checked") or
                    product.get("duplicated_names") != reference.get("duplicated_names") or
                    product.get("consistent") != reference.get("consistent") or
                    product.get("inconsistent") != reference.get("inconsistent")):
                    matched = False
            else:
