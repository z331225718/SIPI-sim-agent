"""Tests for the P5-04g residual-channel PDF stage verifier."""
...
import verify_p5_04g_residual_channel_pdf as GATE


class ResidualPdfStageTests(unittest.TestCase):
    ...
    def test_source_map_has_four_mappings(self) -> None:
        ...
        self.assertIn("build_r480_noise_pdf", source_map["not_ported"])
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-04g", GATE.PLAN.read_text(encoding="utf-8"))

    def test_evidence_matched_ten_cases(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        self.assertEqual(len(evidence["entries"]), 10)
        self.assertTrue(all(entry["matched"] for entry in evidence["entries"]))

    def test_rejects_build_noise_pdf_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["build_r480_noise_pdf"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.ResidualPdfStageError):
                    GATE.validate(ROOT)
