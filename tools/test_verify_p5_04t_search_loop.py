"""Tests for the P5-04t search-loop support stage verifier."""
...
import verify_p5_04t_search_loop as GATE
...
class SearchLoopTests(unittest.TestCase):
    ...
    def test_source_map_has_twelve_mappings(self) -> None:
        source_map = GATE.load_yaml(GATE.SOURCE_MAP)
        self.assertEqual(len(source_map["mapping"]), 12)
        self.assertEqual(source_map["not_ported"], [])
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-04t", GATE.PLAN.read_text(encoding="utf-8"))

    def test_evidence_matches(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        self.assertGreaterEqual(len(evidence["entries"]), 18)
        self.assertTrue(all(entry["matched"] for entry in evidence["entries"]))

    def test_rejects_candidate_admission_off(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["candidate_evaluation"] = False
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.SearchLoopError):
                    GATE.validate(ROOT)
