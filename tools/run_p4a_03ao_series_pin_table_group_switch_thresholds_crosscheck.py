# -*- coding: utf-8 -*-
"""P4A-03ao typed IBIS series pin group switch threshold parameters cross-check (product vs independent ref).

Drives the product series pin group switch threshold runner (p4a_03ao_series_pin_table_group_switch_thresholds_runner) over valid
...
EVIDENCE_SCHEMA = "sipi.p4a-03ao.series-pin-table-group-switch-thresholds-crosscheck-evidence.v1"
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ibis", "--test", "p4a_03ao_series_pin_table_group_switch_thresholds_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4a_03ao_series_pin_table_group_switch_thresholds_runner-*.exe"))[-1]
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4a-03ao-") as tmp:
        work = Path(tmp)
        for case in cases:
