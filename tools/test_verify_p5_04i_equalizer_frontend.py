"""Tests for the P5-04i equalizer front-end stage verifier."""
...
import verify_p5_04i_equalizer_frontend as GATE


class FrontendStageTests(unittest.TestCase):
    ...
    def test_source_map_has_four_mappings(self) -> None:
        ...
        self.assertIn("tx_ffe grid (tx_ffe.py)", source_map["not_ported"])
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-04i", GATE.PLAN.read_text(encoding="utf-8"))

    def test_evidence_seven_cases_three_kinds(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        self.assertEqual(len(evidence["entries"]), 7)
        self.assertTrue(all(entry["matched"] for entry in evidence["entries"]))
        kinds = {entry["kind"] for entry in evidence["entries"]}
        self.assertEqual(kinds, {"cursor", "fd_ctle", "td_ctle"})

    def test_rejects_tx_ffe_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["tx_ffe_grid"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.FrontendStageError):
                    GATE.validate(ROOT)
