"""Mutation coverage for the v6 additive upstream ledger gate."""
...
from tools import verify_upstream_integration_ledger_v6 as gate

EXPECTED_VERIFIER_SHA256 = "ea1b4234e88bbf4408a01d41d5c5f559a38b5c366dc2844dd6ec39dd0f8c0544"

class UpstreamIntegrationLedgerV6Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_current_snapshot_is_open_and_inherits_v5(self) -> None:
        self.assertEqual(gate.validate(self.document), {"valid": True, "rows": 15, "release_ready": 0})
        self.assertEqual(self.document["successor"]["predecessor_sha256"], gate.V5_SHA)
        self.assertEqual(self.document["summary"]["release_ready"], 0)
        verifier_path = gate.ROOT / "tools/verify_upstream_integration_ledger_v6.py"
    ...
    def test_com_fit_dc_accm_and_upstream_claims_are_rejected(self) -> None:
        for claim in ("no_sparam_fit", "no_dc_accm", "no_clean_upstream_numeric_replay"):
            mutated = copy.deepcopy(self.document)
    ...
    def test_source_predecessor_audit_and_harness_drift_are_rejected(self) -> None:
        ...
        mutated = copy.deepcopy(self.document)
        mutated["harness"]["verifier"]["path"] = "tools/verify_upstream_integration_ledger_v5.py"
    ...
    def test_malformed_nested_shapes_fail_closed(self) -> None:
        for field in ("rows", "current_sources", "source_authority", "harness"):
            ...
            self.reject(mutated, "row_shape:AS-01|sources_keys|source_authority|harness_shape|TypeError")
