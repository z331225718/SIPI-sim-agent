# -*- coding: utf-8 -*-
"""Tests for the P4A-03e pin-declaration verifier."""
...
import verify_p4a_03e_pin_declaration as GATE


class PinDeclTests(unittest.TestCase):
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P4A-03e", GATE.PLAN.read_text(encoding="utf-8"))

    def test_pin_counts_match(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        self.assertEqual(evidence["product_pin_count"], 200)
        self.assertEqual(evidence["observer_pin_count"], 200)
        self.assertTrue(evidence["matched"])

    def test_sample_pins_agree(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        self.assertEqual(evidence["product_sample"], evidence["observer_sample"])
    ...
    def test_rejects_profile_admission(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.PinDeclError):
                    GATE.validate(ROOT)
