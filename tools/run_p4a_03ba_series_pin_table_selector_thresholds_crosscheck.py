# -*- coding: utf-8 -*-
"""P4A-03ba typed IBIS series pin table selector threshold parameters cross-check (product vs independent ref).

Drives the product series pin table selector threshold runner (p4a_03ba_series_pin_table_selector_thresholds_runner) over valid
...
EVIDENCE = ROOT / "docs" / "baselines" / "p4a-03ba-series-pin-table-selector-thresholds-crosscheck-evidence.v1.yaml"
EVIDENCE_SCHEMA = "sipi.p4a-03ba.series-pin-table-selector-thresholds-crosscheck-evidence.v1"
POLICY = "sipi.p4a-03ba.series-pin-table-selector-thresholds-v1.typed-selector-thresholds"
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ibis", "--test", "p4a_03ba_series_pin_table_selector_thresholds_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4a_03ba_series_pin_table_selector_thresholds_runner-*.exe"))[-1]
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4a-03ba-") as tmp:
        work = Path(tmp)
        for case in cases:
