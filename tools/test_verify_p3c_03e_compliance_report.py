# -*- coding: utf-8 -*-
"""Tests for the P3C-03e compliance report verifier."""
...
import verify_p3c_03e_compliance_report as GATE
...
class ComplianceReportTests(unittest.TestCase):
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P3C-03e", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_pass_fail_correct(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        for entry in evidence["entries"]:
            if entry["id"] == "fom_pass":
                self.assertTrue(entry["product"]["passed"])
            if entry["id"] == "fom_fail":
                self.assertFalse(entry["product"]["passed"])
    ...
    def test_rejects_profile_admission(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.ComplianceReportError):
                    GATE.validate(ROOT)
