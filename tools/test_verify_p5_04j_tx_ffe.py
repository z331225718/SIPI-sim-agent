"""Tests for the P5-04j TX FFE grid stage verifier."""
...
import verify_p5_04j_tx_ffe as GATE


class TxFfeStageTests(unittest.TestCase):
    ...
    def test_source_map_has_five_mappings(self) -> None:
        ...
        self.assertIn("DFE bank (dfe.py)", source_map["not_ported"])
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-04j", GATE.PLAN.read_text(encoding="utf-8"))

    def test_evidence_seven_cases(self) -> None:
    ...
    def test_rejects_dfe_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["dfe_bank"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.TxFfeStageError):
                    GATE.validate(ROOT)
