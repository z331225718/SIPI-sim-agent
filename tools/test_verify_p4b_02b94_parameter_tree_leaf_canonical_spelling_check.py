# -*- coding: utf-8 -*-
"""Tests for the P4B-02b94 parameter tree leaf canonical spelling check verifier."""
...
import verify_p4b_02b94_parameter_tree_leaf_canonical_spelling_check as GATE


class ParameterTreeLeafCanonicalSpellingCheckTests(unittest.TestCase):
    ...
    def test_implementation_tokens_present(self) -> None:
        ...
        self.assertIn("pub fn check_parameter_tree_leaf_spellings_canonical_v1", source)
        self.assertIn(GATE.POLICY, source)

    def test_plan_row_present(self) -> None:
        self.assertIn("**P4B-02b94", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_rejects_evidence_mismatch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "EVIDENCE", tmp_path):
                with self.assertRaises(GATE.ParameterTreeLeafCanonicalSpellingCheckError):
                    GATE.validate(ROOT)
