# -*- coding: utf-8 -*-
"""Tests for the P4B-02b142 parameter list LCS length verifier."""
...
import verify_p4b_02b142_parameter_list_longest_common_subsequence_length as GATE


class ParameterListLongestCommonSubsequenceLengthTests(unittest.TestCase):
    ...
    def test_implementation_tokens_present(self) -> None:
        ...
        self.assertIn("pub fn list_longest_common_subsequence_length_v1", source)
        self.assertIn(GATE.POLICY, source)

    def test_plan_row_present(self) -> None:
        self.assertIn("**P4B-02b142", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_rejects_evidence_mismatch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "EVIDENCE", tmp_path):
                with self.assertRaises(GATE.ParameterListLongestCommonSubsequenceLengthError):
                    GATE.validate(ROOT)
