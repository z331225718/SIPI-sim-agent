"""Tests for the P5-05b CSV reader stage verifier."""
...
import verify_p5_05b_csv_reader as GATE


class CsvReaderStageTests(unittest.TestCase):
    ...
    def test_source_map_has_four_mappings(self) -> None:
        ...
        self.assertIn("ComSettings.from_mat / _matlab_cell_value", source_map["not_ported"])
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-05b", GATE.PLAN.read_text(encoding="utf-8"))

    def test_evidence_grid_and_negative(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        self.assertEqual(evidence["entries"][0]["rows"], 11)
        self.assertTrue(evidence["negative"][0]["matched"])

    def test_rejects_mat_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["mat_reader"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.CsvReaderStageError):
                    GATE.validate(ROOT)
