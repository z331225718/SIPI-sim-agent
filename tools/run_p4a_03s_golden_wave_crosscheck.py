# -*- coding: utf-8 -*-
"""P4A-03s typed IBIS golden waveforms cross-check (product vs independent ref).

Drives the product golden waveforms runner (p4a_03s_golden_wave_runner) over valid
and invalid golden waveform inputs. An independent Python reference recomputes the lifting rules.
...
EVIDENCE_SCHEMA = "sipi.p4a-03s.golden-wave-crosscheck-evidence.v1"
POLICY = "sipi.p4a-03s.golden-wave-declaration-v1.typed-golden-wave"


def ref_lift_golden_wave(wname: str, dname: str | None) -> dict[str, Any]:
    if not wname or not wname.strip():
        return {"valid": False, "lift_error": "EmptyWaveformName"}
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ibis", "--test", "p4a_03s_golden_wave_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4a_03s_golden_wave_runner-*.exe"))[-1]

    cases = [
        {
            "label": "full_golden_wave",
            "waveform_name": "GOLDEN_WAVE_1",
            "dut_name": "DUT_FBGA84",
        },
        {
            "label": "minimal_golden_wave",
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4a-03s-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_lift_golden_wave(case["waveform_name"], case["dut_name"])
