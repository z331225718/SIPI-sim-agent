...
V4_SCHEMA = "sipi.com.erl-only.exact-profile-replay.v4"
...
CONTROL_CROSSWALK = {
    "runtime_fields": {
        "runtime_n_ui": "runtime.options.N",
        ...
        "tdr_butterworth": "runtime.options.TDR_Butterworth",
        "fb_bw_cutoff": "materialized.parameters.fb_BW_cutoff",
        "f_r": "materialized.parameters.f_r",
        "tukey_enabled": "materialized.parameters.Tukey_Window",
    },
    "raw_source_fields_not_runtime_controls": ["Z0", "T_r_filter_type", "T_r_meas_point", "T_k", "TDR_duration"],
    "source_constants": {"s_reference_ohm": 100.0, "tdr_delay_s": 5.0e-10, "transition_filter_type": 1, "transition_measurement_point": 0},
}
...
def canonical_config(path: Path) -> None:
    document = {
        ...
        "portable": {"erl_only": {**OUTER_CONTROLS, "tdr_profile": {key: value for key, value in PROFILE.items() if key not in {"runtime_n_ui", "tdr_butterworth", "fb_bw_cutoff", "f_r"}}}},
    }
...
def upstream_probe(root: Path, fixture: Path, uv: Path, python: Path, uv_cache_dir: Path, runtime_n_ui: int) -> dict[str, Any]:
    code = r'''
import json
from pathlib import Path
import numpy as np
...
config = load_config(root / "matlab_src/config_com_ieee8023_93a=3ck_SA_120g_C2M_tp1a_08_17_2022.xlsx", overrides={"ERL_ONLY": 1, "TDR_W_TXPKG": 0, "N": __N__})
...
print(json.dumps({
    ...
    "controls": {
        ...
        "runtime_n_ui": int(options["N"]), "tdr_butterworth": bool(options.get("TDR_Butterworth", True)), "fb_bw_cutoff": float(parameters["fb_BW_cutoff"]), "f_r": float(parameters["f_r"]), "tukey_enabled": bool(parameters.get("Tukey_Window", options.get("Tukey_Window", 1))), "tdr_profile": {"name": "r480_s2p_erl_v1", "runtime_n_ui": int(options["N"]), "tdr_butterworth": bool(options.get("TDR_Butterworth", True)), "fb_bw_cutoff": float(parameters["fb_BW_cutoff"]), "f_r": float(parameters["f_r"]), "samples_per_ui": int(parameters["samples_per_ui"]), "levels": int(parameters["levels"]), "bin_size": float(options["BinSize"]), "spec_ber": float(parameters["specBER"]), "rl_norm_test": bool(options["RL_norm_test"]), "baud_hz": float(parameters["fb"]), "sample_dt_s": float(parameters["sample_dt"]), "s_reference_ohm": 100.0, "zt_ohm": float(parameters["Z_t"]), "transition_time_ns": float(parameters["TR_TDR"]), "transition_filter_type": 1, "transition_measurement_point": 0, "receiver_cutoff_multiplier": float(parameters["f_r"]), "receiver_filter_enabled": bool(options["INCLUDE_FILTER"]), "tukey_enabled": bool(parameters.get("Tukey_Window", options.get("Tukey_Window", 1))), "fixture_delay_s": float(np.asarray(parameters["tfx"]).reshape(-1)[0]), "tdr_delay_s": 5.0e-10, "observation_duration_ui": float(options["TDR_duration"]), "gate_n_bx": int(parameters["N_bx"]), "gate_rho_x": float(parameters["rho_x"]), "gate_grr": float(parameters["Grr"]), "gate_beta_x_db_per_s": float(parameters["beta_x"])}
    },
    "diagnostic_keys": sorted(diag.keys()),
}, sort_keys=True))
'''.replace("__FIXTURE__", repr(str(fixture))).replace("__METRICS__", repr(list(METRIC_FIELDS))).replace("__N__", str(int(runtime_n_ui)))
    ...
    with tempfile.TemporaryDirectory(prefix="com-erl-v4-upstream-") as runtime_temp:
...
def main() -> int:
    ...
    parser.add_argument("--runtime-n-ui", type=int, choices=(1, 800), default=800)
    parser.add_argument("--preflight", action="store_true")
    args = parser.parse_args()
    if not re.fullmatch(r"com-erl-exact-v4-(?:n800-run[12]|n1-negative)", args.run_id):
        raise SystemExit("run_id must be com-erl-exact-v4-n800-run1/2 or com-erl-exact-v4-n1-negative")
    report_relative = Path(args.report)
    expected_report = f"docs/baselines/com-erl-exact-profile-replay-v4-{args.run_id.removeprefix('com-erl-exact-v4-')}.json"
    allowed_report = report_relative.as_posix() == "tools/v4-preflight.json" if args.preflight else report_relative.as_posix() == expected_report
    if report_relative.is_absolute() or not allowed_report:
        raise SystemExit("report must be the strict repo-relative v4 run path")
    ...
    positive = args.runtime_n_ui == 800
    with tempfile.TemporaryDirectory(prefix="com-erl-exact-v4-") as directory:
        temp = Path(directory)
        ...
        upstream_payload = upstream_probe(upstream, upstream_fixture, args.uv.resolve(), args.python.resolve(), uv_cache_dir, args.runtime_n_ui)
        upstream_elapsed = round(time.monotonic() - upstream_started, 6)
        # The N=1 guard is upstream-only and must not execute the candidate.
        candidate_payload = {"status": "excluded_from_parity", "runtime_executed": False}
        if args.runtime_n_ui == 800:
        ...
        if upstream_payload.get("status") != "ok" or (args.runtime_n_ui == 800 and candidate_payload.get("status") != "ok"):
            reason = f"runtime_blocked_upstream_{upstream_payload.get('status')}_candidate_{candidate_payload.get('status')}"
            write_blocked_report(report_path, args.run_id, reason, preflight_toolchain)
            return 2
        ...
        stage_differences = {
            name: {"upstream": upstream_payload["stages"].get(upstream_name), "candidate": candidate_payload.get("stages", {}).get(candidate_name)}
            for name, (upstream_name, candidate_name) in stage_mapping.items()
            if positive and upstream_payload["stages"].get(upstream_name) != candidate_payload.get("stages", {}).get(candidate_name)
        } if args.runtime_n_ui == 800 else {}
        differences = {key: {"upstream": upstream_payload["metrics"].get(key), "candidate": candidate_payload.get("metrics", {}).get(key)} for key in METRIC_FIELDS if args.runtime_n_ui == 800 and not metric_equal(key, upstream_payload["metrics"].get(key), candidate_payload.get("metrics", {}).get(key))}
        candidate_profile = dict(PROFILE)
        candidate_profile["runtime_n_ui"] = args.runtime_n_ui
        candidate_controls = {"outer": OUTER_CONTROLS, "tdr_profile": candidate_profile}
        controls_equal = runtime_control_projection(upstream_payload["controls"]) == runtime_control_projection(candidate_controls) if args.runtime_n_ui == 800 else False
        report_controls = {"positive_runtime_n_ui": 800, "negative_runtime_n_ui": 1, "profile_observation_duration_ui": 800, "outer": OUTER_CONTROLS, "tdr_profile": PROFILE}
        report = {
            "schema": V4_SCHEMA,
            "run_id": args.run_id,
            "fresh_run_nonce": secrets.token_hex(32),
            "status": "bound_clean_archive_observation" if positive else "upstream_only_guard",
            "path_policy": {"mode": "relative identities only", "absolute_paths_emitted": False},
            "candidate": {"commit": CANDIDATE_COMMIT, "tree": CANDIDATE_TREE, "archive_sha256": candidate_sha, "materialization": "git archive; no working-tree overlay", "runtime_executed": positive, "excluded_from_parity": not positive, **({"binary_sha256": sha256_file(binary), "binary_custody": windows_pe_replay_custody(binary)} if positive else {}), "cargo_lock_sha256": sha256_file(candidate / "Cargo.lock")},
            ...
            "input": {"fixture_relative": FIXTURE_RELATIVE, "fixture_sha256": fixture_sha, "fixture_bytes": len(fixture_bytes), "fixture_rows": len(rows), "fixture_copies": {"pre_sha256": copy_pre_sha, "post_sha256": copy_post_sha, "independent": True, "read_only": True}, "config_sha256": sha256_file(config), "upstream_workbook_sha256": sha256_file(upstream / "matlab_src/config_com_ieee8023_93a=3ck_SA_120g_C2M_tp1a_08_17_2022.xlsx"), "channel_kind": "exact_s2p_erl_only", "s_parameter_fit": "forbidden", "channel_policy": "raw S11 FD-to-TD impulse; no fit", "controls": report_controls, "runtime_n_ui": args.runtime_n_ui, "observation_duration_ui": 800},
            "control_crosswalk": {"candidate_profile": candidate_controls if positive else None, "upstream_materialized": upstream_payload["controls"], "runtime_projection": {"candidate": runtime_control_projection(candidate_controls) if positive else None, "upstream": runtime_control_projection(upstream_payload["controls"])}, "mapping": CONTROL_CROSSWALK, "status": "matched" if positive and controls_equal else "numeric_mismatch_open" if positive else "excluded_from_parity"},
            "stage_payload": {"mapping": stage_contract, "upstream": upstream_payload["stages"], "candidate": candidate_payload.get("stages", {}), "upstream_diagnostic_keys": upstream_payload["diagnostic_keys"], "candidate_dispatch": candidate_payload.get("dispatch"), "ptdr": {"status": "diagnostic_only_upstream_api_does_not_expose_raw_ptdr", "compared": False}},
            "upstream_output": upstream_payload["metrics"],
            "candidate_output": candidate_payload.get("metrics", {}),
            "artifact": {"candidate_result_sha256": candidate_payload.get("artifact_sha256")},
            ...
            "execution": {"runner": {"path": "tools/run_com_erl_exact_profile_replay_v4.py", "sha256": sha256_file(runner)}, "helper": {"path": "tools/com_erl_exact_profile_replay_v3_support.py", "sha256": sha256_file(helper)}, "pe_helper": {"path": "tools/pb_03_replay_common.py", "sha256": sha256_file(runner.with_name("pb_03_replay_common.py"))}, "timeout_s": RUNTIME_TIMEOUT_S, "build_timeout_s": BUILD_TIMEOUT_S, "upstream_elapsed_s": upstream_elapsed, "candidate_timeout_s": RUNTIME_TIMEOUT_S if positive else None, "candidate_build_exit": build.returncode if build is not None else None, "build_profile": "release", "locked": True, "upstream_python_env": UPSTREAM_PYTHON_ENV, "upstream_uv_cache_policy": UV_CACHE_POLICY, "upstream_command": UPSTREAM_RUNTIME_COMMAND},
            "parity": {"fields": list(METRIC_FIELDS), "numeric_policy": {"float_fields": ["ERL", "ERL11", "ERL_RMS"], "atol": 1.0e-12, "rtol": 1.0e-12, "phase_field": "ERL_phase_index", "phase_policy": "exact"}, "matched": positive and not differences and not stage_differences and controls_equal, "differences": differences, "stage_differences": stage_differences if positive else {}, "controls_equal": controls_equal, "status": ("matched" if not differences and not stage_differences and controls_equal else "numeric_mismatch_open") if positive else "excluded_from_parity"},
    ...
    if args.preflight:
        print(json.dumps({"schema": report["schema"], "run_id": report["run_id"], "runtime_n_ui": args.runtime_n_ui, "upstream_metrics": report["upstream_output"], "candidate_metrics": report["candidate_output"], "stage_payload": report["stage_payload"], "parity": report["parity"]}, sort_keys=True))
