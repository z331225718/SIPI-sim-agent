...
import verify_pb_02_direct_port as verifier  # noqa: E402
...
class VerifyPb02DirectPortTests(unittest.TestCase):
    ...
    def test_current_evidence_is_valid(self):
        ...
        self.assertEqual(result["source_paths"], 19)
    ...
    def test_source_blob_mutation_is_rejected(self):
        document = copy.deepcopy(self.document)
        document["direct_port_files"]["files"]["native/pybert-core/src/input.rs"]["blob_sha1"] = "0" * 40
        result = verifier.verify(document)
        self.assertIn("Git blob binding drift: native/pybert-core/src/input.rs", result["blockers"])
    ...
    def test_adapted_crate_root_provenance_mutation_is_rejected(self):
        ...
        result = verifier.verify(document)
        self.assertIn("adapted crate root binding drift: source_sha256", result["blockers"])
