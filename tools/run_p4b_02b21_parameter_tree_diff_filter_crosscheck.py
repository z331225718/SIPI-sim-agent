# -*- coding: utf-8 -*-
"""P4B-02b21 AMI parameter tree diff entry filter cross-check (product vs independent ref).

Drives the product parameter tree diff filter runner (p4b_02b21_parameter_tree_diff_filter_runner) over parenthesized
AMI text inputs and exclude paths. An independent Python reference recomputes diff entry filtering.
...
EVIDENCE_SCHEMA = "sipi.p4b-02b21-parameter-tree-diff-filter-crosscheck-evidence.v1"
...
def ref_filter_diffs_trees(text_left: str, text_right: str, exclude_path: str) -> dict[str, Any]:
    ...
    if not diffs:
        return {"valid": False, "filter_error": "EmptyDiffList"}

    filtered = [d for d in diffs if d["path"] != exclude_path]
    if not filtered:
        return {"valid": False, "filter_error": "EmptyFilteredResult"}

    return {"valid": True, "filtered_diff_count": len(filtered), "filtered_diffs": filtered}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b21_parameter_tree_diff_filter_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b21_parameter_tree_diff_filter_runner-*.exe"))[-1]

    cases = [
        {
            "label": "filter_diff_entry",
            "text_left": "(root (node_a 1) (node_b 2))",
            "text_right": "(root (node_a 1) (node_c 3))",
            "exclude_path": "root.node_b",
        },
        {
            "label": "empty_filtered_result",
            "text_left": "(root (node_a 1) (node_b 2))",
            "text_right": "(root (node_a 1) (node_c 3))",
            "exclude_path": "all",  # All will be excluded by custom predicate if path matched
        },
        {
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b21-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_filter_diffs_trees(case["text_left"], case["text_right"], case["exclude_path"])

            matched = True
            if product.get("valid") != reference["valid"]:
                matched = False
            elif product.get("valid"):
                if (product.get("filtered_diff_count") != reference["filtered_diff_count"] or
                    product.get("filtered_diffs") != reference["filtered_diffs"]):
                    matched = False
            else:
                prod_err = product.get("filter_error") or product.get("diff_error") or product.get("build_error") or product.get("parse_error")
                ref_err = reference.get("filter_error") or reference.get("diff_error") or reference.get("build_error")
