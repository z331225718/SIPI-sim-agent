"""Strict verifier for one current P5-06s original-13 v4 replay."""
...
try:
    ...
    from .run_p5_06_original13_fresh_matrix_v4 import CANDIDATE, HARNESS, MATLAB_RECEIPT, MATLAB_RELEASE, RAYON_THREADS, SCHEMA, TIMING_SCOPE, WORKER, file_identity
...
def verify(report: object) -> bool:
    ...
    require(report["run_id"] == f"original13-v4-{report['engine']}-{report['nonce']}", "run id")
    ...
    require(isinstance(binary, dict) and set(binary) == {"bytes", "sha256"} and isinstance(binary["bytes"], int) and binary["bytes"] > 0 and isinstance(binary["sha256"], str) and HEX.fullmatch(binary["sha256"]), "binary")
    ...
    require(source["gate_tools"] == {"runner": file_identity(Path(__file__).with_name("run_p5_06_original13_fresh_matrix_v4.py")), "worker": file_identity(WORKER), "matlab_harness": file_identity(HARNESS)}, "gate tools")
