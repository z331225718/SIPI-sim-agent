# -*- coding: utf-8 -*-
"""P4B-02b188 parameter list occurrence-indices map cross-check (product vs ref).

Drives the product occurrence-indices-map runner over one validated list value.
An independent Python reference replicates the rule: the list trims items and
builds a mapping from each distinct item to every 0-based occurrence index in
ascending position order (raw byte equality); non-List values fail closed.
...
EVIDENCE_SCHEMA = "sipi.p4b-02b188-parameter-list-occurrence-indices-map-crosscheck-evidence.v1"
POLICY = "sipi.p4b-02b188.parameter-list-occurrence-indices-map-v1.occurrence-indices-map"


def ref_occurrence_indices_map(type_token: str, value_token: str) -> dict:
    if type_token != "List":
    ...
    for idx, item in enumerate(items):
        mapping.setdefault(item, []).append(idx)
    return {"valid": True, "occurrence_indices_map": mapping}
...
def main() -> int:
    ...
    os.environ["SIPI_P4B_RUNNER"] = "p4b_02b188_parameter_list_occurrence_indices_map_runner"
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b188-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_occurrence_indices_map(case["type"], case["value"])

            matched = (product.get("valid") == reference.get("valid")
                       and product.get("occurrence_indices_map") == reference.get("occurrence_indices_map")
