"""Mutation coverage for the v11 additive upstream ledger gate."""
...
from tools import verify_upstream_integration_ledger_v11 as gate

EXPECTED_VERIFIER_SHA256 = "c0df17f3382a9bca0713af4679638055f271f8aad537f7f5731b5a192e9823a3"


class LedgerV11Tests(unittest.TestCase):
    @classmethod
    ...
    def test_v11_baseline_is_valid(self) -> None:
    ...
    def test_v11_new_scopes_remain_diagnostic_only(self) -> None:
        mutations = (
            ("AS-04", "best_trial_frozen_artifact_custody", False),
            ("AS-06", "native_caller_custody", False),
            ("PB-01", "class_pickle_observation", "implemented"),
            ("COM-02", "do_white_noise", "numeric_port"),
        )
        for row_id, key, value in mutations:
            mutated = copy.deepcopy(self.document)
            self.row(mutated, row_id)["current_observation"][key] = value
            self.reject(mutated, "v11-scope:" + row_id)
    ...
    def test_harness_hashes_cannot_self_declare(self) -> None:
        mutated = copy.deepcopy(self.document)
        verifier = Path(gate.ROOT / "tools/verify_upstream_integration_ledger_v11.py")
