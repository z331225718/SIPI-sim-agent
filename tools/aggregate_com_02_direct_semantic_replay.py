"""Aggregate two fresh COM-02 semantic replay reports."""
...
def main() -> int:
    ...
    if len(reports) != 2 or any(report.get("mode") != "com-02" for report in reports):
        raise SystemExit("exactly two COM-02 reports are required")
    semantic = [report["replays"][0]["semantic"] for report in reports]
    aggregate = {
        "schema": "sipi.com-02.direct-semantic-replay-aggregate.v1",
