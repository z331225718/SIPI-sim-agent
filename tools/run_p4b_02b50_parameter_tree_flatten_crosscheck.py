# -*- coding: utf-8 -*-
"""P4B-02b50 AMI parameter tree flatten cross-check (product vs independent ref).

Drives the product flatten runner (p4b_02b50_parameter_tree_flatten_runner) over parenthesized
AMI text inputs. An independent Python reference replicates the tree build and the canonical
preorder record flattening. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b50-parameter-tree-flatten-crosscheck-evidence.v1"
...
def ref_flatten(text: str) -> dict[str, Any]:
    ...
    def walk(n: dict[str, Any], path: list[str]) -> None:
        if n["kind"] == "leaf":
            records.append({
                "path": list(path) + [n["name"]],
                "kind": "Leaf",
                "name": n["name"],
                "value_tokens": list(n["value_tokens"]),
            })
            return
        records.append({
            "path": list(path) + [n["name"]],
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b50_parameter_tree_flatten_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b50_parameter_tree_flatten_runner-*.exe"))[-1]
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b50-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_flatten(case["text"])

            matched = True
            if product.get("valid") != reference.get("valid"):
                matched = False
            elif product.get("valid"):
                if product.get("records") != reference.get("records"):
                    matched = False
            else:
