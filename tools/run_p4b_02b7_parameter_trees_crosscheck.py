# -*- coding: utf-8 -*-
"""P4B-02b7 AMI parameter tree hierarchy cross-check (product vs independent ref).

Drives the product parameter trees runner (p4b_02b7_parameter_trees_runner) over parenthesized
AMI text inputs. An independent Python reference parses the AST and builds tree hierarchies.
...
EVIDENCE_SCHEMA = "sipi.p4b-02b7.parameter-trees-crosscheck-evidence.v1"
...
def ref_build_node(node: Any) -> tuple[dict[str, Any] | None, str | None]:
    ...
    if not has_sublist:
        ...
        # Sort children by key for BTreeMap comparison
...
def ref_build_trees(text: str) -> dict[str, Any]:
    text_clean = re.sub(r'\|.*', '', text)
    tokens = re.findall(r'\(|\)|"[^"]*"|[^\s()]+', text_clean)
    if not tokens:
        return {"valid": False, "build_error": "EmptyDocument"}
    ...
    if tokens[0] != '(':
        return {"valid": False, "build_error": "NoValidTrees"}
    
    # Parse all top-level lists
    idx = 0
    forms = []
    while idx < len(tokens):
        if tokens[idx] == '(':
    ...
    return {"valid": True, "tree_count": len(trees), "trees": trees}


def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b7_parameter_trees_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b7_parameter_trees_runner-*.exe"))[-1]
    ...
    cases = [
        ...
        },
        {
            ...
            "text": '(root (child_a 1) (child_a 2))',
        },
    ]

    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b7-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_build_trees(case["text"])

            matched = True
            if product.get("valid") != reference["valid"]:
                matched = False
            elif product.get("valid"):
                if (product.get("tree_count") != reference["tree_count"] or
                    product.get("trees") != reference["trees"]):
                    matched = False
            else:
                if str(product.get("build_error")) != str(reference.get("build_error")):
                    matched = False
