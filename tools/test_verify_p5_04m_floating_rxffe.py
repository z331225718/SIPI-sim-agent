"""Tests for the P5-04m floating RxFFE stage verifier."""
...
import verify_p5_04m_floating_rxffe as GATE


class FloatingStageTests(unittest.TestCase):
    ...
    def test_source_map_has_four_mappings(self) -> None:
        ...
        self.assertIn("equalizer search (search.py)", source_map["not_ported"])
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P5-04m", GATE.PLAN.read_text(encoding="utf-8"))

    def test_evidence_three_cases(self) -> None:
        evidence = GATE.load_yaml(GATE.EVIDENCE)
        self.assertEqual(len(evidence["entries"]), 3)
        self.assertTrue(all(entry["matched"] for entry in evidence["entries"]))

    def test_rejects_search_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["equalizer_search"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.FloatingStageError):
                    GATE.validate(ROOT)
