...
from run_p5_06_original13_rust_matlab_prep import METRICS, scalar_equal
from verify_p5_06_original13_fresh_matrix import verify
...
def main():
    ...
    for key in ("source","channels"):
        if any(report[key]!=reports[0][key] for report in reports[1:]):p.error(key+" drift")
    ...
    output={"schema":"sipi.p5-06.original13-fresh-aggregate.v1","status":"accepted_stage1" if matlab_repeat and rust_repeat and cross else "blocked","formal_runs":formal_runs,"source":reports[0]["source"],"channels":reports[0]["channels"],"slot_count":303,"matlab_repeat_1e_12":matlab_repeat,"rust_repeat_exact":rust_repeat,"rust_vs_matlab_1e_9":cross,"stage2":{"status":"diagnostic_only","array_tolerances":"unset","instrumentation_equivalence":"unproven"},"claims":{"historical_python_used":False,"td_iln_required":False,"s_parameter_fit":False,"one_final_fd_to_td_impulse":True,"release":False}}
