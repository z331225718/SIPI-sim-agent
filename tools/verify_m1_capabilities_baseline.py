...
sys.path.insert(0, str(R/'packages'/'sipi-contracts'/'src'))
from sipi_contracts.models import parse_capability_baseline
SRC=R/'docs/baselines/capability-inventory.yaml'; OUT=R/'docs/baselines/capabilities.baseline.v1.json'
...
def main():
 ...
 parsed=parse_capability_baseline(out)
 q(parsed.to_wire()==out,'consumer round trip')
