# -*- coding: utf-8 -*-
"""P4B-02b43 AMI parameter tree reserved-name check cross-check (product vs independent ref).

Drives the product reserved-name check runner (p4b_02b43_parameter_tree_reserved_name_check_runner)
over parenthesized AMI text inputs plus a reserved-name set. An independent Python reference
replicates the tree build and the distinct-leaf-name intersection. Fail closed on any mismatch.
"""
...
EVIDENCE_SCHEMA = "sipi.p4b-02b43-parameter-tree-reserved-name-check-crosscheck-evidence.v1"
...
def ref_check(text: str, reserved: list[str]) -> dict[str, Any]:
    ...
    if not reserved_set:
        return {"valid": False, "reserved_error": "EmptyReservedSet"}
    ...
    collect(node)
    violations = sorted(name for name in leaf_names if name in reserved_set)
    return {
        "valid": True,
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ami-text", "--test", "p4b_02b43_parameter_tree_reserved_name_check_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4b_02b43_parameter_tree_reserved_name_check_runner-*.exe"))[-1]
    ...
    cases = [
        ...
        {
            "label": "single_violation",
            "text": "(root (gain Float 0.5) (Reserved_Parameters Integer 1))",
            "reserved": ["Reserved_Parameters"],
        },
        {
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4b-02b43-") as tmp:
        work = Path(tmp)
        for case in cases:
            ...
            reference = ref_check(case["text"], case["reserved"])
            ...
            if product.get("valid") != reference.get("valid"):
                ...
                prod_err = (product.get("reserved_error") or product.get("build_error")
                            or product.get("parse_error"))
                ref_err = (reference.get("reserved_error") or reference.get("build_error")
