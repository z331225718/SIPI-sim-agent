# -*- coding: utf-8 -*-
"""Tests for the P3C-03c metric-compare engine verifier."""
...
import verify_p3c_03c_metric_compare as GATE


class MetricCompareTests(unittest.TestCase):
    ...
    def test_plan_row_present(self) -> None:
        self.assertIn("**P3C-03c", GATE.PLAN.read_text(encoding="utf-8"))
    ...
    def test_profile_agnostic_charter(self) -> None:
        charter = GATE.load_yaml(GATE.CHARTER)
        self.assertTrue(charter["admission"]["profile_agnostic"])
        self.assertFalse(charter["admission"]["specific_metric_profile"])

    def test_rejects_specific_profile_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["specific_metric_profile"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.MetricCompareError):
                    GATE.validate(ROOT)
