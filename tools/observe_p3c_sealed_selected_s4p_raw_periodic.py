"""Observe the selected S4P through the fixed raw-periodic transform chain."""
...
RUNNER = Path("crates/sipi-p3c/tests/p3c_sealed_s4p_external_raw_periodic_runner.rs")
INVENTORY = (
    Path("Cargo.toml"),
    Path("Cargo.lock"),
    ...
    Path("crates/sipi-ieee-com-sparam/SOURCE-MAP.md"),
    Path("crates/sipi-ieee-com-sparam/src/lib.rs"),
    ...
    RUNNER,
    Path("crates/sipi-touchstone/src/lib.rs"),
...
RUNNER_SCHEMA = "sipi.p3c.sealed-selected-s4p-raw-periodic-runner.v1"
...
def parse_report(path: Path) -> dict[str, Any]:
    ...
    common = {"manifest_sha256", "record_count", "raw_periodic_status"}
    ...
    if first["manifest_sha256"] == second["manifest_sha256"] or first["record_count"] != second["record_count"] or first["raw_periodic_status"] != second["raw_periodic_status"]:
        raise ObservationError("fresh_run_consistency_invalid")
    if value["status"] == "observed":
        fields = {"manifest_sha256", "record_count", "raw_periodic_status", "uniform_bin_count", "raw_sample_count", "frequency_step_bits", "sample_interval_bits", "endpoint_imaginary_residue_bits", "inverse_imaginary_residue_bits", "endpoint_imaginary_residue_bound_bits", "inverse_imaginary_residue_bound_bits", "uniform_spectrum_sha256", "raw_response_sha256"}
        bits = {"frequency_step_bits", "sample_interval_bits", "endpoint_imaginary_residue_bits", "inverse_imaginary_residue_bits", "endpoint_imaginary_residue_bound_bits", "inverse_imaginary_residue_bound_bits"}
        if any(set(run) != fields or run["raw_periodic_status"] != "admitted" or any(not isinstance(run[key], int) or isinstance(run[key], bool) or run[key] <= 0 for key in ("uniform_bin_count", "raw_sample_count")) or any(not hex_string(run[key], 16) for key in bits) or any(not hex_string(run[key], 64) for key in ("uniform_spectrum_sha256", "raw_response_sha256")) for run in (first, second)):
        ...
        fields = {"manifest_sha256", "record_count", "raw_periodic_status", "stage", "error"}
        if any(set(run) != fields or run["raw_periodic_status"] != "rejected" or not isinstance(run["stage"], str) or not run["stage"] or not isinstance(run["error"], str) or not run["error"] for run in (first, second)) or first["stage"] != second["stage"] or first["error"] != second["error"]:
...
def observe(source: Path, report: Path, cargo: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="sipi-p3c-raw-periodic-") as temporary:
        archive_root = Path(temporary) / "archive"
        archive_root.mkdir()
        commit, inventory = clean_archive(archive_root)
        runner_report = Path(temporary) / "runner.json"
        env = os.environ.copy()
        env.update({"SIPI_P3C_SEALED_S4P_EXTERNAL_SOURCE": str(source), "SIPI_P3C_SEALED_S4P_RAW_PERIODIC_RUNNER_REPORT": str(runner_report), "CARGO_TARGET_DIR": str(Path(temporary) / "target"), "CARGO_INCREMENTAL": "0", "CARGO_NET_OFFLINE": "true"})
        outcome = subprocess.run([str(cargo), "test", "-p", "sipi-p3c", "--locked", "--offline", "--test", RUNNER.stem, "--", "--ignored", "--exact", "p3c_sealed_s4p_external_raw_periodic_runner_v1"], cwd=archive_root, env=env, capture_output=True, text=False, check=False)
