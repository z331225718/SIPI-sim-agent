"""Tests for the P5-04d discrete PDF stage verifier."""
...
import verify_p5_04d_discrete_pdf_stage as GATE


class PdfStageTests(unittest.TestCase):
    ...
    def test_source_map_has_six_mappings(self) -> None:
        ...
        self.assertIn("combine_r480_noise_pdf", source_map["not_ported"])
    ...
    def test_plan_row_present(self) -> None:
        ...
        self.assertIn("**P5-04d", plan_text)

    def test_rejects_combination_admission(self) -> None:
        import copy
        import tempfile
        import unittest.mock as mock
        charter = copy.deepcopy(GATE.load_yaml(GATE.CHARTER))
        charter["admission"]["combined_noise_pdf"] = True
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(GATE, "CHARTER", tmp_path):
                with self.assertRaises(GATE.PdfStageError):
                    GATE.validate(ROOT)
