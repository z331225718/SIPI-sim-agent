# -*- coding: utf-8 -*-
"""P4A-03aq typed IBIS series pin Model Selector binding cross-check (product vs independent ref).

Drives the product series pin Model Selector binding runner (p4a_03aq_series_pin_table_selector_runner) over valid
...
EVIDENCE = ROOT / "docs" / "baselines" / "p4a-03aq-series-pin-table-selector-crosscheck-evidence.v1.yaml"
EVIDENCE_SCHEMA = "sipi.p4a-03aq.series-pin-table-selector-crosscheck-evidence.v1"
POLICY = "sipi.p4a-03aq.series-pin-table-selector-v1.typed-table-selector"
...
def main() -> int:
    built = subprocess.run([str(CARGO), "build", "-p", "sipi-ibis", "--test", "p4a_03aq_series_pin_table_selector_runner"],
                           capture_output=True, text=True, encoding="utf-8", errors="replace", cwd=ROOT)
    if built.returncode != 0:
        raise SystemExit("runner build failed: " + built.stdout + built.stderr)
    runner = sorted((ROOT / "target" / "debug" / "deps").glob("p4a_03aq_series_pin_table_selector_runner-*.exe"))[-1]
    ...
    ok_all = True
    entries = []
    with tempfile.TemporaryDirectory(prefix="p4a-03aq-") as tmp:
        work = Path(tmp)
        for case in cases:
