...
详细运行日志默认写入 waveform/JSON 输出同名的 `.log`；没有指定输出文件时写入网表同名 `.log`，也可用 `--log PATH` 覆盖。终端中的 TRAN 只保留一条原地刷新的 ASCII 进度条，显示百分比、点数和 ETA，不会逐行刷屏。

日志会列出每个 RFM 的端口数、唯一极点数、实际动态 mode/状态数、旧稠密布局的等效历史内存及压缩倍数，以及 PWL 源/采样点数量。大 PWL/PWLFILE 的当前值和下一断点均使用二分定位，不会随已运行时间线性变慢。运行中的 TRAN 日志会同步记录每个输出点对应的内部步数、PWL 断点数和数值重分解次数；结束时的 `rfm-stamp`、`numeric-solve`、`cached-solve`、`rfm-candidate`、`lte` 和 `output` 可直接区分模型状态更新、矩阵重分解、误差控制与结果写出瓶颈。
