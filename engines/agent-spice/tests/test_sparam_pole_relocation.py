...
def test_precompressed_reciprocal_relocation_matches_public_path():
    freqs = np.array([1.0e6, 2.0e6, 5.0e6, 9.0e6], dtype=float)
    poles = np.array([-1.0e7 + 0.0j, -2.0e7 + 3.0e7j], dtype=complex)
    s11 = np.array([0.1 + 0.01j, 0.2 + 0.03j, 0.3 + 0.02j, 0.4 + 0.04j])
    s12 = np.array([0.5 - 0.02j, 0.45 - 0.03j, 0.35 - 0.01j, 0.25 - 0.02j])
    ...
    actual = _streaming_reciprocal_pole_relocation_precompressed(
        poles, freqs, compressed_responses, compressed_weights, multiplicities, True, False
    )
    expected = streaming_reciprocal_pole_relocation(poles, freqs, responses, weights, True, False)
