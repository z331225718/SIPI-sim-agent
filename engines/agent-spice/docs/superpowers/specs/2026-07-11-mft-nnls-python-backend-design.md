...
Task 4a fixture classes are explicit. The `ex4_S` raw-start one-step cases are numerical diagnostics, not response-quality gates: MATLAB itself has fixed-pole RMS values `0.2853`, `0.2636`, `0.2399`, and `0.2430` for orders `4`, `5`, `6`, and `7` respectively (orders `9` and `13` remain similarly above `0.01`). Orders `4` and `6` retain strict pole checks; the other raw-start cases remain strict expected-failure diagnostics so a future numerical change cannot silently hide them. The Task 4a acceptance gate is instead the checked-in, deterministic, order-six well-conditioned rational matrix fixture: MATLAB fixed-pole RMS must be below `0.01`, relocated poles use the pole rule above, and the fitted frequency response uses `rtol=1e-7`, `atol=1e-10`. This distinguishes a meaningful response-equivalence proof from an ill-conditioned raw-start coordinate comparison without relaxing either acceptance tolerance.
