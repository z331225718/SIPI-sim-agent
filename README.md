...
该模式完整计算 2x2 差分 S 矩阵、频域 Redheffer 星积级联、源/负载端接加载、DC/Nyquist 覆盖诊断、离散采样无损/互易性诊断，并在最终执行一次严格保留 $t=0$ 原点的 FD-to-TD 变换；同时导出 `frequency-response.csv` 与多级节点 `cascade-nodes.csv`。启用统计分析时另导出 `eye-metrics.csv`、`bathtub.csv`、`eye-contours.csv` 并在离线报告中提供互动浴盆曲线视图；配置 DFE 时导出 `dfe-adaptation.csv`、`dfe-events.csv` 并严格校验训练窗口后的权重零漂移不变性；配置 PAM4 调制时自动计算四电平、三门限、三眼高/宽、RLM 与 SER/BER 并在报告中直观呈现；配置 Aggressors 时执行多通道 NEXT/FEXT 感应噪声卷积、时域无混叠线性叠加并计算 ICN 与 SCR 指标；配置 Measured TX 脉冲时执行时域符号线性重叠相加（Overlap-Add）取代矩形保持；执行 `sipi channel sweep` 时进行受物理能量约束的 CTLE/FFE 参数网格扫描与自动调优并导出 `sweep-results.csv`；支持通过 `sipi ami run` 安全执行符合 IBIS 5.0~7.2 规范的 Windows x64 AMI 动态库模型（`AMI_Init`、`AMI_GetWave`、`AMI_Close`）并导出波形、时钟流与参数文本；内置三大经典 SI/PI 对照基准：S 参数频域对比（微带线衰减、过孔/残桩 15 GHz 陷波谐振、4 端口差共模转换）、DDR 内存接口对比（DDR4-3200 / DDR5-6400 DQ 拓扑、ODT 匹配、JEDEC VdIVW x TdIVW 掩模合规与 4-Tap DFE 闭环）、瞬态与 PDN 对比（TDR 阶跃阻抗剖面 Z(t)、多阶去耦电容抗谐振峰与供电轨动态下陷 Droop）。
