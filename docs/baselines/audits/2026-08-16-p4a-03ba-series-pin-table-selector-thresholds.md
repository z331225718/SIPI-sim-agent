# P4A-03ba Typed IBIS Series Pin Mapping Model Selector Threshold Table Core — Audit Record

- Date (UTC): 2026-08-16
- Scope: P4A-03 sub-slice 03ba (typed IBIS [Series Pin Mapping] Model Selector threshold table binding core)
...
- Cross-check: 3 test cases (full selector threshold record, minimal selector threshold record, empty pin name)
  driven through product runner `p4a_03ba_series_pin_table_selector_thresholds_runner`; independent Python reference
  matches 100% on valid flags, pin names, selector names, threshold parameters, and error strings; 3/3 matched_hash_bound.

## Binding

- Verifier `verify_p4a_03ba_series_pin_table_selector_thresholds.py` + 6 tests; crosscheck evidence
  `docs/baselines/p4a-03ba-series-pin-table-selector-thresholds-crosscheck-evidence.v1.yaml`.
- Charter `p4a-03ba-series-pin-table-selector-thresholds-stage.v1.yaml`; source map
  `p4a-03ba-mit-source-map.v1.yaml`.
- PLAN **P4A-03ba**; ledger note/gate P4A-03; coverage gates 182 -> 183.
