# P4A-03bf Typed IBIS Series Pin Mapping Group Model Binding Core — Audit Record

- Date (UTC): 2026-08-16
- Scope: P4A-03 sub-slice 03bf (typed IBIS [Series Pin Mapping] group model binding record core)
...
- Cross-check: 3 test cases (full group model record, minimal group model record, empty model name)
  driven through product runner `p4a_03bf_series_pin_table_group_runner`; independent Python reference
  matches 100% on valid flags, group names, model names, function table groups, and error strings; 3/3 matched_hash_bound.

## Binding

- Verifier `verify_p4a_03bf_series_pin_table_group.py` + 6 tests; crosscheck evidence
  `docs/baselines/p4a-03bf-series-pin-table-group-crosscheck-evidence.v1.yaml`.
- Charter `p4a-03bf-series-pin-table-group-stage.v1.yaml`; source map
  `p4a-03bf-mit-source-map.v1.yaml`.
- PLAN **P4A-03bf**; ledger note/gate P4A-03; coverage gates 187 -> 188.
