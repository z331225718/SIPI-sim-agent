# AS-06 run-rfm immutable v2 audit
...
The two reports are immutable-archive-bound, independently nonce/run-id tagged, report-SHA distinct, and use the same recorded cargo/rustc/python identities. RFM parse/convert observation; external execution fail-closed. No numerical parity or acceptance tolerance is claimed.
