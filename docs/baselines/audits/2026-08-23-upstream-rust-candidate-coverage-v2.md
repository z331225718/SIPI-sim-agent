...
# Upstream to Rust Candidate Coverage Audit (Current Rebind)

Date: 2026-08-23

Evidence: `docs/baselines/upstream-rust-candidate-coverage.v2.yaml`
Predecessor: `docs/baselines/upstream-rust-candidate-coverage.v1.yaml`

This additive successor preserves the historical v1 audit and its open
classification. It only rebinds the three current Agent-COM/COM-02 candidate
surface hashes after the Rust preparation wave changed those source bytes.
The fifteen rows remain `completion: open`, `parity: not_evaluated`, and
`product_capability: not_claimed`. No row is promoted to direct Rust parity,
acceptance, or release readiness.

The current source bindings are mechanically checked against the live
candidate files and the pinned upstream Git objects. External process
transport, self-tests, bounded profiles, and product CLI reachability remain
non-parity evidence under the same rules as v1. The v1 document and verifier
remain retained for historical auditability; this v2 record is the current
source-drift successor only.

This record does not change PLAN, migration ledgers, product-boundary,
license, or source-map decisions.
